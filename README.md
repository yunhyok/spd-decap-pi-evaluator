...
> **Current AV-BS1 H4-P0R-P1 boundary:** five public P1 invocations are immutable and all five one-use tokens were retired without reuse. The fifth invocation used token-only commit `5ba4b69398f526a0fcf640cf1dc4e7197cc7e660`, child of clean retry-v4 contract `099db849564207b636f7431ee2fb52a540a7cb4e`. It created the claim and reached factor-only child PID `55552`, which remained visible for `108` successful resource samples, but the factor monitor stopped on `NONROOT_DISAPPEARANCE_NOT_CONFIRMED` because the four active factor tree-sample calls still used default `MaximumAttempts=1`. No factor prefix or certificate completed; `factorization_attempted` and `factorization_performed` are therefore `null`, while RHS/solve/H4 physics/PowerSI work is definitively absent. The outer close SHA-256 `9d0c0aab58e78eb5d91d36be2aef46bf89968df87d4abcc044089805acce9af2` independently recorded `27` confirmed disappearances but retained only the bounded `16`, so `tree_sample_retry_events_truncated=true`, the mandatory outer gate failed, and no seal was written. The token was consumed in commit `46c08d405f530cce0cfbba9d908f8c266a26a002` and removed by retirement commit `71d3dab442cbdfa6361e4e91de57d8f5b4d1a990`. Retry-v5 changes only the four active factor sample calls to explicit maximum `3` and raises the shared bounded outer/control retry-event cap from `16` to `64`; every retry predicate and all other fatal/default-one paths remain unchanged. The observed zero-byte-stdout schema overlay is documented but deferred outside retry-v5. Frozen SHA-256 bindings are Python `54aa7da9daa013cec41585ae07757e6c54e8e9d15da7f4f16d4f6546b6fb35aa`, runner `3888524877f7a90966fb932f7f9fc4c9a48480134eb6295712eaa0ef548416a3`, and tests `b51498ebe27a0210a09b3a12b26e0146d39c1249906469bcb1add1d2f2443f08`; focused `29/29` and full no-cache `324/324` passed. Current token state is absent, factor fit remains unproven, and `next_stage_authorized=false`. See the [results](docs/evaluation-research/T1_AV_BOUNDARY_SCHUR_RESULTS.md) and [P1 preregistration](docs/evaluation-research/T1_AV_BOUNDARY_SCHUR_H4_P0R_P1_PREREG.md).

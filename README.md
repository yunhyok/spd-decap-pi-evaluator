...
> **Current AV-BS1 H4-P0R-P1 boundary:** three public P1 invocations stopped before claim and factor work, and all three one-use tokens were retired without reuse. The third invocation reached the outer and control ready/start releases, then failed closed when the control preflight still used the one-attempt tree-sample default and exhausted a confirmed-descendant-disappearance retry. The token-absent control-plane retry-v3 candidate now gives only six explicitly instrumented outer-observer sampling contexts plus six explicitly instrumented control-plane sampling contexts (12 total) three attempts with a 16-event typed evidence cap; factor sampling retains the default `MaximumAttempts=1`. Its Python/runner/tests are frozen and **312 static tests passed**, but no H4-P0R factorization or H4 physics ran, no PowerSI or 8 GiB result is claimed, and `next_stage_authorized=false`. See the [results](docs/evaluation-research/T1_AV_BOUNDARY_SCHUR_RESULTS.md) and [P1 preregistration](docs/evaluation-research/T1_AV_BOUNDARY_SCHUR_H4_P0R_P1_PREREG.md).

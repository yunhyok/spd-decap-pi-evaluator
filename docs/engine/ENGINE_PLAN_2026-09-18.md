...
- W14-a 완료(`W14A_REPORT.md`): `Rail.mesh → MeshedRail → solve`, numerics_id 불변, Port14 비트 동일. W14-b 완료(`W14B_REPORT.md`): 판정 **R3**(200 vs 400 8/9, 100 vs 200 1/9); h=400 ≈ h=200, h=100은 3–100 MHz에서 |Z| +1–1.8 dB(패키지), 1 MHz 아래 ≤ 0.15 dB → R 결손 격자 무관. 부록 W14-b-2 판정 **(i)**: h=100이 고주파에서 참조에 더 가까움(패키지 6/7, PCB 2/2; 개선 0.09–0.63 dB, 잔차 0.6–3.9 dB, G2 3건 상실). 소유자 결정 대기: (1) 기본 격자(권장 h=200 유지, h=100은 옵션), (2) G4 f_res 항이 고정값 1.585 MHz와 비교하는 결함 수정(`receipt.py`, 비수치). W14-c는 엔진 쪽(격자 검사 함수·validity 노트)만 진행, 제품 배선은 보류 권장.

## 결정 기록 E6–E7 (2026-09-20, 소유자 — "권장 사항대로")
- **E6. 기본 격자는 h=200 µm 유지.** W14-b/b-2 근거: h=100은 고주파 참조 오차를 0.09–0.63 dB 줄이지만 잔차(패키지 0.6–2.2 dB, PCB 3.4–3.9 dB)의 대부분은 격자가 아니며, 3케이스가 G2를 잃고, 비용은 미지수 2.1배·벽시계 1.4배다. 영수증·계약·테스트는 h=200에 고정. h=100(`ModelOptions(h=100, sub=(10,10,10))`)은 문서화된 옵션이며 `python -m spd_pi_engine converge --pair fine`으로 검토할 수 있다. 재검토 조건: 워크스테이션에서 92포트 전체 + PowerSI MaxEdgeLength 재시뮬레이션(참조 자체의 격자 의존, 인계 §6-2).
- **E7. 엔진 결과의 수렴 게이트 면제 승인(R3), 기본 프로파일은 옵트인 유지.** 제품 게이트(`_rejected_comparison_convergence`)는 엔진 프로파일을 건너뛰고, 정확도 근거는 영수증 validity 노트(6줄, 격자 민감도 포함)와 `numerics_id`다. 엔진 쪽 격자 검사(`convergence.py`, CLI `converge`)는 있으나 제품 배선(`convergence_check`)은 보류. 기본 프로파일을 엔진으로 바꾸는 시점은 패키지 급전 경로 R 결손(참조/모델 1.46배)이 해소되거나 문서화된 편향으로 수용될 때다.
- G4 게이트 정정(D10, `receipt.py` 참조 f_res 사용)은 W14-d로 진행(`W14D_REPORT.md`).

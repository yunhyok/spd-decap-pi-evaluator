...
기존 코드(`mfdm.py:782 copper_surface_impedance`)는 one-face 식(내부 L=μ0t/3)을 양측 귀환 셀에도 오용했다(EXP-4 A1). 양측 셀에 `½·Zc·coth(γt/2)`(내부 L=μ0t/12)를 적용해 G3를 12.7%→9.1%로 개선, 격자에도 견고(9.0%@fine25). 내부 L=0(EXP-4 (a))이 더 잘 맞지만(7.7%) 물리적 근거가 없어 채택하지 않음. 근거: `EXP4_REPORT.md`.

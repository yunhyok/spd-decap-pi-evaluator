...
- 문서 버전: **1.136**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.135
...
| 현재 active work item | `W7-PHYS-W6-PLANE-GEOMETRIES-PRODUCER-CALLER-BINDING-TRACE` (17DD) |
| 다음 권장 묶음 | `17DD` exact once-only producer→caller binding trace (canonical command fenced only in this section) |
| current authorization | 17DC consumed at clean main HEAD `1124ec19858061412631a0baa23e3cd955d2ce88`; exact AST owner diagnostic ran once, exit0, exactly 2 metadata lines (3226→`_parse_shapes` 2962-3276 span315; 9090→`analyze_spd` 8877-9544 span668), with no body/temp/dependency read. 17DD permits exactly one bounded Get-Content command: ranges A3200-3276 (77 lines) and B9060-9140 (81 lines), total158 ordered nonoverlap lines; cut/delegated origin/helper/container or any additional search/read stops BLOCKED/source-classified with no extension or retry. |
...
| 고비용 검증 권한 | none; 17DD allows only the bounded producer→caller binding read; additional source search/read, artifact/test/import/build/code/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-GEOMETRIES-PRODUCER-CALLER-BINDING-TRACE` | 17DD | ACTIVE | bind producer ranges A3200-3276 and caller ranges B9060-9140 for the two assignment targets | exact fenced command once in Work §17DD: one Get-Content read, A=77 lines and B=81 lines, total158 ordered/nonoverlapping numbered lines; five binding questions must pass, otherwise BLOCKED/source-classified STOP with no extension/search/reread |
...
### 17DD. W7-PHYS-W6-PLANE-GEOMETRIES-PRODUCER-CALLER-BINDING-TRACE (ACTIVE)
...
Gate: exact once, exit0, exactly 158 numbered lines from one `Get-Content` pass: A3200-3276 (77 lines) followed by B9060-9140 (81 lines), ordered and nonoverlapping. Five questions must all pass from these bounded lines: (1) producer construction is present and complete; (2) caller assignment consumes that producer; (3) layer/net/geometry/source identity fields retain one deterministic mapping; (4) no required origin, helper, container, or serialization step is cut or delegated outside the ranges; and (5) the two ranges form the exact 158-line contract. Any count/order/cut/delegated-origin/helper/container failure, additional search/read, or range extension stops with BLOCKED/source-classified STOP and no retry. This is a producer→caller synthesis only; it does not authorize implementation, artifact, test, production, accuracy, or ownership claims. Exclusions remain asset/SHA/net/layer meaning beyond the bounded mapping, islands, stamps, replacement, profile, nullspace, Zii, implementation, test, production, accuracy, and release.
## 20. 변경 기록

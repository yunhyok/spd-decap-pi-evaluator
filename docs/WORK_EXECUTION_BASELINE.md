...
- 문서 버전: **1.51**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.50
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S1` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S1` (ACTIVE; typed StackupLayer/fixture static gate + Sol review + one focused node) |
| current authorization | 17AA consumed at `4af8ae120469be2fc3bf99ff30f07b1a5e961565`; its exact `pytest -q` node ran once and failed (exit1, `1 failed in 1.55s`) with `RAW_SPATIAL_PLANE_SHEET_REQUIRED: ProjectSpec stackup physical fields are incomplete`, so no rerun. 17AA-S1 permits only `src/spd_decap_pi/raw_spatial_contact_compiler.py` and `tests/test_raw_spatial_contact_compiler.py`; inspect actual typed StackupLayer/discriminator/thickness_um/material_name/conductivity_s_per_m and dielectric-point epsilon/loss fields, with no aliases/ad-hoc metadata/monkeypatch. Sol reviews before the same node once; failure stops. The retained uncommitted diff also includes `src/spd_decap_pi/raw_spatial_contact_asset.py`; no production/old-root/release/broad-suite work |
...
| 고비용 검증 권한 | one bounded S1 compiler/test static correction + one Sol review + one focused node are authorized; failure stops with no rerun and production remains unauthorized; stop at Usage Guard 50% or further command |
...
| `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION` | 17AA | BLOCKED (test-gate) | implement opt-in hash-bound schema-v3 plane-sheet payload and focused test | exact `pytest -q` command ran once, exit1, `1 failed in 1.55s`; `_plane_sheet_payload` returned `RAW_SPATIAL_PLANE_SHEET_REQUIRED` / `ProjectSpec stackup physical fields are incomplete`; no rerun; retained uncommitted technical diff is the three paths above |
| `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S1` | 17AA-S1 | ACTIVE | typed StackupLayer/fixture contract correction before the same focused node | exact editable whitelist `src/spd_decap_pi/raw_spatial_contact_compiler.py`, `tests/test_raw_spatial_contact_compiler.py`; raw asset diff remains retained/frozen; bind actual discriminator, thickness_um, material_name, conductivity_s_per_m and dielectric-point epsilon/loss fields without aliases/metadata/monkeypatch; Sol review then one node, PASS may proceed, FAIL stops/no rerun |
...
17Z는 DONE으로 닫고, 17AA는 test-gate BLOCKED, ACTIVE는 `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S1`로 지정한다.
...
S1 is now the sole ACTIVE bounded successor. Its editable whitelist is
`src/spd_decap_pi/raw_spatial_contact_compiler.py`, `tests/test_raw_spatial_contact_compiler.py`;
the raw asset diff remains retained/frozen. Exact whitelist before the failure was
...
`tests/test_raw_spatial_contact_compiler.py`다. v2 constants/API는
...
Sol은 4-file diff를 먼저 review하고, 정확히 한 번만
`pytest -q tests/test_raw_spatial_contact_compiler.py::test_plane_sheet_payload_v3_is_hash_bound_and_v2_remains_unchanged`를 실행한다. 실패 시 rerun하지
않는다. Production/accuracy authority는 없다.

...
- 문서 버전: **1.52**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.51
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S2` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S2` (ACTIVE; one-file asset grouping fix + Sol review + one focused node) |
| current authorization | 17AA-S1 consumed at `4af8ae120469be2fc3bf99ff30f07b1a5e961565`; its exact `pytest -q` node ran once and failed (exit1, `1 failed in 1.51s`) at `raw_spatial_contact_asset.py::_plane_sheet_rows` line 1389 with `KeyError: 0` from direct `circles_by[row["primitive_ordinal"]]` for a polygon with no circle rows. No rerun. S2 permits only `src/spd_decap_pi/raw_spatial_contact_asset.py`; use `.get(primitive_ordinal, [])` for both grouped maps, retaining union and `_validate_plane_primitive` authority. Sol reviews before the same node once; failure stops. Compiler/test remain retained/frozen; no production/old-root/release/broad-suite work |
...
| 고비용 검증 권한 | one bounded S2 asset correction + one Sol review + one focused node are authorized; failure stops with no rerun and production remains unauthorized; stop at Usage Guard 50% or further command |
...
| `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S2` | 17AA-S2 | ACTIVE | asset grouping correction before the same focused node | exact editable whitelist `src/spd_decap_pi/raw_spatial_contact_asset.py`; use `.get(primitive_ordinal, [])` for both grouped maps, preserving union and `_validate_plane_primitive`; compiler/test retained/frozen; Sol review then one node, PASS may proceed, FAIL stops/no rerun |
...
17Z는 DONE으로 닫고, 17AA-S1은 test-gate BLOCKED, ACTIVE는 `W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S2`로 지정한다.
...
`circles_by[row["primitive_ordinal"]]` for a polygon with no circle rows; no rerun. This is a
source-classified asset grouping bug. S2 is now the sole ACTIVE bounded successor with editable
whitelist `src/spd_decap_pi/raw_spatial_contact_asset.py`; compiler/test remain retained/frozen.
The exact whitelist before the failure was

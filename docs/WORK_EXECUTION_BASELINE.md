...
- 문서 버전: **1.124**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.123
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-SYMBOL-OWNER-DISCOVERY` (17CR) |
| 다음 권장 묶음 | `17CR` exact once-only `_digest_hex` symbol-owner query |
| current authorization | 17CQ consumed at clean main `46b2cca9c1448ff7ad696fe066e2d1e512a16c74`; its exact boundary command ran once, exit3, zero output because the unique top-level definition condition failed. No body read/retry. 17CR permits one location-only query for `_digest_hex` in `src/spd_decap_pi/raw_spatial_contact_compiler.py`, requiring exit0, 1–12 unique lines/1 file and one unambiguous owner/use span <=900; ambiguity or cap/span failure stops with no retry. |
...
| 고비용 검증 권한 | none; 17CR allows only the symbol-owner location query; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-SYMBOL-OWNER-DISCOVERY` | 17CR | ACTIVE | locate one `_digest_hex` owner and actual use | exact query once; exit0, 1–12 unique lines/1 file, one top-level definition/import/alias plus use including frozen line861, nearest owner/use span<=900; location-only, no body or semantic inference |
...
### 17CR. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-SYMBOL-OWNER-DISCOVERY (ACTIVE)
...
Gate: exit0, 1–12 unique lines/1 file; exactly one top-level definition/import/import-list/alias owner and actual use including frozen line861; nearest owner/use span<=900. Missing/multiple owner, uses-only/comments-strings-only, dynamic ambiguity, or cap/span failure stops with no retry. Location-only; body/import target is separate. Asset-name meaning, islands, ownership, stamps, profile, replacement, nullspace, Zii, production, accuracy, and release remain out of scope.

...
- 문서 버전: **1.226**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.225; [Source-derived physical IR](SOURCE_DERIVED_PHYSICAL_IR.md) v1.60
...
| 현재 active work item | **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION** |
| 현재 정확성 상태 | `260729 retrospective FAIL`; unseen/generalization `unknown / not_run` |
| current authorization | `main`-only, `accuracy_parse.py` 보존. `tests/test_source_plane_ownership_ir_producer.py`의 남은 `with sqlite3.connect(evidence.spool.path)` 한 문장만 기존 `closing(...)`으로 감싼다. 모든 product/source IR/other tests/schema/caps는 동결. Sol static 뒤 12.59의 원인 노드 우선 exact 18 nodes once/300 s. Pressure/exact-300K/full suite/original SPD/solver/production/PowerSI 금지. |
| 다음 gate | **tamper connection fix** — test-only tamper SQLite handle 한 곳을 명시적으로 닫은 뒤 동일 bounded scope를 fresh/fail-fast로 한 번 |
...
| current candidate | DSU-01–05, Recovery-01/DIAG-01, original-SPD EVIDENCE-01/-02/-03/-04/-05와 OWNERSHIP-COUNT-DIAG-01/-02는 영구 DONE/STOP; owner-join, quotient-scope, terminal pad-layer test-oracle 및 compact-request-bound fixes are DONE/ACCEPT; streamed-IR-300K FIX-01은 collection STOP, collection-fix는 failure/runtime STOP, validation-runtime fix는 ACCEPT_WITH_OUTPUT_LIMITATION, lightweight acceptance는 oracle STOP, oracle-fix는 product selection omission STOP, material-selection fix는 streamed-handoff regression STOP, streamed-handoff fix는 test capture connection leak STOP, capture-connection fix는 test tamper connection leak STOP; P12 prospective review NO-GO; tamper-connection fix sole ACTIVE |
...
| runtime evidence | DIAG-02 exact report/receipt가 persisted됐다. WinAPI measured peak 13.09 GiB, current 1.42 GiB, scratch 131.6 MiB; expanded Node/Via 77,852/38,926, six-key selection 116,791, source-record lower bound 122,146, final-v2 lower bound 249,635. Validation-runtime pressure+300K process는 exit 0 / 22.8483887 s. Lightweight run은 oracle STOP, oracle-fix run은 product Material-kind omission STOP이다. Material-fix run은 raw-v3 handoff STOP이다. Streamed-handoff run은 test capture connection STOP이다. Capture-connection run은 18 collected 뒤 first node가 intended Node-layer `SpdImportError`에 도달했으나 test-only tamper connection 때문에 WinError 32 cleanup 실패, `1 failed in 2.44s`, child 1, wrapper 3.1867265 s, reusable PASS 0이다. |
...
| candidate staging / acceptance | producer test의 마지막 tamper connection close 한 문장만; 모든 product/source IR/other tests 동결; Sol static 뒤 culprit-first 18-node invocation 1회/300 s; accepted pressure/exact-300K와 full suite 재실행 금지 |
...
동시에 `ACTIVE`는 하나만 허용하며 현재 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01**이다. 완료된 미시적 item을
...
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01` | 53 | ACTIVE | explicitly close the final producer-test tamper SQLite connection | one statement only; product/other tests frozen; Sol static; culprit-first exact 18 nodes once/300 s |
...
flowchart LR
  ...
  CC --> TC[W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01<br/>FROZEN / READY_FOR_IMPLEMENTATION]
  R -->|예| H
...
### 12.59 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 — FROZEN / READY_FOR_IMPLEMENTATION

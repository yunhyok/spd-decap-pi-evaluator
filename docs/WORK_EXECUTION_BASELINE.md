...
- 문서 버전: **1.137**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.136
...
| 현재 active work item | `W7-PHYS-W6-PLANE-GEOMETRIES-ORIGIN-HANDOFF-DUAL-LOCATION-DISCOVERY` (17DF) |
| 다음 권장 묶음 | `17DF` exact once-only dual-location origin/handoff discovery (canonical command fenced only in this section) |
| current authorization | 17DD consumed at clean main HEAD `897f330ce59e6d36b29d066b703432d9644370c0`; exact producer→caller binding read ran once, exit0, 158 numbered lines, with Q1 geometry origin and Q4 downstream handoff UNPROVEN; 17DE frozen synthesis is BLOCKED/design REJECT. 17DF permits exactly one bounded `rg` command over `_core/io/spd.py`: A2962-3199 geometry init/population and B9105-9544 `plane_geometries` handoff, each 1-12 matches/span<=120, total 2-24; any cap, ambiguity, or external/delegated origin stops BLOCKED/location STOP with no retry/body read. |
...
| 고비용 검증 권한 | none; 17DF allows only the bounded dual-location `rg` discovery; additional source search/read, artifact/test/import/build/code/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-GEOMETRIES-ORIGIN-HANDOFF-DUAL-LOCATION-DISCOVERY` | 17DF | ACTIVE | locate geometry origin and exact `plane_geometries` handoff in bounded ranges | exact fenced `rg` command once in Work §17DF; A2962-3199 and B9105-9544 each 1-12 matches/span<=120, total2-24; cap/ambiguity/delegation STOP/no retry/body |
...
### 17DF. W7-PHYS-W6-PLANE-GEOMETRIES-ORIGIN-HANDOFF-DUAL-LOCATION-DISCOVERY (ACTIVE)

The exact command is run once:

```powershell
$path = 'src/spd_decap_pi/_core/io/spd.py'
$pattern = '\bgeometry\b(?:\s*:\s*[^=]+)?\s*=|\bgeometry\s*\[[^]]+\]\s*=|\bgeometry\.(?:setdefault|update)\s*\(|\bplane_geometries\b'
$hits = @(rg -n --with-filename --no-heading --color never $pattern $path)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$rows = foreach ($line in $hits) {
    if ($line -notmatch '^(.+):(\d+):(.*)$') { exit 2 }
    [pscustomobject]@{ Path = $Matches[1]; Number = [int]$Matches[2]; Text = $Matches[3]; Raw = $line }
}
$a = @($rows | Where-Object {
    $_.Number -ge 2962 -and $_.Number -le 3199 -and
    $_.Text -match '\bgeometry\b(?:\s*:\s*[^=]+)?\s*=|\bgeometry\s*\[[^]]+\]\s*=|\bgeometry\.(?:setdefault|update)\s*\('
} | Sort-Object Number)
$b = @($rows | Where-Object { $_.Number -ge 9105 -and $_.Number -le 9544 -and $_.Text -match '\bplane_geometries\b' } | Sort-Object Number)
$selected = @($a + $b)
if ($a.Count -lt 1 -or $a.Count -gt 12) { exit 3 }
if ($b.Count -lt 1 -or $b.Count -gt 12) { exit 4 }
if ((($a[-1].Number - $a[0].Number) + 1) -gt 120) { exit 5 }
if ((($b[-1].Number - $b[0].Number) + 1) -gt 120) { exit 6 }
if ($selected.Count -lt 2 -or $selected.Count -gt 24) { exit 7 }
if (@($selected | Where-Object { $_.Path -ne $path }).Count -ne 0) { exit 8 }
if ($a[-1].Number -ge $b[0].Number) { exit 9 }
if (@($selected | Select-Object -ExpandProperty Raw -Unique).Count -ne $selected.Count) { exit 10 }
$output = @(
    $a | ForEach-Object { 'A:{0}' -f $_.Raw }
    $b | ForEach-Object { 'B:{0}' -f $_.Raw }
)
if ($output.Count -ne $selected.Count) { exit 10 }
$output
```

Gate: exact once, exit0; A is text-filtered actual geometry initialization/index assignment or `setdefault`/`update` within 2962-3199, and B is an exact `plane_geometries` handoff within 9105-9544. Each yields 1-12 lines with inclusive span<=120, total 2-24 unique lines from one file, ordered A before B; output is explicitly labeled A/B. Annotation-only, unrelated reads/comparisons, multiple clusters, ambiguous/delegated origin/helper/container, or any persisted-vs-runtime `plane_geometries` non-equivalence stops BLOCKED/location STOP with no retry/body read. A successful location result only permits a separately gated producer→caller synthesis; no implementation or physical/accuracy claim is authorized. Exclusions remain asset/SHA/net/layer meaning, islands, ownership, stamps, replacement, profile, nullspace, Zii, implementation, test, production, accuracy, and release.
## 20. 변경 기록

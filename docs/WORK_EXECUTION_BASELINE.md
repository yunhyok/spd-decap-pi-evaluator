...
- 문서 버전: **1.40**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.39
...
| 현재 active work item | `W7-PHYS-W6-PLANE-SHEET-SOURCE-CONTRACT-TRACE` |
| 다음 권장 묶음 | 17S exact three-file complete-read source-contract trace; one Sol review; no code/test/artifact/production execution |
| current authorization | clean `main` HEAD `78cd240dff6b704595574d888da4b9c089ae39ab`; read exactly once each: `src/spd_decap_pi/_core/services.py`, `src/spd_decap_pi/_core/solver/surface_certificate_asset.py`, `src/spd_decap_pi/_core/solver/layer_surface_termination.py`; no other source/search/import/Python/test/artifact/solver/production/edit |
...
| 고비용 검증 권한 | 17S bounded three-file read-only source trace + one Sol review only; no code/test/artifact/Python/import/solver/production execution; any external delegate closes 17S BLOCKED with no fourth read or automatic whitelist expansion |
...
| `W7-PHYS-W6-PLANE-SHEET-SOURCE-CONTRACT-TRACE` | 17S | ACTIVE | bind services/profile enforcement, hash-bound surface certificate inputs, balanced termination/nullspace and stable rail/Zii, and Via/termination ownership | clean `78cd240dff6b704595574d888da4b9c089ae39ab`; exact whitelist `src/spd_decap_pi/_core/services.py`, `src/spd_decap_pi/_core/solver/surface_certificate_asset.py`, `src/spd_decap_pi/_core/solver/layer_surface_termination.py`; one complete read each; any external delegate including unknown raw-spatial compiler closes 17S BLOCKED with no fourth read/expansion |
...
tests, or execution is permitted. At 17R closure the active item was NONE; subsequent user
approval makes 17S the sole bounded read-only item. Production runs, old-root reuse, release, and
parent accuracy/causal promotion remain unauthorized.

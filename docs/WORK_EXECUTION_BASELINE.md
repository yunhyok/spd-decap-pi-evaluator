...
- 문서 버전: **1.121**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.120
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-POPULATION-BODY-TRACE` (17CO) |
| 다음 권장 묶음 | `17CO` exact once-only 23-line record-by-key population body read |
| current authorization | 17CO body gate: exact read once lines780-802 (23 lines); complete records source/container, raw-row key + blank/duplicate guard, asset/asset_sha256 validation, record_by_key population, and geometry lookup key; endpoint/delegated normalization/unproven domain STOP/no expansion/retry. |
...
| 고비용 검증 권한 | none; 17CO allows only the bounded 23-line body read; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-POPULATION-BODY-TRACE` | 17CO | ACTIVE | trace record-by-key population body | exact body read once lines780-802 (23 lines); require records source/container, raw-row key/guard, asset/asset_sha256 validation, record_by_key population and geometry lookup key; endpoint/delegation/domain STOP/no expansion |
...
### 17CO. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-POPULATION-BODY-TRACE (ACTIVE)
...
if ($lines.Count -lt 802) { exit 2 }
$output = for ($line = 780; $line -le 802; $line++) {
    '{0}:{1}:{2}' -f $path, $line, $lines[$line - 1]
}
if ($output.Count -ne 23) { exit 3 }
$output
```

PASS only if records source/container, raw-row key with blank/duplicate guard, asset/asset_sha256 validation, record_by_key population, and geometry lookup key are complete. Endpoint/delegated field or SHA validation, normalization, or key-domain equality => BLOCKED/STOP; no expansion/retry. Location/body scope only; islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.

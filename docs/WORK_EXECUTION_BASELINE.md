...
- 문서 버전: **1.119**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.118
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-RECORD-BY-KEY-POPULATION-DISCOVERY` (17CM) |
| 다음 권장 묶음 | `17CM` exact once-only record_by_key population discovery query |
| current authorization | 17CM location gate: exact query once; exit0 2-20 unique lines/1 file; actual init+records-based population+key construction+line802 lookup in one cluster span<=120; annotation/lookup-only/no population/multiple/span/cap STOP/no retry; no body read. |
...
| 고비용 검증 권한 | none; 17CM allows only the bounded location query, with source body/code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-RECORD-BY-KEY-POPULATION-DISCOVERY` | 17CM | ACTIVE | locate record_by_key population dataflow | exact query once; exit0 2-20 unique lines/1 file; initialization + records-based population + key construction + line802 lookup in one cluster span<=120; annotation/lookup-only/no population/multiple/span/cap STOP/no retry; no body read |
...
17CL is BLOCKED/source-classified STOP at `4c07698a84f03c5305b59a965061dff27a01d982` (exact query exit0, 3 lines/1 file, span38; record_by_key population and asset validation unproven). Sole current read-only successor 17CM owns the exact record_by_key population query. Frozen
...
### 17CM. W7-PHYS-W6-PLANE-PRIMITIVE-RECORD-BY-KEY-POPULATION-DISCOVERY (ACTIVE)
...
Gate: exit0, 2-20 unique lines/1 file; actual initialization + records-based population + key construction + line802 lookup must form one clear cluster span<=120. Annotation/lookup-only, no population, multiple/span/cap => STOP/no retry. Location-only; no body read/requery/expansion. Islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.

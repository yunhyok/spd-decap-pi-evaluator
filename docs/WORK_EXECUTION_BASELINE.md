...
- 문서 버전: **1.114**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.113
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-BOUNDARY-DISCOVERY` (17CH) |
| 다음 권장 묶음 | `17CH` exact once-only `_decode_token` definition-boundary query |
| current authorization | 17CH location gate: exact query once; exit0 exactly2 lines/1 file, unique `def _decode_token(` and immediate next top-level positive boundary, span<=120; any fail/duplicate/no boundary/span>120 STOP/no retry. |
...
| 고비용 검증 권한 | none; 17CH allows only the bounded definition-boundary query, with source body/code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-BOUNDARY-DISCOVERY` | 17CH | ACTIVE | locate decode-token owner boundary | exact query once; exit0 exactly2 lines/1 file, unique `_decode_token` def + immediate next top-level positive boundary, span<=120; any fail/duplicate/no boundary/over-span STOP/no retry |
...
17CD is BLOCKED/source-classified STOP, 17CE is BLOCKED/location STOP, 17CF is DONE/location PASS, 17CG is BLOCKED/source-classified STOP, and 17CH is the sole ACTIVE decode-token boundary discovery.
...
sole current read-only successor 17CH owns the exact `_decode_token` boundary query. Frozen
...
### 17CH. W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-BOUNDARY-DISCOVERY (ACTIVE)
...
$targets = @($numbered | Where-Object { $_.Text -match '^def _decode_token\(' })
if ($targets.Count -ne 1) { exit 3 }

$next = @($numbered |
    Where-Object { $_.Number -gt $targets[0].Number } |
...
Gate: one query, exit0 exactly2 lines/1 file; unique `def _decode_token(` plus immediate next top-level positive
boundary, span<=120. Any fail/duplicate/no boundary/span>120 STOP/no retry. Location-only; no source body. SHA/source

...
- 문서 버전: **1.126**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.125
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATION-USE-DISCOVERY` (17CT) |
| 다음 권장 묶음 | `17CT` exact once-only validation-use location query |
| current authorization | 17CS consumed at clean main `609617badf755977af1d3e54337a2b6ff7205a1b`; its line-861 read ran once, exit0, exactly one line: `digest = row.get("source_asset_sha256") if isinstance(row, Mapping) else None`, a simple accessor with no validator/helper/inline check. No retry or expansion. 17CT permits one query for `digest` lines 862–900 in `src/spd_decap_pi/raw_spatial_contact_compiler.py`, requiring exit0, 2–12 unique lines/1 file and a validation/use cluster span<=38; unrelated meaning, missing validation, use/multiple/alias/dynamic/cap/span cases stop with no retry. |
...
| 고비용 검증 권한 | none; 17CT allows only the bounded validation-use location query; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATION-USE-DISCOVERY` | 17CT | ACTIVE | locate format validation and downstream use | exact query once; exit0, 2–12 unique lines/1 file, all 862–900; validation/normalization plus downstream primitive/digest use in one cluster span<=38; unrelated/missing validation/use/multiple/alias/dynamic/cap/span STOP/no retry; location-only |
...
### 17CT. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATION-USE-DISCOVERY (ACTIVE)
...
$selected=@($hits | Where-Object {
    $_ -match '^[^:]+:(\d+):' -and [int]$Matches[1] -ge 862 -and [int]$Matches[1] -le 900
})
if ($selected.Count -lt 2 -or $selected.Count -gt 12) { exit 2 }
$selected
```

Gate: exact once, exit0, 2–12 unique lines/1 file all within lines 862–900; line861 validation/normalization and downstream primitive/digest use must form one cluster span<=38. Unrelated meaning, missing validation, use/multiple/alias/dynamic cases, or cap/span failure stops with no retry. Location only; line861 reread/body and helper/import target are out of scope. Asset-name meaning, islands, ownership, stamps, profile, replacement, nullspace, Zii, production, accuracy, and release remain excluded.

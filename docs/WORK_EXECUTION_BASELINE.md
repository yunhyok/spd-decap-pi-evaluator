...
- 문서 버전: **1.66**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.65
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-PRODUCTION-IMPORT-CALLER-DISCOVERY` | 17AO | ACTIVE | locate exactly one production import application caller | exact once query `rg -n --with-filename --no-heading --color never --glob '*.py' '\bimport_spd_scenario\b' 'src/spd_decap_pi'`; cap <=20 output lines/<=5 files; ignore import/definition, require exactly one actual application caller for later bounded trace; zero/multiple/error/overcap => STOP; location evidence only, no file open/code/test/production/accuracy proof |
...
### 17AO. W7-PHYS-W6-RAW-SPATIAL-V3-PRODUCTION-IMPORT-CALLER-DISCOVERY (ACTIVE)

Run exactly once:
`rg -n --with-filename --no-heading --color never --glob '*.py' '\bimport_spd_scenario\b' 'src/spd_decap_pi'`.
Cap <=20 output lines and <=5 files; ignore import/definition matches and require exactly one actual
application caller to select a later bounded trace; zero/multiple/error/overcap => STOP. Location
evidence only; no file open/code/test/production/accuracy proof.
## 20. 변경 기록

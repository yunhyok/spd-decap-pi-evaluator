...
- 문서 버전: **1.61**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.60
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DIAGNOSTIC` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DIAGNOSTIC` (ACTIVE; one diagnostic gate) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AH is DONE at clean main HEAD `d45a23f62ebd417a50498724bdea79ba77d8d041`: exact query once, exit0, 22 lines/1 file; frozen call list and metadata checks are recorded above, with no compiler/include flag matches. 17AI is BLOCKED at clean main HEAD `0b9e75f5367c12e76e8e06edb652f112d518b69d`: exact frozen PowerShell gate once, exit2, zero output; no rerun; failure stage unclassified among silent gates and no cause inferred. 17AJ permits only the exact diagnostic PowerShell gate below; success requires exit0, exactly 4 lines for one expected file, with final EOF sentinels valid only after a complete top-level test-definition scan. Any STOP/nonzero/count/path mismatch => STOP. Use only frozen 17AH non-import call lines for interval membership; select at most one owner iff exactly one interval contains an observed call; zero/both/overlap/ambiguity => STOP. No file open, code, test, profile, solver, accuracy, or production proof.
...
| 고비용 검증 권한 | one frozen 17AJ owner-boundary diagnostic gate + one Sol review are authorized; exact once/exit/count/path/STOP rules apply; no file open, code, test, or production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DIAGNOSTIC` | 17AJ | ACTIVE | diagnose the silent owner-boundary gate without opening source files | run exactly once the frozen PowerShell below; success exit0 exactly 4 lines for one expected file, with final EOF sentinel valid only after complete top-level test-definition scan; any STOP/nonzero/count/path mismatch => STOP; use only frozen 17AH non-import call lines for intervals and select at most one owner iff exactly one interval contains an observed call; zero/both/overlap/ambiguity => STOP; no files opened, code/test/inference |
...
### 17AJ. W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DIAGNOSTIC (ACTIVE)

Freeze exactly once:

```powershell
$path = 'tests/test_spd_decap_spd_adapter.py'
$targets = @(1750, 2487)
$defs = @(rg -n --with-filename --no-heading --color never '^(?:async\s+def|def)\s+test_' $path)
$rgExit = $LASTEXITCODE
if ($rgExit -ne 0) { "STOP:RG_EXIT:$rgExit"; exit 2 }
$numbered = @(
    foreach ($line in $defs) {
        $m = [regex]::Match($line, '^(.*):(\d+):(.*)$')
        if (-not $m.Success) { 'STOP:PARSE'; exit 2 }
        [pscustomobject]@{
...
if ($numbered.Count -ne $defs.Count) { 'STOP:COUNT'; exit 2 }
$files = @($numbered.File | Sort-Object -Unique)
if ($files.Count -ne 1) { 'STOP:FILE_COUNT'; exit 2 }
if (@($numbered.Number | Sort-Object -Unique).Count -ne $numbered.Count) { 'STOP:DUPLICATE_LINE'; exit 2 }
$out = @()
foreach ($target in $targets) {
    $before = $numbered | Where-Object Number -le $target | Select-Object -Last 1
    $after = $numbered | Where-Object Number -gt $target | Select-Object -First 1
    if ($null -eq $before) { "STOP:NO_BEFORE:$target"; exit 2 }
    $out += $before.Text
    $out += if ($null -eq $after) { "${path}:EOF_AFTER_TARGET_${target}:<EOF>" } else { $after.Text }
}
if ($out.Count -ne 4) { 'STOP:OUTPUT_COUNT'; exit 2 }
$out
```

Run exactly once. Success requires exit0 and exactly four lines referring to one expected file; the
final EOF sentinel is valid only because the complete top-level test-definition scan proves no later
test definition. Any STOP, nonzero exit, count mismatch, or path mismatch => STOP. Use only the frozen
17AH non-import call lines for interval membership; select at most one owner iff exactly one interval
contains an observed call; zero, both, overlap, or ambiguity => STOP. No file open, code, or test.

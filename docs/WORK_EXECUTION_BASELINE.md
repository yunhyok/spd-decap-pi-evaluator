...
- 문서 버전: **1.53**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.52
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-PERSISTENCE-OWNER-DISCOVERY` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-PERSISTENCE-OWNER-DISCOVERY` (ACTIVE; one read-only caller discovery query + Sol review) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AB permits only the exact query `rg -n --no-heading --color never --glob '*.py' '\b(?:build_compiled_topology_asset|COMPILED_TOPOLOGY_ASSET_METADATA_KEY|RAW_SPATIAL_CONTACT_ASSET_METADATA_KEY)\b' 'src/spd_decap_pi'`; cap <=60 lines and <=10 files, definitions/reexports-only or zero useful caller is source-classified STOP. No file open; select at most one existing non-test caller, with no code/test/profile/accuracy/production proof or whitelist expansion |
...
| 고비용 검증 권한 | one bounded 17AB read-only discovery + one Sol review + one focused query are authorized; cap/STOP rules above apply; no code/test/production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-PERSISTENCE-OWNER-DISCOVERY` | 17AB | ACTIVE | discover one existing persistence owner/caller | exact query only: `rg -n --no-heading --color never --glob '*.py' '\b(?:build_compiled_topology_asset|COMPILED_TOPOLOGY_ASSET_METADATA_KEY|RAW_SPATIAL_CONTACT_ASSET_METADATA_KEY)\b' 'src/spd_decap_pi'`; cap <=60 lines/<=10 files; definitions/reexports-only or zero useful caller => source-classified STOP; no file open |
...
17Z는 DONE으로 닫고, 17AA와 17AA-S1은 historical BLOCKED로 남기며 17AA-S2는 test-gate PASS DONE으로 닫는다. ACTIVE는 `W7-PHYS-W6-RAW-SPATIAL-V3-PERSISTENCE-OWNER-DISCOVERY`(17AB)로 지정한다.
...
`tests/test_raw_spatial_contact_compiler.py`다. 17AB is now the sole ACTIVE read-only discovery
with exact query
`rg -n --no-heading --color never --glob '*.py' '\b(?:build_compiled_topology_asset|COMPILED_TOPOLOGY_ASSET_METADATA_KEY|RAW_SPATIAL_CONTACT_ASSET_METADATA_KEY)\b' 'src/spd_decap_pi'`, cap <=60 lines/<=10 files, and STOP on definitions/reexports-only or zero useful caller. No file open or code/test/profile/accuracy/production proof is authorized. v2 constants/API는

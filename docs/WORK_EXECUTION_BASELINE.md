...
- 문서 버전: **1.139**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.138
...
- Sole ACTIVE item (current): 17DG `W7-PHYS-SPARSE-GAUGE-SAFE-SURFACE-PATCH-FINITE-PORT-CONDENSATION-FOUNDATION`; 17DF ACTIVE NONE is historical closure wording.
...
| 현재 active work item | 17DG `W7-PHYS-SPARSE-GAUGE-SAFE-SURFACE-PATCH-FINITE-PORT-CONDENSATION-FOUNDATION` (ACTIVE) |
| 다음 권장 묶음 | 17DG exact two-file implementation and focused gate, at most once after Sol static ACCEPT |
| current authorization | 17DG exact whitelist only: `src/spd_decap_pi/_core/solver/surface_patch_plane.py`, `tests/test_surface_patch_plane.py`; sparse gauge-safe finite-port condensation foundation as detailed below. No production run/release/accuracy authority. |
...
| 고비용 검증 권한 | no high-cost validation; only the exact focused 17DG pytest is authorized at most once after Sol static ACCEPT; no build/production/accuracy/release authority |
...
| `W7-PHYS-SPARSE-GAUGE-SAFE-SURFACE-PATCH-FINITE-PORT-CONDENSATION-FOUNDATION` | 17DG | ACTIVE | standalone sparse gauge-safe finite-port condensation foundation | exact two-file whitelist; Sol static ACCEPT; focused analytic gate once; standalone claim only |
...
17DF is closed with W7-PHYS BLOCKED. 17DG is now the sole ACTIVE item under the exact whitelist and gate below; no production run/release/accuracy authority is active.

### 17DG. W7-PHYS-SPARSE-GAUGE-SAFE-SURFACE-PATCH-FINITE-PORT-CONDENSATION-FOUNDATION (ACTIVE)

Parent Product baseline: v1.138; Work baseline: v1.139. This supersedes the terminal/current wording in the historical 17DF closure.
...
Focused gate (at most once after Sol static ACCEPT): `pytest -q tests/test_surface_patch_plane.py::test_surface_patch_finite_port_condensation_is_gauge_safe_and_fails_without_return`. Expected one-cell/two-conductor fixture returns `y[[1,-1],[-1,1]]`, reciprocal/passive/common-mode-null, sparse `P/K`; one-terminal input fails closed. Claim is standalone mathematically gauge-safe reciprocal/passive finite-port condensation only. Exclude production island attachment, production-scale sparse performance, profile/global-MNA wiring, old Maxwell owner-off transition, Via/termination integration, W7/PowerSI accuracy, production run, release, and version behavior. Future integration must disable old Maxwell plane C/loss and retain finite Via and termination exactly once.

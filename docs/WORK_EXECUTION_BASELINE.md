...
- 문서 버전: **1.125**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.124
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-CONSUMER-EXPRESSION-TRACE` (17CS) |
| 다음 권장 묶음 | `17CS` exact once-only line-861 consumer-expression read |
| current authorization | 17CR consumed at clean main `92a92a95b5d9f0d9da10c738c07b8195d47dfd87`; its exact file-scoped `_digest_hex` query ran once, exit1, zero output. No repo-global absence inference, body read, or retry. 17CS permits one read of line 861 in `src/spd_decap_pi/raw_spatial_contact_compiler.py`, requiring exit0, exactly one line/one file and a complete expression consuming the primitive row's `source_asset_sha256` value plus an actual validator/helper name or inline check; multiline, simple mapping/serialization, unknown validator, alias/dynamic cases stop with no retry. |
...
| 고비용 검증 권한 | none; 17CS allows only the bounded line-861 consumer-expression read; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-CONSUMER-EXPRESSION-TRACE` | 17CS | ACTIVE | read line 861 consumer expression | exact one-line read once; exit0, exactly 1 line/1 file; complete expression must consume the primitive row's `source_asset_sha256` value and actual validator/helper name or inline check; multiline/simple mapping/unknown validator/alias/dynamic cases STOP/no retry; one-line consumer-expression read only |
...
### 17CS. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-CONSUMER-EXPRESSION-TRACE (ACTIVE)

The exact one-line read is run once:

```powershell
$path = 'src/spd_decap_pi/raw_spatial_contact_compiler.py'
$lines = @(Get-Content -LiteralPath $path)
if ($lines.Count -lt 861) { exit 2 }
$output = @('{0}:861:{1}' -f $path, $lines[860])
if ($output.Count -ne 1) { exit 3 }
$output
```

Gate: exit0, exactly 1 line/1 file; line861 must be an independently complete expression consuming the primitive row's `source_asset_sha256` value plus an actual validator/helper name or inline check. A mid-multiline expression, simple mapping/serialization, unknown validator, alias, or dynamic case is BLOCKED/STOP with no retry or expansion. One-line consumer-expression read only; no additional source body read/expansion. Asset-name meaning, islands, ownership, stamps, profile, replacement, nullspace, Zii, production, accuracy, and release remain out of scope.

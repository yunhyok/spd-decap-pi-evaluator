...
- 문서 버전: **1.131**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.130
...
| 현재 active work item | `W7-PHYS-W6-PLANE-GEOMETRIES-CANONICAL-PRODUCER-DISCOVERY` (17CY) |
| 다음 권장 묶음 | `17CY` exact once-only `plane_geometries` mapping-producer location query |
| current authorization | 17CX consumed at clean canonical docs HEAD `15d860672cdb11d09d080207a46a37789ba94c45`; docs-only frozen judgment ran with zero source/search/artifact/test/import/build/code execution. Record/artwork-name transfer and net/layer casefold domains were unproven; attachment-name/exact-byte SHA domain passed; tuple cross-row match unproven, so BLOCKED/design REJECT with no retry/new reads. 17CY permits exactly one `plane_geometries` mapping-producer query: exit0, 1–12 unique lines, max3 files, exactly one actual producer/normalizer; otherwise BLOCKED/location STOP, no retry/file read. |
...
| 고비용 검증 권한 | none; 17CY allows only the bounded `plane_geometries` location query; additional source reads, artifact/test/import/build/code/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-GEOMETRIES-CANONICAL-PRODUCER-DISCOVERY` | 17CY | ACTIVE | locate a single mapping/normalizer producer for `plane_geometries` | use the sole canonical exact fenced command in Work §17CY; exit0, 1–12 unique lines/max3 files, exactly one actual `spd_import.plane_geometries` producer; consumer/get/annotation-only, zero/multiple/dynamic/ambiguous/cap STOP/no retry; location-only |
...
### 17CY. W7-PHYS-W6-PLANE-GEOMETRIES-CANONICAL-PRODUCER-DISCOVERY (ACTIVE)
...
Gate: exact once, exit0; 1–12 unique lines, maximum 3 files; exactly one actual `spd_import.plane_geometries` mapping producer/normalizer is required for DONE/location PASS. Consumer/get/annotation-only hits, zero/multiple/dynamic/ambiguous producers, or cap failure stop with BLOCKED/location STOP and no retry/file read. PASS would only permit a separately documented future <=1-file/120-line normalization trace; it does not authorize implementation now. Islands/electrical ownership/negative-shape/overlap/stamp/replacement/profile/nullspace/Zii/production/accuracy/release remain excluded.
## 20. 변경 기록

...
- 문서 버전: **1.115**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.114
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-CONTRACT-TRACE` (17CI) |
| 다음 권장 묶음 | `17CI` exact once-only numbered body read lines402-414 |
| current authorization | 17CI body gate: one numbered read exactly once; 13 lines/1 file; full `_decode_token` contract and same string domain required; delegated helper/out-of-range/truncation STOP/no expansion/retry. |
...
| 고비용 검증 권한 | none; 17CI allows only the bounded 13-line body read, with source expansion/code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-CONTRACT-TRACE` | 17CI | ACTIVE | trace decode-token contract | exact numbered read once lines402-414 (13 lines); full signature/return/error/whitespace/encoding/normalization and same string domain required; delegated helper/out-of-range/truncation STOP/no expansion/retry |
...
17CD is BLOCKED/source-classified STOP, 17CE is BLOCKED/location STOP, 17CF is DONE/location PASS, 17CG is BLOCKED/source-classified STOP, 17CH is DONE/location PASS, and 17CI is the sole ACTIVE decode-token contract trace.
...
sole current read-only successor 17CI owns the exact 13-line contract read. Frozen
...
### 17CI. W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-CONTRACT-TRACE (ACTIVE)
...
if ($lines.Count -lt 414) { exit 2 }

$output = for ($line = 402; $line -le 414; $line++) {
    '{0}:{1}:{2}' -f $path, $line, $lines[$line - 1]
}
if ($output.Count -ne 13) { exit 3 }
$output
```

Gate: one invocation, exact13 output lines/1 file for402-414. PASS requires full signature/return/error/whitespace/
encoding/normalization and returned token in the same string domain usable by later casefold mapping. Delegated helper,
out-of-range, or truncation STOP/no expansion/retry. Body-only; SHA/source hash/islands/ownership/stamp/profile/
replacement/nullspace/Zii/production/accuracy/release remain excluded.

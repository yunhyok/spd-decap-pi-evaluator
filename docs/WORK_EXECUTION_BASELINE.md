...
- 문서 버전: **1.142**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.141
...
- Sole ACTIVE item (current): 17DK `W7-PHYS-RAW-SPD-LANDING-CONTACT-STRICT-COVERAGE-AUDIT`; 17DH/17DI/17DJ remain historical BLOCKED closures.
- The historical chain's terminal `ACTIVE NONE. Any implementation remains gated.` wording is superseded: sole ACTIVE is 17DK under the exact whitelist.
...
| 현재 active work item | 17DK `W7-PHYS-RAW-SPD-LANDING-CONTACT-STRICT-COVERAGE-AUDIT` (ACTIVE) |
| 다음 권장 묶음 | 17DK exact whitelist, focused node once, Usage Guard, then one production audit/no retry |
| current authorization | exact 17DK whitelist and gates only; no unrelated implementation/test/build/production/release authority |
...
| `W7-PHYS-RAW-SPD-LANDING-CONTACT-STRICT-COVERAGE-AUDIT` | 17DK | ACTIVE | strict raw-SPD landing/contact coverage audit | exact two-file whitelist; focused node once; Usage Guard; one 260729 audit/no retry; SOURCE-COVERAGE STOP on failure |
...
### 17DK. W7-PHYS-RAW-SPD-LANDING-CONTACT-STRICT-COVERAGE-AUDIT (ACTIVE)
...
```mermaid
flowchart LR
  A[Raw SPD + candidate] --> B[Strict coverage audit]
  B -->|PASS| C[Next design eligible]
  B -->|STOP| D[SOURCE-COVERAGE STOP; no retry]
```

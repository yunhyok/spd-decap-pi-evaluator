...
- 문서 버전: **1.227**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.226; [Source-derived physical IR](SOURCE_DERIVED_PHYSICAL_IR.md) v1.61
- 현재 상태: W6 numerical FAIL; W7-SOURCE-IR-P1/P2/P3/P4 및 W7-PHYS-PROSPECTIVE-P0/P1/P2/P3/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP; Actual-P0/R1/R2, DIAG-01과 Recovery-01 DONE/STOP; FIX-01/FIX-02 DONE/ACCEPT; `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01` DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  ...
  `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-05` DONE/`STOP_MULTIPLE_NOT_REPRODUCED`; W7-PHYS production physics remains BLOCKED. Terminal pad-layer FIX-01 is DONE/`STOP_TEST_ORACLE_FALSE_NEGATIVE`; its test-oracle FIX-01 successor is DONE/ACCEPT. Original-SPD EVIDENCE-01/-02/-04/-05 are DONE/`STOP_IMPORT_OR_OWNERSHIP`, EVIDENCE-03 is DONE/`STOP_UNEXPECTED`; compact-request-bound FIX-01 is DONE/ACCEPT. OWNERSHIP-COUNT-DIAG-01/-02 are DONE/`STOP_RESOURCE_OR_CANCELLED`; STREAMED-IR-300K-FIX-01 is DONE/`STOP_TEST_COLLECTION_SYNTAX`; collection-fix successor is DONE/`STOP_TEST_FAILURE_AND_RUNTIME_BUDGET`; validation-runtime fix is DONE/`ACCEPT_WITH_OUTPUT_LIMITATION`; lightweight acceptance is DONE/`STOP_TEST_ORACLE_COMPOUND_MUTATION`; oracle-fix successor is DONE/`STOP_PRODUCT_SELECTION_VALIDATION_OMISSION`; material-selection fix is DONE/`STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION`; streamed-handoff fix is DONE/`STOP_TEST_CAPTURE_CONNECTION_LEAK`; capture-connection fix is DONE/`STOP_TEST_TAMPER_CONNECTION_LEAK`; tamper-connection fix is DONE/`STOP_TEST_PROVISIONAL_CAP_SCOPE_LEAK`; current sole ACTIVE is **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION**. Production physics/`Zii` are unchanged.
...
| 현재 active work item | **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION** |
| 현재 정확성 상태 | `260729 retrospective FAIL`; unseen/generalization `unknown / not_run` |
| current authorization | `main`-only, `accuracy_parse.py` 보존. Producer test의 `capture_compile`이 request를 포착한 직후 `MAX_SOURCE_PLANE_OWNERSHIP_IR_ROWS`를 `AUTHORITATIVE_IR_ROW_CAP`으로 복원하는 한 문장만 추가한다. 모든 product/source IR/other tests/schema/caps는 동결. Sol static 뒤 12.60의 provisional 노드 우선 exact 18 nodes once/300 s. Pressure/exact-300K/full suite/original SPD/solver/production/PowerSI 금지. |
| 다음 gate | **provisional-cap oracle fix** — synthetic 48을 request 단계에만 한정한 뒤 동일 bounded scope를 fresh/fail-fast로 한 번 |
...
| current candidate | DSU-01–05, Recovery-01/DIAG-01, original-SPD EVIDENCE-01/-02/-03/-04/-05와 OWNERSHIP-COUNT-DIAG-01/-02는 영구 DONE/STOP; owner-join, quotient-scope, terminal pad-layer test-oracle 및 compact-request-bound fixes are DONE/ACCEPT; streamed-IR-300K FIX-01은 collection STOP, collection-fix는 failure/runtime STOP, validation-runtime fix는 ACCEPT_WITH_OUTPUT_LIMITATION, lightweight acceptance는 oracle STOP, oracle-fix는 product selection omission STOP, material-selection fix는 streamed-handoff regression STOP, streamed-handoff fix는 test capture connection leak STOP, capture-connection fix는 test tamper connection leak STOP, tamper-connection fix는 provisional-cap scope leak STOP; P12 prospective review NO-GO; provisional-cap oracle fix sole ACTIVE |
...
| runtime evidence | DIAG-02 exact report/receipt가 persisted됐다. WinAPI measured peak 13.09 GiB, current 1.42 GiB, scratch 131.6 MiB; expanded Node/Via 77,852/38,926, six-key selection 116,791, source-record lower bound 122,146, final-v2 lower bound 249,635. Validation-runtime pressure+300K process는 exit 0 / 22.8483887 s. Lightweight run은 oracle STOP, oracle-fix run은 product Material-kind omission STOP이다. Material-fix run은 raw-v3 handoff STOP, streamed-handoff/capture runs는 test connection STOP이다. Tamper-connection run은 18 collected, first 13 explicit PASS 뒤 provisional node에서 synthetic 48이 actual final spool count까지 제한하여 `1 failed, 13 passed in 8.79s`, child 1, wrapper 9.435489 s로 STOP했다. |
...
| candidate staging / acceptance | producer test에서 request capture 뒤 authoritative cap 복원 한 문장만; 모든 product/source IR/other tests 동결; Sol static 뒤 provisional-first 18-node invocation 1회/300 s; accepted pressure/exact-300K와 full suite 재실행 금지 |
...
동시에 `ACTIVE`는 하나만 허용하며 현재 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01**이다. 완료된 미시적 item을
...
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-VALIDATION-RUNTIME-FIX-01` | 47 | DONE/ACCEPT_WITH_OUTPUT_LIMITATION | remove deterministic superlinear folded validation plans | source-only plan fix; Sol no P0/P1; exact two-node process exit 0 / 22.8483887 s; stdout unavailable; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-LIGHTWEIGHT-ACCEPTANCE-01` | 48 | DONE/STOP_TEST_ORACLE_COMPOUND_MUTATION | close only still-unproved changed streamed-transport paths | 18 collected; 7 explicit PASS then FK-invalid ledger fixture expected/actual mismatch; exit 1; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-LIGHTWEIGHT-ACCEPTANCE-ORACLE-FIX-01` | 49 | DONE/STOP_PRODUCT_SELECTION_VALIDATION_OMISSION | preserve FK while testing Unicode folded ledger collision | oracle fixed; 10 explicit PASS then missing Material-kind presence validation false STOP; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-MATERIAL-SELECTION-FIX-01` | 50 | DONE/STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION | include staged Material rows in generic selected-source presence validation | clause traversed to later callback; raw-v3 provenance missing and eager cursor cleanup masks primary failure; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-FIX-01` | 51 | DONE/STOP_TEST_CAPTURE_CONNECTION_LEAK | restore raw-v3 stackup/dielectric provenance and deterministic cursor ownership | product static ACCEPT; normal callback/build; test capture leaked connection at cleanup; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-CAPTURE-CONNECTION-FIX-01` | 52 | DONE/STOP_TEST_TAMPER_CONNECTION_LEAK | explicitly close two producer-test read-only SQLite captures | intended SpdImportError reached; separate tamper connection masked it with WinError 32; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01` | 53 | DONE/STOP_TEST_PROVISIONAL_CAP_SCOPE_LEAK | explicitly close the final producer-test tamper SQLite connection | first 13 PASS; historical synthetic 48 leaked into actual final-spool cap; no rerun |
| `W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01` | 54 | ACTIVE | restore authoritative cap after provisional request capture | one producer-test statement only; product frozen; Sol static; provisional-first exact 18 nodes once/300 s |
...
| `D-070` | VALIDATION-RUNTIME-FIX-01은 ownership IR SHA `1027E694...79DE`, Sol static P0/P1 none, exact ordered two-node process exit 0 / wrapper 22.8483887 s로 DONE/`ACCEPT_WITH_OUTPUT_LIMITATION`이다. Stdout 부재로 literal `2 passed`/per-node duration은 주장하지 않고 fresh exact node IDs/temp DB evidence까지만 인정하며 invocation은 consumed/no-rerun이다. Sole ACTIVE LIGHTWEIGHT-ACCEPTANCE-01은 product/tests byte-frozen, Sol `CONTRACT_RECOMMEND`의 18 exact nodes cheap-first 단일 Python312 invocation 1회/300 s만 허용한다. Static-only dataclass test, redundant section-pass, accepted pressure/exact-300K, unchanged/full suite/original SPD/solver/production/PowerSI는 제외한다 | 확정 |
| `D-071` | LIGHTWEIGHT-ACCEPTANCE-01 sole run은 18 collected, first 7 explicit PASS 뒤 eighth node의 mutation index 10에서 expected `ID_COLLISION`/actual `DATABASE_INVALID`, `1 failed, 7 passed in 6.39s`, child exit 1, wrapper 7.0349161 s로 DONE/`STOP_TEST_ORACLE_COMPOUND_MUTATION`이다. Parent ledger ID만 바꿔 child member가 orphan된 compound fixture가 FK-enabled main copy에서 먼저 정상 거부된 것이며 product bug가 아니다. Partial progress는 gate acceptance로 재사용하지 않는다. Sole ACTIVE ORACLE-FIX-01은 해당 child ledger ID update 한 statement만 추가하고 Sol static 뒤 same exact 18 nodes를 한 번/300 s 실행한다. Product/other tests/pressure/exact-300K/full suite/original SPD/solver/PowerSI는 불변·금지다 | 확정 |
| `D-072` | ORACLE-FIX-01 sole run은 18 collected, first 10 explicit PASS 뒤 producer component-layer node에서 `RAW_SPATIAL_OWNERSHIP_EVIDENCE_INCOMPLETE`, `1 failed, 10 passed in 7.87s`, child exit 1, wrapper 8.516575 s로 DONE/`STOP_PRODUCT_SELECTION_VALIDATION_OMISSION`이다. Corrected Unicode oracle는 PASS했다. Selected `Material/abf/''`와 `Material/copper/''` source rows는 staged되지만 generic presence SQL kind disjunction에 Material이 없어 둘 다 오탐이다; unordered `LIMIT 1`의 반환 행은 주장하지 않는다. Partial progress는 acceptance에 재사용하지 않는다. Sole ACTIVE MATERIAL-SELECTION-FIX-01은 raw compiler SQL에 Material kind clause 하나만 추가하고 Sol static 뒤 culprit-first exact 18 nodes를 한 번/300 s 실행한다. 다른 product/tests, pressure/exact-300K/full suite/original SPD/solver/PowerSI는 불변·금지다 | 확정 |
| `D-073` | MATERIAL-SELECTION-FIX-01 sole run은 18 collected 뒤 first culprit node에서 `sqlite3.IntegrityError: NOT NULL constraint failed: stackup_layers.raw_layer_ordinal`, `1 failed in 1.87s`, child exit 1, wrapper 2.6307 s로 DONE/`STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION`이다. Material clause가 later callback까지 열었지만 streamed rewrite가 predecessor의 stackup/dielectric raw-v3 merge를 누락하여 provenance columns를 `None`으로 전달했고, eager cursor generator가 traceback에 남아 TemporaryDirectory cleanup의 WinError 32가 primary를 가렸다. Reusable PASS는 0이고 run은 consumed/no-rerun이다. Sole ACTIVE STREAMED-HANDOFF-FIX-01은 raw compiler에 exact proven merge를 현재 spool writer로 복원하고 adapter가 spool-backed stackup/dielectric을 소비하게 하며 iterator/cursor를 rollback/connection close 전에 닫는다. 두 product files 외 source IR/tests/schema/caps/fallback은 동결하고 Sol static 뒤 culprit-first exact 18 nodes를 한 번/300 s 실행한다. Pressure/exact-300K/full suite/original SPD/solver/production/PowerSI는 불변·금지, improvement는 0이다 | 확정 |
| `D-074` | STREAMED-HANDOFF-FIX-01은 raw compiler `5B02CC51...45BA2`, adapter `F8E81D7B...13D14`, Sol final STATIC_ACCEPT/P0-P1 none이었으나 sole run은 18 collected 뒤 first node의 normal callback/build 반환 후 TemporaryDirectory가 `raw-spatial-spool.sqlite`를 삭제할 때 WinError 32로 실패했다. Pytest `1 failed in 2.27s`, child exit 1, wrapper 2.8645396 s, reusable PASS 0이고 invocation은 consumed/no-rerun이다. `exc/value/tb=None`과 static trace는 product가 아니라 producer test의 two capture wrappers가 transaction-only `with sqlite3.connect(...)`를 사용해 close를 GC에 맡긴 결함을 확정한다. Policy-rejected Start-Process wrapper는 process 생성 전 거부되어 run으로 세지 않는다. Sole ACTIVE CAPTURE-CONNECTION-FIX-01은 producer test에 stdlib `closing` import와 정확히 두 `with closing(sqlite3.connect(...))`만 허용한다. Product/source IR/other tests/schema/caps 동결, Sol static 뒤 same culprit-first exact 18 nodes once/300 s. Pressure/exact-300K/full suite/original SPD/solver/production/PowerSI 금지, improvement 0 | 확정 |
| `D-075` | CAPTURE-CONNECTION-FIX-01은 producer test SHA `44E82828...E83A`, Sol STATIC_ACCEPT/P0-P1 none이었으나 sole run은 18 collected 뒤 first node의 intended `SOURCE_PLANE_OWNERSHIP_IR_TERMINAL_INCOMPLETE`에 도달한 다음 test-only `tamper_node` SQLite handle 때문에 TemporaryDirectory cleanup이 WinError 32로 실패했다. Pytest `1 failed in 2.44s`, child exit 1, wrapper 3.1867265 s, reusable PASS 0이고 output은 249,289 B/SHA `F02D5541...AFD0`, invocation은 consumed/no-rerun이다. Product connections는 모두 먼저 닫혔고 producer file의 세 connect 중 capture 두 곳은 fixed, tamper 한 곳만 남았다. Sole ACTIVE TAMPER-CONNECTION-FIX-01은 기존 `closing` import를 재사용해 그 한 문장만 감싼다. Product/source IR/other tests/schema/caps 동결, Sol static 뒤 same culprit-first exact 18 nodes once/300 s. Pressure/exact-300K/full suite/original SPD/solver/production/PowerSI 금지, improvement 0 | 확정 |
| `D-076` | TAMPER-CONNECTION-FIX-01은 producer test SHA `49446835...7193`, Sol STATIC_ACCEPT/P0-P1 none이었고 sole run은 18 collected, first 13 explicit PASS 뒤 provisional-request node에서 adapter의 actual final spool cumulative count가 synthetic cap 48을 초과해 `SOURCE_PLANE_OWNERSHIP_IR_BOUND_EXCEEDED`로 STOP했다. Pytest `1 failed, 13 passed in 8.79s`, child exit 1, wrapper 9.435489 s, output 244,178 B/SHA `3031D61F...1B0B`; invocation은 consumed/no-rerun이고 13 PASS는 acceptance로 재사용하지 않는다. Streamed adapter의 actual final cap 검사는 정상이며 historical af84334 test가 request-only synthetic cap을 final 단계까지 누출한 oracle 결함이다. Sole ACTIVE PROVISIONAL-CAP-ORACLE-FIX-01은 request capture 직후 authoritative cap 복원 한 문장만 허용한다. Product/source IR/other tests/schema/caps 동결, Sol static 뒤 provisional-first exact 18 nodes once/300 s. Pressure/exact-300K/full suite/original SPD/solver/production/PowerSI 금지, improvement 0 | 확정 |
...
- 따라서 현재 product risk는 그대로 **외부 정확성 미달**이다. **W7-PHYS-OWNER-JOIN-QUOTIENT-SCOPE-FIX-01**은 DONE/ACCEPT다. W7-PHYS-OWNER-JOIN-EVIDENCE-01은 DONE/ACCEPT로 닫혔고 original-SPD EVIDENCE-01/-02/-04/-05는 DONE/`STOP_IMPORT_OR_OWNERSHIP`, EVIDENCE-03은 missing `httpx`의 pre-import `STOP_UNEXPECTED`로 영구 닫혔다. Terminal pad-layer test-oracle successor와 compact-request-bound FIX-01은 DONE/ACCEPT다. OWNERSHIP-COUNT-DIAG-01/-02, streamed-IR FIX-01과 collection-fix는 DONE/STOP, validation-runtime fix는 DONE/`ACCEPT_WITH_OUTPUT_LIMITATION`, lightweight acceptance는 DONE/`STOP_TEST_ORACLE_COMPOUND_MUTATION`, oracle-fix는 DONE/`STOP_PRODUCT_SELECTION_VALIDATION_OMISSION`, material-selection fix는 DONE/`STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION`, streamed-handoff fix는 DONE/`STOP_TEST_CAPTURE_CONNECTION_LEAK`, capture-connection fix는 DONE/`STOP_TEST_TAMPER_CONNECTION_LEAK`, tamper-connection fix는 DONE/`STOP_TEST_PROVISIONAL_CAP_SCOPE_LEAK`이고 현재 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION**이다.
...
flowchart LR
  ...
  TC --> PC[W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01<br/>FROZEN / READY_FOR_IMPLEMENTATION]
  R -->|예| H
...
닫혔고 DIAG-01/-02도 resource STOP으로 닫혔다. old component evidence와 DSU-01–05는 모두 DONE/STOP했고 W7-PHYS-OWNER-JOIN-EVIDENCE-01은 DONE/ACCEPT로 닫혔다. **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-01/-02/-03/-04/-05**와 ownership-count DIAG-01/-02는 영구 DONE/STOP이고, **W7-PHYS-OWNER-JOIN-QUOTIENT-SCOPE-FIX-01**, terminal pad-layer test-oracle와 compact-request-bound fixes는 DONE/ACCEPT다. streamed-IR FIX-01과 collection-fix는 DONE/STOP, validation-runtime fix는 DONE/`ACCEPT_WITH_OUTPUT_LIMITATION`, lightweight acceptance는 DONE/`STOP_TEST_ORACLE_COMPOUND_MUTATION`, oracle-fix는 DONE/`STOP_PRODUCT_SELECTION_VALIDATION_OMISSION`, material-selection fix는 DONE/`STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION`, streamed-handoff fix는 DONE/`STOP_TEST_CAPTURE_CONNECTION_LEAK`, capture-connection fix는 DONE/`STOP_TEST_TAMPER_CONNECTION_LEAK`, tamper-connection fix는 DONE/`STOP_TEST_PROVISIONAL_CAP_SCOPE_LEAK`이고 현재 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION**이다. 아래 조건은 그 다음 physical promotion gate를 열 때도 계속 적용한다.
...
### 12.53 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-VALIDATION-RUNTIME-FIX-01 — DONE / ACCEPT_WITH_OUTPUT_LIMITATION
...
Luna's final ownership IR SHA-256 is `1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE`.
Sol static review found P0/P1 none and no out-of-contract delta. The exact ordered two-node Python312 process exited 0
within wrapper wall 22.8483887 s. Fresh `.pytest_cache` node IDs and ordered temporary databases prove both requested
cases executed; stdout was unavailable, so literal `2 passed`, per-node pytest text and durations are not claimed. The
invocation is consumed/no-rerun.

### 12.54 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-LIGHTWEIGHT-ACCEPTANCE-01 — DONE / STOP_TEST_ORACLE_COMPOUND_MUTATION

Freeze the six candidate files at raw compiler
`0612A85B347BD2C034180E949AA1BF9A113CFB5E61B22A7226D689744BF7D9E5`, ownership IR
`1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE`, adapter
`25CE48DDA16027C580F0D6755B9089E262C983920958C53EAF3C2233B52D0D4D`, and tests
...
`D08714921FDBBB9577DCE4CD4568EB359331B56FCB3FC1CFEFD3DA6DBC6AA3C5`. No file edit is allowed.

After Sol `CONTRACT_RECOMMEND`, run exactly one Python312 process with `-x -vv --tb=long`, in this order:

1. `tests/test_raw_spatial_contact_compiler.py::test_ownership_selection_duplicate_is_sql_unique_failure`
2. `tests/test_raw_spatial_contact_compiler.py::test_ownership_selection_cap_exact_pass_plus_one_fails`
3. `tests/test_raw_spatial_contact_compiler.py::test_ownership_source_staging_keeps_original_spelling_and_canonical_lookup`
4. `tests/test_source_plane_ownership_ir.py::test_v2_mapping_and_spool_are_logically_equivalent_and_exclude_internal_tables`
5. `tests/test_source_plane_ownership_ir.py::test_spool_caps_and_cancellation_fail_closed`
6. `tests/test_source_plane_ownership_ir.py::test_spool_sql_validation_rejects_provenance_mutations`
7. `tests/test_source_plane_ownership_ir.py::test_spool_sql_contact_identity_and_edge_owner_rejections`
8. `tests/test_source_plane_ownership_ir.py::test_spool_sql_rejects_origin_scalar_casefold_and_ordinal_holes`
9. `tests/test_source_plane_ownership_ir.py::test_spool_sql_rejects_contact_opposite_external_node_chain`
10. `tests/test_source_plane_ownership_ir.py::test_spool_temp_artifacts_are_removed_after_cancel_and_exception`
11. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_component_layer_is_authoritative_for_mismatched_endpoint`
12. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_filters_global_quotient_before_selected_row_bound[unrelated_global]`
13. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_filters_global_quotient_before_selected_row_bound[projected_over_cap]`
14. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_provisional_request_aggregate_does_not_consume_final_ir_row_cap`
15. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_producer_roundtrip_and_atomic_failure`
16. `tests/test_raw_spatial_contact_compiler.py::test_ownership_expanded_selection_116791_and_global_cap`
17. `tests/test_source_plane_ownership_ir.py::test_v2_exact_section_and_total_caps[150001-2-True-None]`
18. `tests/test_source_plane_ownership_ir.py::test_v2_exact_section_and_total_caps[150000-149978-True-None]`

Enforce a 300 s external wall limit and capture stdout/stderr. Any collection error, failure, timeout or interrupt closes
this item as DONE/STOP without rerun; partial progress is not PASS evidence. Omit the static-only dataclass-field test,
the redundant `[150000-2-False-150025]`, the accepted pressure/exact-300K nodes and all unchanged tests/full suite.

Passing proves only these named synthetic/MINI-SPD paths: streamed SQLite transport, v2 logical parity, fail-closed
bounds/invariants/cleanup and adapter producer branches. It does not prove the original-SPD 13.09 GiB path, production
owner join, solver/`Y_global`/`Zii`, PowerSI accuracy/performance or release.

The sole run collected all 18 nodes and produced seven explicit PASS results before node 8 failed. Its 0-based mutation
index 10 expected `SOURCE_PLANE_OWNERSHIP_IR_ID_COLLISION` but received
`SOURCE_PLANE_OWNERSHIP_IR_DATABASE_INVALID`; final summary `1 failed, 7 passed in 6.39s`, child exit 1, wrapper wall
7.0349161 s. The fixture changed `replacement_ledger.ledger_id` but left three child member IDs at `ledger:0`; the
FK-enabled destination therefore rejected the orphan during section copy before `_validate_spooled_rows` could classify
the Unicode folded collision. Sol and Luna classify this as a test oracle/compound-mutation defect, P0 none, not a
product defect. The invocation is consumed/no-rerun and its partial progress does not accept this gate.

### 12.55 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-LIGHTWEIGHT-ACCEPTANCE-ORACLE-FIX-01 — DONE / STOP_PRODUCT_SELECTION_VALIDATION_OMISSION

Base docs contract is exact `2988dd5`. Freeze every product file and all tests except
`tests/test_source_plane_ownership_ir.py`, whose pre-fix SHA-256 is
`B3324E162E77A8D3C4BE6D0F8FC80A020CE0FD83FDF1EC78B1442EF806D89F32`. Luna may add exactly one statement in mutation
index 10 to update `replacement_ledger_members.ledger_id` from `ledger:0` to `i̇` alongside its parent. Preserve the
expected `SOURCE_PLANE_OWNERSHIP_IR_ID_COLLISION`; no product or other test change is allowed.

After Sol static acceptance, run the exact 18 nodes and order already enumerated in section 12.54 once with `-x -vv
--tb=long`, captured stdout/stderr and an external 300 s limit. Any failure/timeout closes the successor with no rerun.
Pressure, exact-300K, redundant/unchanged tests and full suite remain excluded. Passing proves only the same named
synthetic/MINI-SPD streamed transport and producer paths; original SPD, production owner join, solver/`Y_global`/`Zii`,
PowerSI accuracy/performance and release remain unproved.

The corrected Unicode collision node passed. The sole run collected 18 nodes and produced ten explicit PASS results
before the producer component-layer node failed with `RAW_SPATIAL_OWNERSHIP_EVIDENCE_INCOMPLETE: selected ownership
source record is absent`; final summary `1 failed, 10 passed in 7.87s`, child exit 1, wrapper wall 8.516575 s. Sol and
Luna traced the product flow from importer selection through `stage_material_draft`: the selected
`Material/abf/''` and `Material/copper/''` rows are present in `ownership_source_stage`, but the generic final
selected-source presence query's kind disjunction contains Layer/Node/Via/Pad/Surface only. Both Material selections are
therefore false negatives before the later exact-one Material check. Its unordered `LIMIT 1` does not identify which
row was returned. The invocation is consumed/no-rerun; its first ten PASS results are diagnostic only.

### 12.56 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-MATERIAL-SELECTION-FIX-01 — DONE / STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION

Base docs contract is exact `dfeca13`. Freeze all candidate bytes except the final selected-source presence query in
`src/spd_decap_pi/raw_spatial_contact_compiler.py`, whose pre-fix SHA-256 is
`0612A85B347BD2C034180E949AA1BF9A113CFB5E61B22A7226D689744BF7D9E5`. Luna may add exactly
`OR (s.kind='Material' AND r.kind='Material')` to its existing kind disjunction. The outer `lookup_a/key_a` and
`lookup_b/key_b` predicates already enforce exact identity. No second query, helper, fallback, cap/schema change or
other product/test edit is allowed.

After Sol static acceptance, run exactly one fresh Python312 process with `-x -vv --tb=long`, in this fail-fast order:

1. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_component_layer_is_authoritative_for_mismatched_endpoint`
2. `tests/test_raw_spatial_contact_compiler.py::test_ownership_selection_duplicate_is_sql_unique_failure`
3. `tests/test_raw_spatial_contact_compiler.py::test_ownership_selection_cap_exact_pass_plus_one_fails`
4. `tests/test_raw_spatial_contact_compiler.py::test_ownership_source_staging_keeps_original_spelling_and_canonical_lookup`
5. `tests/test_source_plane_ownership_ir.py::test_v2_mapping_and_spool_are_logically_equivalent_and_exclude_internal_tables`
6. `tests/test_source_plane_ownership_ir.py::test_spool_caps_and_cancellation_fail_closed`
7. `tests/test_source_plane_ownership_ir.py::test_spool_sql_validation_rejects_provenance_mutations`
8. `tests/test_source_plane_ownership_ir.py::test_spool_sql_contact_identity_and_edge_owner_rejections`
9. `tests/test_source_plane_ownership_ir.py::test_spool_sql_rejects_origin_scalar_casefold_and_ordinal_holes`
10. `tests/test_source_plane_ownership_ir.py::test_spool_sql_rejects_contact_opposite_external_node_chain`
11. `tests/test_source_plane_ownership_ir.py::test_spool_temp_artifacts_are_removed_after_cancel_and_exception`
12. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_filters_global_quotient_before_selected_row_bound[unrelated_global]`
13. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_filters_global_quotient_before_selected_row_bound[projected_over_cap]`
14. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_provisional_request_aggregate_does_not_consume_final_ir_row_cap`
15. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_producer_roundtrip_and_atomic_failure`
...
Use captured stdout/stderr and a 300 s external wall limit. Any collection error, failure, timeout or interrupt closes
this item as DONE/STOP without rerun; partial progress is not acceptance evidence. The already consumed pressure and
exact-300K cases, redundant `[150000-2-False-150025]`, static-only dataclass check, unchanged tests and full suite stay
excluded. Passing proves only the named synthetic/MINI-SPD streamed SQLite transport, v2 logical parity, fail-closed
bounds/invariants/cleanup and adapter producer branches. It does not prove the original-SPD 13.09 GiB path, production
owner join, solver/`Y_global`/`Zii`, PowerSI accuracy/performance or release; numerical improvement remains 0.

The one-clause fix produced raw compiler SHA-256
`3C3E256C4717C3349F78D537EE92FB52E2AEF3500851408A741C106B9B15BD61`; removing only that clause reproduces the frozen
pre-fix hash. Sol found no P0/P1 static issue in the clause. The sole run collected all 18 nodes, then the first culprit
node failed in the later adapter callback with primary `sqlite3.IntegrityError: NOT NULL constraint failed:
stackup_layers.raw_layer_ordinal`; pytest summary `1 failed in 1.87s`, child exit 1, wrapper wall 2.6307 s. During unwind,
an eager cursor-backed generator retained the SQLite file and TemporaryDirectory cleanup raised WinError 32, masking
the primary exception in the final traceback. No node passed and no partial result is reusable. The run is
consumed/no-rerun. This establishes only that the Material clause reached the later callback; it does not accept the
selection fix or any physical/accuracy claim.

### 12.57 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-FIX-01 — DONE / STOP_TEST_CAPTURE_CONNECTION_LEAK

Base docs contract is exact `d4d3fab`. The current pre-fix raw compiler SHA-256 is
`3C3E256C4717C3349F78D537EE92FB52E2AEF3500851408A741C106B9B15BD61` and adapter SHA-256 is
`25CE48DDA16027C580F0D6755B9089E262C983920958C53EAF3C2233B52D0D4D`. Freeze
`source_plane_ownership_ir.py` at `1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE` and all three
test files at `D51D4E45E0182DC99F60E12D7DB48528F1F38567EBD2A4E245DE416C4EF102BE`,
`56AAECB2A83F9E96E81D2F3F77D7EDC67F896E7AAD63BE38999AFAC06FDA8756`, and
`D08714921FDBBB9577DCE4CD4568EB359331B56FCB3FC1CFEFD3DA6DBC6AA3C5`.

Luna may edit only `raw_spatial_contact_compiler.py` and `spd_adapter.py`:

1. In the raw compiler, port the exact proven predecessor stackup/dielectric raw-v3 merge into the current spool writer
   while `plane_sheet_payload` and the analysis draft coexist. Join stackup by folded layer and dielectric by folded
   layer plus point ordinal. Raw physical fields override draft placeholders while lineage fields remain; final
   `ordinal` is zero-based contiguous, raw ordinals remain source-global, and canonical row SHA-256 is preserved. Insert
   the merged rows directly into the existing v2 spool tables.
2. In the adapter, consume stackup/dielectric from the spool-backed sections and mark them streamed. The request draft is
   provenance input only and must not delete or overwrite the merged rows.
3. Replace the two eager cursor expressions for retained/member rows with lazy generators that open their `execute`
   cursor inside iteration and close it in `finally`. The generic insert loop owns each iterator and closes it in
   `finally` before rollback, connection close, or rethrow.

Do not reload the raw asset, add a schema/cap/fallback, suppress cleanup errors, add a helper module, modify tests or edit
any other product file. The existing culprit and producer roundtrip/atomic-failure nodes are the executable regressions;
Sol's cursor lifetime review is the static cleanup check.
...
Capture stdout/stderr and enforce a 300 s external wall limit. Any collection error, failure, timeout or interrupt closes
this successor as DONE/STOP without rerun; partial progress is not acceptance evidence. The consumed pressure and
exact-300K cases, redundant `[150000-2-False-150025]`, static-only dataclass check, unchanged tests, full suite, original
SPD, solver, production and PowerSI runs remain excluded. Passing proves only the named synthetic/MINI-SPD streamed
v2/v3 transport and producer paths. Production owner join, `Y_global`/`Zii`, PowerSI accuracy/performance and release
remain unproved; numerical improvement remains 0.

The implementation finalized at raw compiler SHA-256
`5B02CC51677FEB6BCBE537071C7A13342E2036C0F2D7910103118865C3A45BA2` and adapter SHA-256
`F8E81D7B702E155C51766A8CC0CAC2B3FE3E189450BEB43DAC0811FA9A213D14`. Sol's second static review found no P0/P1;
source-record FK order, raw/global ordinals and hashes, strict provenance refs, spool-backed sections and iterator-close
ordering all matched the contract.

The sole fresh run collected all 18 nodes and stopped at the first. Its product callback/build returned normally
(`TemporaryDirectory.__exit__` received `exc/value/tb=None`), then Windows raised WinError 32 deleting
`raw-spatial-spool.sqlite`; pytest `1 failed in 2.27s`, child exit 1, wrapper wall 2.8645396 s. No node passed and no
partial result is reusable. The invocation is consumed/no-rerun. A prior policy-rejected `Start-Process` wrapper attempt
created no process and is not an execution.

Static tracing found no product survivor. The producer test's `capture_draft` and later `capture_ownership_draft` both
use `sqlite3.Connection` as a context manager. That context manager ends a transaction but does not close the
connection, so the read-only capture handle remains GC-dependent during immediate outer temporary cleanup. The first
wrapper caused this run; the second would expose the same defect at the roundtrip node.

### 12.58 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-CAPTURE-CONNECTION-FIX-01 — DONE / STOP_TEST_TAMPER_CONNECTION_LEAK

Base docs contract is exact `f772023`. Freeze raw compiler
`5B02CC51677FEB6BCBE537071C7A13342E2036C0F2D7910103118865C3A45BA2`, adapter
`F8E81D7B702E155C51766A8CC0CAC2B3FE3E189450BEB43DAC0811FA9A213D14`, source IR
`1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE`, raw compiler test
`D51D4E45E0182DC99F60E12D7DB48528F1F38567EBD2A4E245DE416C4EF102BE` and source IR test
`56AAECB2A83F9E96E81D2F3F77D7EDC67F896E7AAD63BE38999AFAC06FDA8756`. The only editable test is
`tests/test_source_plane_ownership_ir_producer.py`, pre-fix SHA-256
`D08714921FDBBB9577DCE4CD4568EB359331B56FCB3FC1CFEFD3DA6DBC6AA3C5`.

Luna may add `from contextlib import closing` and replace exactly the two read-only capture statements with
`with closing(sqlite3.connect(spool.path)) as connection:`. Do not add GC, sleep, retry, product cleanup or any other
test/product/schema/cap change. The existing culprit, temp-artifact and producer roundtrip/atomic-failure nodes cover
success, cleanup and failure paths; no new node is needed.
...
Capture stdout/stderr and enforce a 300 s external wall limit. Any collection error, failure, timeout or interrupt closes
this successor as DONE/STOP without rerun; partial progress is not acceptance evidence. The consumed prior 18-node run,
pressure and exact-300K cases, redundant `[150000-2-False-150025]`, static-only dataclass check, unchanged tests, full
suite, original SPD, solver, production and PowerSI runs remain excluded. Passing proves only the named
synthetic/MINI-SPD streamed v2/v3 transport and producer paths. Production owner join, `Y_global`/`Zii`, PowerSI
accuracy/performance and release remain unproved; numerical improvement remains 0.

The implementation changed only the producer test to SHA-256
`44E82828827CF5D4C43D489FC51340884F791FB7ED4E806081115B11F1CBE83A`. Removing the import and reverting the two
capture statements reconstructs exact pre-fix SHA-256
`D08714921FDBBB9577DCE4CD4568EB359331B56FCB3FC1CFEFD3DA6DBC6AA3C5`. Sol's static review found no P0/P1 and all
five frozen sibling hashes matched.

The sole fresh run used 18 unique nodes, collected all 18 and stopped at the first. The two capture handles were closed
and execution reached the intended `SOURCE_PLANE_OWNERSHIP_IR_TERMINAL_INCOMPLETE: terminal Node layer differs from
retained source layer`. The same test's separate `tamper_node` connection remained open, so raw TemporaryDirectory
cleanup masked the expected exception with WinError 32. Pytest reported `1 failed in 2.44s`; child exit 1, wrapper wall
3.1867265 s, reusable PASS 0. Captured stdout/stderr was 249,289 bytes with SHA-256
`F02D5541266A4F3DFC5A987FB0812C93F8104F533030C2C7B8C72A160AD8AFD0`. The invocation is consumed/no-rerun.

Static tracing confirmed the raw compiler and adapter connections close before cleanup. The producer file contains
exactly three `sqlite3.connect` sites: the two capture sites now use `closing`, while the `tamper_node` statement is the
only remaining leak.

### 12.59 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 — DONE / STOP_TEST_PROVISIONAL_CAP_SCOPE_LEAK

Base docs contract is exact `fa29fe8`. Freeze raw compiler
`5B02CC51677FEB6BCBE537071C7A13342E2036C0F2D7910103118865C3A45BA2`, adapter
`F8E81D7B702E155C51766A8CC0CAC2B3FE3E189450BEB43DAC0811FA9A213D14`, source IR
`1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE`, raw compiler test
`D51D4E45E0182DC99F60E12D7DB48528F1F38567EBD2A4E245DE416C4EF102BE`, source IR test
`56AAECB2A83F9E96E81D2F3F77D7EDC67F896E7AAD63BE38999AFAC06FDA8756` and producer test
`44E82828827CF5D4C43D489FC51340884F791FB7ED4E806081115B11F1CBE83A`.

Luna may replace exactly the remaining producer-test statement with
`with closing(sqlite3.connect(evidence.spool.path)) as connection:`. Reuse the existing import. Do not change the
explicit commit, mutation, exception, any other test/product byte, schema, cap or cleanup policy. The existing culprit,
temp-artifact and producer roundtrip/atomic-failure nodes cover the relevant paths; no new node is needed.
...
Capture stdout/stderr and enforce a 300 s external wall limit. Any collection error, failure, timeout or interrupt closes
this successor as DONE/STOP without rerun; partial progress is not acceptance evidence. The two consumed prior 18-node
runs, pressure and exact-300K cases, redundant `[150000-2-False-150025]`, static-only dataclass check, unchanged tests,
full suite, original SPD, solver, production and PowerSI runs remain excluded. Passing proves only the named
synthetic/MINI-SPD streamed v2/v3 transport and producer paths. Production owner join, `Y_global`/`Zii`, PowerSI
accuracy/performance and release remain unproved; numerical improvement remains 0.

The implementation changed only the tamper connection statement and finalized the producer test at SHA-256
`494468358C939CA25A0FA013616DD93702CDBE4051C47DB3CD9004C757CE7193`; exact reversal reconstructs
`44E82828827CF5D4C43D489FC51340884F791FB7ED4E806081115B11F1CBE83A`. Sol static review found no P0/P1 and all
five frozen sibling hashes matched.

The sole fresh run collected 18 nodes and explicitly passed the first 13. The provisional-request node then failed the
adapter's actual final spool cumulative check with `SOURCE_PLANE_OWNERSHIP_IR_BOUND_EXCEEDED: total rows exceed bound`.
Pytest reported `1 failed, 13 passed in 8.79s`; child exit 1, wrapper wall 9.435489 s. Captured stdout/stderr was 244,178
bytes with SHA-256 `3031D61F5EA12AB5328CBDB02AD73627AF3B0F73F8C2C685A7AA89E624621B0B`. The invocation is consumed/no-rerun and
the 13 partial PASS results are not acceptance evidence.

The streamed adapter correctly applies its cap to actual final spool rows. Historical test commit `af84334` patched
the adapter symbol to synthetic 48 for request construction, while its legacy builder used a separate authoritative
source-module constant. Final counting now correctly occurs in the adapter, so the test's synthetic value leaked past
its intended provisional-request scope.

### 12.60 W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 — FROZEN / READY_FOR_IMPLEMENTATION

Base docs contract is exact `6fb577b`. Freeze raw compiler
`5B02CC51677FEB6BCBE537071C7A13342E2036C0F2D7910103118865C3A45BA2`, adapter
`F8E81D7B702E155C51766A8CC0CAC2B3FE3E189450BEB43DAC0811FA9A213D14`, source IR
`1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE`, raw compiler test
`D51D4E45E0182DC99F60E12D7DB48528F1F38567EBD2A4E245DE416C4EF102BE`, source IR test
`56AAECB2A83F9E96E81D2F3F77D7EDC67F896E7AAD63BE38999AFAC06FDA8756` and producer test
`494468358C939CA25A0FA013616DD93702CDBE4051C47DB3CD9004C757CE7193`.

Luna may add exactly one statement in the provisional test's `capture_compile`, after `captured["request"]` is assigned
and before `original_compile` is called:
`monkeypatch.setattr(spd_adapter, "MAX_SOURCE_PLANE_OWNERSHIP_IR_ROWS", AUTHORITATIVE_IR_ROW_CAP)`. Request formation
and any regressed provisional aggregate guard still see synthetic 48; raw compilation and actual final-spool counting
see the authoritative cap. Do not change the synthetic value, group assertions, product, any other test, schema or cap.

After Sol static acceptance, run exactly one fresh Python312 process with `-x -vv --tb=long`, in this fail-fast order:

1. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_provisional_request_aggregate_does_not_consume_final_ir_row_cap`
2. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_component_layer_is_authoritative_for_mismatched_endpoint`
3. `tests/test_raw_spatial_contact_compiler.py::test_ownership_selection_duplicate_is_sql_unique_failure`
4. `tests/test_raw_spatial_contact_compiler.py::test_ownership_selection_cap_exact_pass_plus_one_fails`
5. `tests/test_raw_spatial_contact_compiler.py::test_ownership_source_staging_keeps_original_spelling_and_canonical_lookup`
6. `tests/test_source_plane_ownership_ir.py::test_v2_mapping_and_spool_are_logically_equivalent_and_exclude_internal_tables`
7. `tests/test_source_plane_ownership_ir.py::test_spool_caps_and_cancellation_fail_closed`
8. `tests/test_source_plane_ownership_ir.py::test_spool_sql_validation_rejects_provenance_mutations`
9. `tests/test_source_plane_ownership_ir.py::test_spool_sql_contact_identity_and_edge_owner_rejections`
10. `tests/test_source_plane_ownership_ir.py::test_spool_sql_rejects_origin_scalar_casefold_and_ordinal_holes`
11. `tests/test_source_plane_ownership_ir.py::test_spool_sql_rejects_contact_opposite_external_node_chain`
12. `tests/test_source_plane_ownership_ir.py::test_spool_temp_artifacts_are_removed_after_cancel_and_exception`
13. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_filters_global_quotient_before_selected_row_bound[unrelated_global]`
14. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_filters_global_quotient_before_selected_row_bound[projected_over_cap]`
15. `tests/test_source_plane_ownership_ir_producer.py::test_source_plane_ownership_producer_roundtrip_and_atomic_failure`
16. `tests/test_raw_spatial_contact_compiler.py::test_ownership_expanded_selection_116791_and_global_cap`
17. `tests/test_source_plane_ownership_ir.py::test_v2_exact_section_and_total_caps[150001-2-True-None]`
18. `tests/test_source_plane_ownership_ir.py::test_v2_exact_section_and_total_caps[150000-149978-True-None]`
...
Capture stdout/stderr and enforce a 300 s external wall limit. Any collection error, failure, timeout or interrupt closes
this successor as DONE/STOP without rerun; partial progress is not acceptance evidence. The three consumed prior
18-node runs, pressure and exact-300K cases, redundant `[150000-2-False-150025]`, static-only dataclass check, unchanged
tests, full suite, original SPD, solver, production and PowerSI runs remain excluded. Passing proves only the named
synthetic/MINI-SPD streamed v2/v3 transport and producer paths. Production owner join, `Y_global`/`Zii`, PowerSI
accuracy/performance and release remain unproved; numerical improvement remains 0.

...
- 문서 버전: **1.64**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.63
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-IMPORT-API-SIGNATURE-TRACE` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-IMPORT-API-SIGNATURE-TRACE` (ACTIVE; one bounded source excerpt) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AL is DONE (source-classified test gap) at clean main HEAD `7618408f8979d07f6810ed1052cb472b8837b222`: exact tail lines2500-2540 once, natural EOF at2524; complete loader checks expected source/project/certificate/topology/geometry hashes but no `require_plane_sheet_payload=True`, only get_via assertions plus rail witness. Together with known `import_spd_scenario(source)`, the selected owner is a viable integration seam, but default-v2 and no-v3-regression claims remain. 17AM authorizes exactly once reading `src/spd_decap_pi/spd_adapter.py` lines6423-6465 (max43), no search/second range. Require complete `import_spd_scenario` signature, kw-only boundary/defaults, return type, and immediate setup; truncation at6465 => STOP without extension. No code/test authority in17AM. Frozen later candidate only (not active/authorized): minimal 2-file edit adding kw-only `include_plane_sheet_payload: bool=False` in `spd_adapter.py` and forwarding it at known call7860; selected test `import_spd_scenario(source, include_plane_sheet_payload=True)` at2298 and loader call2500 `require_plane_sheet_payload=True`; loader remains canonical v3 behavioral assertion without manifest-key duplication; then Sol diff + exactly one focused node `pytest -q tests/test_spd_decap_spd_adapter.py::test_import_runs_one_union_reachability_pass_and_persists_surface_certificate`. Other calls remain default-v2.
...
| 고비용 검증 권한 | one frozen 17AM bounded source excerpt + one Sol review are authorized; exact once/max43/endpoint STOP rules apply; candidate edit/test remains frozen and unauthorized; no search, code, test, or production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-IMPORT-API-SIGNATURE-TRACE` | 17AM | ACTIVE | trace import API signature and immediate setup boundary | exactly once read `src/spd_decap_pi/spd_adapter.py` lines6423-6465 (max43), no search/second range; require complete `import_spd_scenario` signature, kw-only boundary/defaults, return type, immediate setup; truncation at6465 => STOP; no code/test authority |
...
### 17AM. W7-PHYS-W6-RAW-SPATIAL-V3-IMPORT-API-SIGNATURE-TRACE (ACTIVE)

Authorize exactly once reading `src/spd_decap_pi/spd_adapter.py` lines 6423-6465 (maximum 43 lines),
with no search or second range. Require the complete `import_spd_scenario` signature, kw-only boundary
and defaults, return type, and immediate setup. If the signature/default boundary is truncated at6465
=> STOP without extension. No code or test authority in17AM.

Frozen later candidate only (not active/authorized): minimal two-file edit adding kw-only
`include_plane_sheet_payload: bool=False` in `spd_adapter.py` and forwarding it at known call7860;
selected test `import_spd_scenario(source, include_plane_sheet_payload=True)` at2298 and loader call2500 with `require_plane_sheet_payload=True`; loader
is the canonical v3 behavioral assertion without manifest-key duplication; then Sol diff plus exactly
one focused node `pytest -q tests/test_spd_decap_spd_adapter.py::test_import_runs_one_union_reachability_pass_and_persists_surface_certificate`.
Other calls remain default-v2.
## 20. 변경 기록

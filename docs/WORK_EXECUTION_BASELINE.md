...
- 문서 버전: **1.127**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.126
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-ATTACHMENT-CONTENT-HASH-BODY-TRACE` (17CU) |
| 다음 권장 묶음 | `17CU` exact once-only bounded body read |
| current authorization | 17CT consumed at clean evidence `6dec0446c8b23d294104d1aea4fc843f763e20f6`; its exact validation-use query ran once, exit0, 2 lines/1 file, span6. Line862 had name/digest str plus attachment-membership guard; line868 was open `if observed != digest:` with observed origin and complete mismatch body absent. No retry/body read. 17CU permits one exact bounded body read of lines862–870 in `src/spd_decap_pi/raw_spatial_contact_compiler.py`, requiring all type/membership, canonical SHA-256 observation, equality/mismatch branch, and no external/default/alias dependency to be complete; otherwise stop with no retry or expansion. |
...
| 고비용 검증 권한 | none; 17CU allows only the bounded lines862–870 body read; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-ATTACHMENT-CONTENT-HASH-BODY-TRACE` | 17CU | ACTIVE | bounded body trace for attachment content SHA | exact lines862–870 read once; all nine lines must prove type/membership guard, canonical SHA-256 over exact bytes, equality/mismatch fail branch, no alias/default/synthesis/external dependency, line870 closed; any gap STOP/no retry; location exclusions remain |
...
### 17CU. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-ATTACHMENT-CONTENT-HASH-BODY-TRACE (ACTIVE)

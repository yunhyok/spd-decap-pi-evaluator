...
- 문서 버전: **1.101**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.100
...
| 현재 active work item | `W7-PHYS-W6-RAW-SURFACE-FIELD-ORIGIN-LOCAL-TRACE` (17BU) |
| 다음 권장 묶음 | `17BU` exact once-only numbered local read 2460-2535 |
| current authorization | 17BU bounded read: use the recorded Get-Content numbered command exactly once for compiler.py lines2460-2535; require complete item creation/return, direct origins+normalization/validation for net/layer/asset SHA/islands, and complete `_surface_source_hashes(...)` call/args in range; endpoint/dependency outside range STOP/no expansion/retry. Helper internals, canonical SHA, island/electrical ownership remain separate. No physical stamp/accuracy execution. |
...
| 고비용 검증 권한 | none; 17BU allows only one bounded 76-line source read, with source expansion/code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-RAW-SURFACE-FIELD-ORIGIN-LOCAL-TRACE` | 17BU | ACTIVE | trace local field origins and source-hash call | exact once-only numbered read compiler.py2460-2535 (76 lines), no expansion/retry; require complete item creation/return, net/layer/asset SHA/islands origins+normalization/validation, and `_surface_source_hashes(...)` call/args in range; helper/canonical SHA/island ownership separate |
...
is DONE/location PASS, and 17BU is the sole ACTIVE bounded local field-origin trace.
...
at the single 2500-2512 cluster. Sole current read-only successor 17BU owns the exact 2460-2535 local trace.
Frozen evidence establishes no physical stamp/profile/replacement/nullspace/gauge/Zii/production/release/
PowerSI accuracy claim.
...
### 17BU. W7-PHYS-W6-RAW-SURFACE-FIELD-ORIGIN-LOCAL-TRACE (ACTIVE)
...
for ($number = 2460; $number -le 2535; $number++) {
...
Gate: exactly once, output only numbered lines2460-2535 (76 lines). Require item creation and return,
net/layer/asset SHA/islands direct origins plus normalization/validation, and the complete
`_surface_source_hashes(...)` call/args in range. STOP/no expansion/no retry if an endpoint cuts a
statement/loop/branch or values depend on pre-range data/external helper semantics. Helper internals,
canonical SHA, island/electrical ownership, physical stamp, and accuracy remain separate.

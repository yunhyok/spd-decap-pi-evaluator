...
- 문서 버전: **1.62**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.61
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-SEAM-TRACE` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-SEAM-TRACE` (ACTIVE; two bounded excerpts) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AJ is DONE at clean main HEAD `4aaf242dc6da530b600b01e924f4a9c01ab88c01`: exact diagnostic once exit0, exactly 4 lines for the expected file; target 1750 is bracketed by definition lines 1736-1764 and contains no frozen 17AH non-import call; target 2487 is bracketed by line 1764 and a valid EOF sentinel and contains frozen call 2298; exactly one owner selected. 17AK authorizes exactly two excerpts, each once, from `tests/test_spd_decap_spd_adapter.py`: A lines 2284-2312 (29 lines) and B lines 2468-2505 (38 lines), total <=67; no owner-wide read/search/other range. A must answer complete call 2298 assignment/args/kwargs and immediate result/status; B must answer complete RAW-key extraction/assertions/schema/version/tables/attachment expectations and whether the test tail completes. If A truncates call/args or B truncates RAW-manifest assertions/test tail => STOP. Only conclusion is whether this owner is a viable minimal opt-in regression seam while default-v2 tests stay separate. No profile, accuracy, production, or code/test execution proof.
...
| 고비용 검증 권한 | one frozen 17AK two-excerpt seam trace + one Sol review are authorized; exactly-once/range/67-line caps and endpoint STOP rules apply; no owner-wide read, search, code, test, or production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-SEAM-TRACE` | 17AK | ACTIVE | trace the selected owner with two bounded excerpts for opt-in regression seam viability | exactly two excerpts once from `tests/test_spd_decap_spd_adapter.py`: A lines2284-2312 (29), B lines2468-2505 (38), total <=67; no owner-wide read/search/other range; A complete call2298 assignment/args/kwargs + immediate result/status; B complete RAW-key extraction/assertions/schema/version/tables/attachment expectations + test-tail completion; endpoint truncation => STOP; only conclusion is viability as minimal opt-in seam while default-v2 tests stay separate |
...
### 17AK. W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-SEAM-TRACE (ACTIVE)

Authorize exactly two excerpts, each once, from the same test file, total <=67 lines:

- Excerpt A: `tests/test_spd_decap_spd_adapter.py` lines 2284-2312 (29 lines). Answer the complete
  call 2298 assignment, arguments/kwargs, and immediate result/status.
- Excerpt B: the same file lines 2468-2505 (38 lines). Answer complete RAW-key extraction/assertions,
  schema/version/tables/attachment expectations, and whether the relevant test tail completes.

No owner-wide read, search, or other range is authorized. If A truncates the call/arguments or B
truncates RAW-manifest assertions/test tail => STOP without extension. The only conclusion permitted
is whether this owner is a viable minimal opt-in regression seam while default-v2 tests stay separate.
No code/test/profile/accuracy/production proof.
## 20. 변경 기록

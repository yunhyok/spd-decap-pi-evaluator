...
- 문서 버전: **1.65**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.64
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-WIRING` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-WIRING` (ACTIVE; exact two-file wiring authority) |
| current authorization | S2 focused node PASS remains exact prior evidence (`1 passed in 1.09s`), no rerun/broad-suite/production run. 17AL DONE (source-classified test gap) at clean main `7618408f8979d07f6810ed1052cb472b8837b222`: tail2500-2540 once, natural EOF2524; loader checks hashes but no `require_plane_sheet_payload=True`, only get_via assertions plus rail witness; selected owner viable seam, default-v2/no-v3-regression claims remain. 17AM DONE at clean main `2076ba2764a4ae7614a7fdaa55ee6b208f6fdbad`: exact6423-6465 once, complete `import_spd_scenario(path: str|Path, *, progress=None, is_cancelled=None) -> ScenarioImport`, safe kw-only extension point, setup not truncated. 17AN is the sole ACTIVE authority: exactly two files only—`spd_adapter.py` adds kw-only `include_plane_sheet_payload: bool=False` and forwards that exact named kwarg at frozen compiler call; selected test calls `import_spd_scenario(source, include_plane_sheet_payload=True)` and loader uses `require_plane_sheet_payload=True`; no manifest internals/helper/unrelated assertion. After Sol static diff, run exactly once `pytest -q tests/test_spd_decap_spd_adapter.py::test_import_runs_one_union_reachability_pass_and_persists_surface_certificate`; failure STOP/no rerun; PASS => exact two authorized technical files only atomic commit; FAIL => STOP, no rerun. other callers default-v2; no broad/production/release/profile/accuracy claim.
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-WIRING` | 17AN | ACTIVE | wire opt-in plane-sheet payload through adapter and selected regression seam | exactly two files only: add kw-only `include_plane_sheet_payload: bool=False` and forward exact named kwarg at frozen compiler call; selected test calls `import_spd_scenario(source, include_plane_sheet_payload=True)` and loader uses `require_plane_sheet_payload=True`; no manifest internals/helper/unrelated assertion; after Sol static diff exactly one focused node, failure STOP/no rerun; PASS => exact two authorized technical files only atomic commit; FAIL => STOP, no rerun. Other callers default-v2; no broad/production/release/profile/accuracy claim |
...
### 17AN. W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-WIRING (ACTIVE)

Exact authority is two files only. In `spd_adapter.py`, add kw-only
`include_plane_sheet_payload: bool=False` and forward that exact named kwarg at the frozen compiler
call. In the selected adapter test, call
`import_spd_scenario(source, include_plane_sheet_payload=True)` and add
`require_plane_sheet_payload=True` to the loader; do not add manifest internals, helpers, or unrelated
assertions. After Sol static diff, run exactly once
`pytest -q tests/test_spd_decap_spd_adapter.py::test_import_runs_one_union_reachability_pass_and_persists_surface_certificate`;
failure is STOP with no rerun. PASS => exact two authorized technical files only atomic commit; FAIL => STOP, no rerun. Other callers remain default-v2; no broad/production/release/profile/accuracy claim.
## 20. 변경 기록

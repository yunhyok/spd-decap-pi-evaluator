...
- 문서 버전: **1.57**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.56
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-SYMBOL-DISCOVERY` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-SYMBOL-DISCOVERY` (ACTIVE; one frozen symbol query) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AD is DONE at clean main HEAD `7537c8ebee9109071db54cc84f5562e375bfc07f`, exact query once exit0, 2 lines/1 file; definition `src/spd_decap_pi/spd_adapter.py:131` and selected non-definition caller `src/spd_decap_pi/spd_adapter.py:7868`, location evidence only. 17AE is DONE as source-location boundary evidence at clean main HEAD `c5f3b1a234c8deeffa26b0fa6cf60dabc4d576d9`: exact frozen PowerShell once exit0, exactly 2 lines/1 file (`src/spd_decap_pi/spd_adapter.py:6423:def import_spd_scenario(` and `src/spd_decap_pi/spd_adapter.py:8024:def verify_scenario_source(`); bracketing span 1601 lines is not reasonably bounded, so no contiguous/implementation read or compiler/delegate inference. 17AF permits only the exact query `rg -n --with-filename --no-heading --color never '\b(?:compile_raw_spatial_contact_asset|_merge_raw_spatial_contact_asset)\b' 'src/spd_decap_pi/spd_adapter.py'`, cap <=10 output lines and exactly 1 file; ignore import/definition matches, require a non-import/non-definition compiler occurrence at/before the known merge call and nearest compiler-to-merge span <=120 lines; error/over-cap/wrong file/missing call/compiler after merge/span >120 => STOP. Location evidence only; no files open or contiguous read. |
...
| 고비용 검증 권한 | one frozen 17AF compiler/merge symbol query + one Sol review are authorized; cap/span/STOP rules above apply; no file open, code/test/production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-SYMBOL-DISCOVERY` | 17AF | ACTIVE | locate compiler symbol relative to the known merge call | exact once-only query `rg -n --with-filename --no-heading --color never '\b(?:compile_raw_spatial_contact_asset|_merge_raw_spatial_contact_asset)\b' 'src/spd_decap_pi/spd_adapter.py'`; cap <=10 output lines/exactly 1 file; ignore import/definition matches; require non-import/non-definition compiler occurrence at/before merge and nearest span <=120 lines; error/over-cap/wrong file/missing call/compiler after merge/span >120 => STOP; location evidence only, no files open |
...
17Z는 DONE으로 닫고, 17AA와 17AA-S1은 historical BLOCKED로 남기며 17AA-S2는 test-gate PASS DONE으로 닫는다. 17AB는 DONE, 17AC는 source-classified STOP/BLOCKED로 닫고 17AD는 DONE으로 닫는다. 17AE는 source-location boundary evidence DONE으로 닫고 ACTIVE는 `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-SYMBOL-DISCOVERY`(17AF)로 지정한다.
...
Output is location evidence only; no files open or automatic contiguous read. v2 constants/API는
...
### 17AF. W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-SYMBOL-DISCOVERY (ACTIVE)
...
any contiguous read requires separate authorization.

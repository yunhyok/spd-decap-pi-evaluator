...
- 문서 버전: **1.63**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.62
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-TAIL-TRACE` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-TAIL-TRACE` (ACTIVE; one bounded tail excerpt) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AJ is DONE with deterministic one-owner location evidence at clean main HEAD `4aaf242dc6da530b600b01e924f4a9c01ab88c01`. 17AK is BLOCKED (source-classified endpoint STOP) at clean main HEAD `286e939fce53918465de72f8be9b67d3f1844470`: exact excerpts A lines 2284-2312 and B lines 2468-2505 each once; A proves `import_spd_scenario(source)` line2298 with no args/kwargs/opt-in and immediate status assertions; B proves persisted topology/raw manifest attachment/source/project/certificate/topology hash checks through2499, then `load_raw_spatial_contact_asset` begins at2500 and is truncated at2505, leaving schema/version/tables/require-plane/tail UNPROVEN; no extension or reread. 17AL authorizes exactly once reading `tests/test_spd_decap_spd_adapter.py` lines2500-2540 (max41; natural EOF earlier valid), with no search/other range. Require complete loader call/args, returned payload assertions including schema/version/tables/plane-sheet requirement if any, and actual test termination/EOF; if call/assertions open at2540 => STOP without extension. Only conclusion is whether this owner can be a minimal opt-in regression seam while default-v2 tests stay separate. No production API design/code/test/profile/accuracy inference.
...
| 고비용 검증 권한 | one frozen 17AL bounded tail excerpt + one Sol review are authorized; exact once/max41/endpoint STOP rules apply; no search, other range, code, test, or production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-TAIL-TRACE` | 17AL | ACTIVE | complete the selected owner’s loader and test-tail bounded trace | exactly once read `tests/test_spd_decap_spd_adapter.py` lines2500-2540 (max41; natural EOF earlier valid); no search/other range; require complete loader call/args, returned payload assertions including schema/version/tables/plane-sheet requirement if any, and actual test termination/EOF; call/assertion open at2540 => STOP; only conclusion is minimal opt-in seam viability while default-v2 tests stay separate |
...
### 17AL. W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-OPTIN-TEST-TAIL-TRACE (ACTIVE)

Authorize exactly once reading `tests/test_spd_decap_spd_adapter.py` lines 2500-2540 (maximum 41
lines; natural EOF earlier is valid), with no search or other range. Require the complete loader
call/arguments, returned payload assertions including schema/version/tables and plane-sheet requirement
if any, and actual test termination/EOF. If the call or assertions remain open at line 2540 => STOP
without extension. The conclusion ceiling is whether this owner can be a minimal opt-in regression
seam while default-v2 tests stay separate; no production API design/code/test/profile/accuracy
inference.
## 20. 변경 기록

...
- 문서 버전: **1.123**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.122
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATOR-BOUNDARY-DISCOVERY` (17CQ) |
| 다음 권장 묶음 | `17CQ` exact once-only `_digest_hex` definition-boundary query |
| current authorization | 17CQ gate: exact query once, unique `_digest_hex` definition plus immediate next top-level boundary, exit0 exactly 2 lines/1 file, positive span<=80; missing/duplicate/parse/path/order/over-span STOP/no retry, location-only body forbidden. |
...
| 고비용 검증 권한 | none; 17CQ allows only the bounded definition query; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATOR-BOUNDARY-DISCOVERY` | 17CQ | ACTIVE | trace asset SHA digest validator boundary | exact query once; `_digest_hex` definition + immediate next top-level boundary, exactly 2 lines/1 file, positive span<=80; location-only/no body read |
...
### 17CQ. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATOR-BOUNDARY-DISCOVERY (ACTIVE)

Standing preapproval authorizes exactly this definition-boundary query once:
...
$targets = @($numbered | Where-Object { $_.Text -match '^def _digest_hex\(' })
if ($targets.Count -ne 1) { exit 3 }
$next = @($numbered | Where-Object { $_.Number -gt $targets[0].Number } | Select-Object -First 1)
if ($next.Count -ne 1) { exit 4 }
if ($targets[0].Path -ne $path -or $next[0].Path -ne $path) { exit 5 }
if ($next[0].Number -le $targets[0].Number) { exit 6 }
$output = @($targets[0].Raw, $next[0].Raw)
if ($output.Count -ne 2) { exit 7 }
$output
```

Gate: exact once exit0, exactly 2 lines/1 file, unique `_digest_hex` definition and immediate next top-level boundary, positive span <=80; missing/duplicate/parse/path/order/over-span => STOP/no retry. Location-only; no body read, islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.

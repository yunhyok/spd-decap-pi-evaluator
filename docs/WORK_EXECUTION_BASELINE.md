...
- 문서 버전: **1.82**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.81
...
- 상태: **APPROVED/MACHINE-FROZEN CONTROL DOCUMENT**; full phase/item history lives in the register and changelog; W5 DONE; W6-BASE 260729 numerical FAIL; W7-PHYS BLOCKED; 17AX BLOCKED/STOP; 17AY DONE (location evidence); 17AZ BLOCKED/STOP (source-classified delegated substrate owner); 17BA BLOCKED/STOP (boundary span627>500); 17BB BLOCKED/STOP (cache-only multiple clusters); 17BC BLOCKED/STOP (endpoint truncation + delegated identity); 17BD DONE (location evidence); 17BE ACTIVE.
- 현재 권위 상태: sole ACTIVE 17BE authorizes exactly one read of `src/spd_decap_pi/_core/solver/layerwise_network.py` lines3343-3545 (203); questions canonical/hash inputs and ordering, project/rail/geometry asset/certificate/topology/omission/attachment binding, raw-v3 manifest/payload/content hash status, additive opt-in-only payload-hash seam, and collision/alias/nonfinite/missing fail-closed; PASS requires complete helpers plus source-proven opt-in additive seam; endpoint/delegated hash normalization => STOP; no expansion or replacement/no-double-count/nullspace/gauge/Zii/code/test/solver/production/release/accuracy claims.
...
| 현재 active work item | `W7-PHYS-W6-LAYERWISE-SUBSTRATE-IDENTITY-HELPER-TRACE` |
| 다음 권장 묶음 | `W7-PHYS-W6-LAYERWISE-SUBSTRATE-IDENTITY-HELPER-TRACE` (ACTIVE; one exact 3343-3545 read) |
| current authorization | 17BA BLOCKED/STOP: boundary span627>500; 17BB BLOCKED/STOP: 8 cache-only rows, combined span385, raw-v3 token0; 17BC BLOCKED/STOP: exact3669-3945 read endpoint-truncated with identity delegated. 17BD DONE(location evidence): helper spans84/119, repeated3462 intended adjacency. Sole ACTIVE 17BE authorizes one read layerwise_network.py3343-3545 (203); endpoint/delegated hash normalization => STOP; no expansion or implementation execution. |
...
| 고비용 검증 권한 | none; only the low-cost bounded 17BE 3343-3545 read is authorized, with no expansion or code/test/solver/production/release/accuracy execution |
...
| `W7-PHYS-W6-LAYERWISE-SUBSTRATE-IDENTITY-HELPER-TRACE` | 17BE | ACTIVE | trace substrate identity helpers | exactly one read `src/spd_decap_pi/_core/solver/layerwise_network.py` lines3343-3545 (203); questions canonical/hash inputs/order, project/rail/geometry/certificate/topology/omission/attachment binding, raw-v3 manifest/payload/content hash status, additive opt-in payload-hash seam, collision/alias/nonfinite/missing fail-closed; endpoint/delegated hash normalization => STOP; no expansion or replacement/no-double-count/nullspace/gauge/Zii/code/test/solver/production/release/accuracy claim |
...
### 17BE. W7-PHYS-W6-LAYERWISE-SUBSTRATE-IDENTITY-HELPER-TRACE (ACTIVE)

Authorize exactly one read of `src/spd_decap_pi/_core/solver/layerwise_network.py` lines3343-3545 (203).
Questions: canonical/hash inputs and ordering; project/rail/geometry asset/certificate/topology/omission/
attachment binding; raw-v3 manifest/payload/content hash inclusion or absence; additive opt-in-only payload-hash
seam preserving legacy identity; collision/alias/nonfinite/missing fail-closed. PASS requires complete helpers
and source-proven raw-v3 inclusion status plus opt-in additive seam. STOP if delegates external hash/normalization
or the endpoint cuts a statement/block; no expansion. Exclude replacement/no-double-count, nullspace/gauge/Zii,

...
- 문서 버전: **1.69**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.68
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-GUI-OPTIN-ACTIVATION` | 17AR | ACTIVE | activate the GUI v3 opt-in at the existing import call | only `src/spd_decap_pi/gui/main_window.py`; add exactly one line `include_plane_sheet_payload=True,`; expected diff 1 file/1 insertion; preserve path/progress/cancellation/view/return/API default/other callers; Sol static diff exactly once; no tests; ACCEPT one-file atomic commit, REJECT STOP; no search/source extension/new GUI test/helper/production/solver/profile/accuracy/release/version claim |
...
### 17AR. W7-PHYS-W6-RAW-SPATIAL-V3-GUI-OPTIN-ACTIVATION (ACTIVE)

Only `src/spd_decap_pi/gui/main_window.py` is authorized: add exactly one line
`include_plane_sheet_payload=True,` to the existing import call. Expected diff is 1 file/1 insertion;
preserve path/progress/cancellation/view/return and API defaults/other callers. Sol performs one static
diff review; no tests (reuse prior adapter opt-in focused PASS). ACCEPT => one-file atomic commit;
REJECT => STOP. No search/source extension/new GUI test/helper/production SPD or GUI run/solver/profile/
accuracy/release/version claim.
## 20. 변경 기록

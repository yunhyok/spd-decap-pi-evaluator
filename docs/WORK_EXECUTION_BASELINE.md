...
- 문서 버전: **1.31**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.30
...
| 현재 active work item | `W7-PHYS-MAXWELL-PARTIAL-SOURCE-TRACE` |
| 다음 권장 묶음 | `W7-PHYS-MAXWELL-PARTIAL-SOURCE-TRACE` ACTIVE; exact one-file source V0 + one Sol review |
| current authorization | clean `main` HEAD `675a7d9ccb9240ab3c88aadcef5dbe059aef7bb3`; only `src/spd_decap_pi/_core/solver/uniform_c00.py` may be read; Python/import/test/artifact/raw-SPD/solver/production/edit remain zero |
...
| 고비용 검증 권한 | none; bounded one-file read-only source V0 + Sol review only; no artifact/Python/import/test/solver/production execution |
...
| `W7-PHYS-MAXWELL-PARTIAL-SOURCE-TRACE` | 17K | ACTIVE | determine whether `uniform_c00.py` generates only adjacent-gap quasi-static/dispersive dielectric C00 or also plane-sheet series R/L/nonuniform/current-spreading | exact whitelist `src/spd_decap_pi/_core/solver/uniform_c00.py`; one source V0 + one Sol review; all execution/artifact/edit paths zero |
...
The sole ACTIVE item is `W7-PHYS-MAXWELL-PARTIAL-SOURCE-TRACE`: read only
`src/spd_decap_pi/_core/solver/uniform_c00.py` once, then one independent Sol static review.
Classify whether the delegate generates adjacent-gap quasi-static/dispersive dielectric C00 or
any position/material-dependent plane-sheet series R/L, nonuniform, or current-spreading term;
stop and require a separately approved delegate whitelist if the file delegates any coefficient
or frozen W6 binding cannot be maintained. This is observability ordering only, not causality or
physics authorization.

17J is closed: topology-only links are ideal vertical-node coalescence, not a same-layer
numeric stamp; incoming C[F] is remapped/collapsed, dielectric-ratio `j2πf·C` becomes S,
finite vertical Via links use `count/(R+j2πfL)`, and termination returns S. No local
position/width/plane-conductor-material sheet R/L or nonuniform/current-spreading term exists.
The sole ACTIVE successor is 17K, limited to `src/spd_decap_pi/_core/solver/uniform_c00.py`.

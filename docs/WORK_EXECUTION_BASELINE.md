...
- 문서 버전: **1.38**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.37
...
| 현재 active work item | `W7-PHYS-W6-PLANE-SHEET-INTEGRATION-BINDING-TRACE` |
| 다음 권장 묶음 | 17R exact three-file complete-read integration trace; one Sol review, then code/test whitelist and V1/V2 freeze |
| current authorization | clean `main` HEAD `2bf27d4c49d8d4c7d51866815df4ab42ca537f16`; read only `src/spd_decap_pi/_core/solver/profiles.py`, `src/spd_decap_pi/_core/solver/layerwise_network.py`, `src/spd_decap_pi/spd_adapter.py`, one complete read each; no other source/search/import/Python/test/artifact/solver/production/edit |
...
| 고비용 검증 권한 | 17R three-file source trace + one Sol review only; no code/test/artifact/Python/import/solver/production execution; coding waits for frozen 17R whitelist/identity/commit/V1/V2 plan |
...
| `W7-PHYS-W6-PLANE-SHEET-INTEGRATION-BINDING-TRACE` | 17R | ACTIVE | bind opt-in no-fallback profile, hash-bound inputs, balanced differential incidence, stable rail/Zii output, and exact ownership transitions | clean `2bf27d4c49d8d4c7d51866815df4ab42ca537f16`; exact whitelist `src/spd_decap_pi/_core/solver/profiles.py`, `src/spd_decap_pi/_core/solver/layerwise_network.py`, `src/spd_decap_pi/spd_adapter.py`; one complete read each; any whitelist-outside delegate ends 17R as source-classified BLOCKED, with no fourth read or automatic whitelist expansion; one Sol review freezes code/test whitelist, identity, commits, V1/V2 |
...
The sole ACTIVE item is 17R `W7-PHYS-W6-PLANE-SHEET-INTEGRATION-BINDING-TRACE` at clean HEAD
`2bf27d4c49d8d4c7d51866815df4ab42ca537f16`. Read exactly once each:
`src/spd_decap_pi/_core/solver/profiles.py`, `src/spd_decap_pi/_core/solver/layerwise_network.py`,
`src/spd_decap_pi/spd_adapter.py`. Acceptance binds opt-in no-fallback profile/static identity,
hash-bound polygon/material input without solve-time raw SPD/full-Scenario hydration, balanced
differential incidence `N^H B=0`, stable rail order/Zii, old plane Maxwell C/ideal topology disabled,
Via/termination exactly once, and single-worker/no-shared-mutable-cache behavior. Any delegate
outside the whitelist ends 17R as source-classified BLOCKED; no fourth read or automatic whitelist
expansion is permitted. One Sol review then freezes code/test whitelist, identity,
commits and V1/V2; no coding or execution occurs before that review, and production runs, old-root

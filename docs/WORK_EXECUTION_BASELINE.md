...
- 문서 버전: **1.72**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.71
...
- 17AT BLOCKED/implementation REJECT after one frozen-evidence design pass; all six bindings remain UNPROVEN. Sole ACTIVE 17AU: `W7-PHYS-W6-RAW-SPATIAL-V3-EVALUATION-HANDOFF-BOUNDARY-DISCOVERY`.
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-EVALUATION-HANDOFF-BOUNDARY-DISCOVERY` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-EVALUATION-HANDOFF-BOUNDARY-DISCOVERY` (ACTIVE; one frozen query) |
| current authorization | Sole ACTIVE 17AU authorizes one exact bounded search; no file open/read, code, test, production, PowerSI, profile, solver, accuracy, or causal execution. |
...
| 고비용 검증 권한 | none; only the low-cost bounded 17AU query is authorized, with no file read/code/test/production execution |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-EVALUATION-HANDOFF-BOUNDARY-DISCOVERY` | 17AU | ACTIVE | discover evaluation→solver handoff boundary from frozen evidence | exact once query `rg -n --with-filename --no-heading --color never '\b(?:load_raw_spatial_contact_asset|require_plane_sheet_payload|resolve_solver_profile|build_layerwise_uniform_source_model)\b' 'src/spd_decap_pi/raw_spatial_contact_asset.py' 'src/spd_decap_pi/evaluation.py'`; max30 lines/exact2 files; require clusters in both files; one unambiguous cluster each selects later bounded range; missing/multiple/ambiguous/error/over => STOP; no file open/code/test/production/PowerSI/profile/solver/accuracy/causal claim |
...
### 17AU. W7-PHYS-W6-RAW-SPATIAL-V3-EVALUATION-HANDOFF-BOUNDARY-DISCOVERY (ACTIVE)

Run exactly once on clean docs-commit HEAD `293c64b4de6aad9f5d828f7c270e5c8eccd53e68`:
`rg -n --with-filename --no-heading --color never '\b(?:load_raw_spatial_contact_asset|require_plane_sheet_payload|resolve_solver_profile|build_layerwise_uniform_source_model)\b' 'src/spd_decap_pi/raw_spatial_contact_asset.py' 'src/spd_decap_pi/evaluation.py'`.
Cap at 30 lines and exactly 2 files; require at least one relevant cluster in each file. Only one
unambiguous cluster per file may select a later bounded range; missing, multiple, ambiguous, error, or
over-budget => STOP. No file open/code/test/production/PowerSI/profile/solver/accuracy/causal claim.
## 20. 변경 기록

...
- 문서 버전: **1.130**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.129
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SURFACE-ASSET-DOMAIN-FROZEN-SYNTHESIS` (17CX) |
| 다음 권장 묶음 | 17CX docs-only frozen-evidence judgment: four cross-row asset/name/SHA/net/layer bindings must directly match; otherwise BLOCKED/design REJECT |
| current authorization | 17CW consumed at clean docs/evidence HEAD `0540090cd05ea29b202c5b62462d9f03c12e6c4f`; exact defs-boundary query ran once, exit0 exactly 2 lines/1 file (line857 `_validate_plane_sheet_assets(...)`, line872 `_parse_layers(`, span15), and combined 17CV/17CU evidence proves all-path attachment-byte digest provenance. 17CX permits one docs-only judgment using frozen evidence only; source/search/artifact/test/import/build/code edit budget is 0; all four asset/name/SHA/net/layer bindings must be directly proven or STOP as BLOCKED/design REJECT. |
...
| 고비용 검증 권한 | none; 17CX is docs-only frozen-evidence judgment; no source/search/artifact/test/import/build/code/production execution |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SURFACE-ASSET-DOMAIN-FROZEN-SYNTHESIS` | 17CX | ACTIVE | frozen-evidence-only cross-row source-asset/name/SHA/net/layer domain judgment | one docs-only judgment; source/search/artifact/test/import/build/code edit budget 0; all four bindings must directly match or BLOCKED/design REJECT; no new reads/retry; islands/electrical ownership/negative-shape meaning/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded |
...
### 17CX. W7-PHYS-W6-PLANE-PRIMITIVE-SURFACE-ASSET-DOMAIN-FROZEN-SYNTHESIS (ACTIVE)

This is a docs-only, frozen-evidence-only judgment. Using canonical docs and frozen 17BS/17BW/17BX/17CI/17CJ/17CO/17CP/17CU–17CW evidence, evaluate whether (1) primitive `source_asset_name`/`source_asset_sha256` and surface artwork asset fields derive unchanged from the same `spd_import.plane_geometries` record fields, (2) both paths use the same attachment-name and exact-byte SHA domain, (3) net/layer strings and casefold key domains agree, and (4) `(asset_sha, casefold(net), casefold(layer))` cross-row exact matching is directly proven. PASS requires all four bindings; inference yields BLOCKED/design REJECT. Source/search/artifact/test/import/build/code edit budget is zero; no new reads or retry. Islands/electrical ownership/negative-shape meaning/overlap/stamp/replacement/profile/nullspace/Zii/production/accuracy/release remain excluded.
## 20. 변경 기록

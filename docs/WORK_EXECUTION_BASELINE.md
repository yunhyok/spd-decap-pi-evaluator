...
- 문서 버전: **1.135**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.134
...
| 현재 active work item | `W7-PHYS-W6-PLANE-GEOMETRIES-AST-OWNER-DIAGNOSTIC` (17DC) |
| 다음 권장 묶음 | `17DC` exact once-only AST owner diagnostic (canonical command fenced only in this section) |
| current authorization | 17DB consumed at clean main HEAD `2b5563dd90915747652e92efc0ae75960f6f62c5`; exact assignment-owner boundary diagnostic ran once, exit3/effectively zero output, so BLOCKED/location STOP with no retry/body read. 17DC permits exactly one fenced PowerShell/stdlib-AST diagnostic command using the verified Python executable: exit0 with exactly two metadata lines for targets 3226 and 9090, each selecting one smallest lexical owner and declaring distinct owner identities; otherwise BLOCKED/location STOP with no retry/body read. |
...
| 고비용 검증 권한 | none; 17DC allows only the bounded AST owner diagnostic; additional source search/read, artifact/test/import/build/code/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-GEOMETRIES-AST-OWNER-DIAGNOSTIC` | 17DC | ACTIVE | use stdlib AST to identify unique smallest lexical owner around assignments at lines3226 and9090 | exact fenced command once in Work §17DC using verified Python executable; exit0 exactly 2 metadata lines/1 file, one smallest lexical owner per target, distinct owner identities; failures STOP/no retry/body read; separate diagnostic, not 17DB retry or relaxation |
...
### 17DC. W7-PHYS-W6-PLANE-GEOMETRIES-AST-OWNER-DIAGNOSTIC (ACTIVE)

The exact command is run once, using the verified Python 3.12 executable and only the standard-library AST parser; it creates no temporary file and uses no dependency:

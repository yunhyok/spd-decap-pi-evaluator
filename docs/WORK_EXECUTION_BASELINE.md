...
- 문서 버전: **1.143**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.142
...
- Sole ACTIVE item (current): 17DL `W7-PHYS-CANDIDATE-RETAINED-SURFACE-ARTWORK-LIFECYCLE-REPAIR`; 17DK is historical BLOCKED/execution STOP.
- The historical chain's terminal `ACTIVE NONE. Any implementation remains gated.` wording is superseded: sole ACTIVE is 17DL under the exact whitelist.
...
| 현재 active work item | 17DL `W7-PHYS-CANDIDATE-RETAINED-SURFACE-ARTWORK-LIFECYCLE-REPAIR` (ACTIVE) |
| 다음 권장 묶음 | 17DL exact whitelist and focused lifecycle-contract node; no production run |
| current authorization | exact 17DL whitelist and focused node only; no 17DK rerun or production audit; no unrelated build/release authority |
...
| `W7-PHYS-CANDIDATE-RETAINED-SURFACE-ARTWORK-LIFECYCLE-REPAIR` | 17DL | ACTIVE | candidate retained-surface artwork lifecycle repair | exact two-file whitelist; Sol static ACCEPT; focused lifecycle-contract node once; no production run |
...
### 17DL. W7-PHYS-CANDIDATE-RETAINED-SURFACE-ARTWORK-LIFECYCLE-REPAIR (ACTIVE)

Sol design ACCEPT. Exact whitelist remains `scripts/validate_island_finite_via_gate.py`, `tests/test_validate_island_finite_via_gate.py`. Reject `distribution._distribution_plane_geometries`; use existing `core_services.spd_plane_geometry_record_payload()` per persisted record fail-closed, wrap with existing `SimpleNamespace`, and supply a small context manager for current `_retained_surface_artwork(project, attachments, plane_geometries, ...)` with `finally close()`. Map lifecycle fields exactly and wire batch/release/artwork callbacks matching production. One focused node only after Sol static ACCEPT: `pytest -q tests/test_validate_island_finite_via_gate.py::test_candidate_retained_surface_artwork_uses_current_lifecycle_contract`; no production run. 17DM future fresh production audit is NOT ACTIVE and eligible only after 17DL PASS/clean commit/new docs/Usage Guard/new output root.

```mermaid
flowchart LR
  A[17DK SOURCE-COVERAGE STOP] --> B[17DL lifecycle repair]
  B --> C[focused gate]
  C --> D[17DM eligible; not active]
```

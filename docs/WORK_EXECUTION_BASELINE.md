...
- 문서 버전: **1.133**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.132
...
| 현재 active work item | `W7-PHYS-W6-PLANE-GEOMETRIES-ASSIGNMENT-OWNER-BOUNDARY-DISCOVERY` (17DA) |
| 다음 권장 묶음 | `17DA` exact once-only assignment-owner boundary location query (canonical command fenced only in this section) |
| current authorization | 17CZ consumed at clean docs/evidence HEAD `8f3c40a1e02524f427fb07e66fcd9375f5651433`; assignment query ran once, exit0, 6 lines/3 files; one-file/one-cluster span<=120 gate failed, so no producer selection/retry/body read. 17DA permits exactly one fenced boundary query: exit0, exactly 4 unique lines/1 file, each target immediate top-level owner boundary span<=240, otherwise BLOCKED/location STOP with no retry/body read. |
...
| 고비용 검증 권한 | none; 17DA allows only the bounded assignment-owner boundary location query; additional source search/read, artifact/test/import/build/code/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-GEOMETRIES-ASSIGNMENT-OWNER-BOUNDARY-DISCOVERY` | 17DA | ACTIVE | locate immediate top-level owners around assignments at lines3226 and9090 in `_core/io/spd.py` | exact fenced command once in Work §17DA; exit0 exactly 4 unique lines/1 file; each target owner boundary span<=240; parse/path/order/count/span failure STOP/no retry/body read; location-only |
...
The superseded historical summary above is retained for audit history; current status is 17CZ BLOCKED/location STOP at 8f3c40a1e02524f427fb07e66fcd9375f5651433 and 17DA sole ACTIVE assignment-owner boundary discovery.
...
### 17DA. W7-PHYS-W6-PLANE-GEOMETRIES-ASSIGNMENT-OWNER-BOUNDARY-DISCOVERY (ACTIVE)
...
$output = foreach ($target in $targets) {
    ...
    if (($after[0].Number - $before[0].Number) -gt 240) { exit 6 }

    $before[0].Raw
    $after[0].Raw
}

if ($output.Count -ne 4) { exit 7 }
if (@($output | Select-Object -Unique).Count -ne 4) { exit 8 }
$output
```

Gate: exact once, exit0, exactly 4 unique lines/1 file; each target must have a unique enclosing top-level owner and immediate next boundary with span<=240. Parse/path/order/count/span failure, same or nested owner, or missing boundary stops with BLOCKED/location STOP and no retry/body read. Location-only; asset/SHA/net/layer meaning, islands, ownership, stamps, replacement, profile, nullspace, Zii, implementation, test, production, accuracy, and release remain excluded.
## 20. 변경 기록

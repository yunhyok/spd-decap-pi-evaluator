...
- 문서 버전: **1.128**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.127
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNERSHIP-DISCOVERY` (17CV) |
| 다음 권장 묶음 | `17CV` exact once-only `digests` cache-ownership location query |
| current authorization | 17CU consumed at clean docs/evidence HEAD `c2acac75c41a65c30a88efb65d11a202f8dfa4f9`; its lines862–870 body read ran once, exit0, exactly 9 lines. Type/name membership guard+fail, cache-miss `sha256(attachments[name]).hexdigest()`, cache write, equality/mismatch fail, and closed line870 were proven; cache-hit `digests.get(name)` trusts cached values and cache initialization/ownership/aliasing were unproven, so all-path byte provenance failed. No retry/expansion. 17CV permits one exact query for `digests` in `src/spd_decap_pi/raw_spatial_contact_compiler.py`, requiring exit0, 3–12 unique lines/1 file, one local empty-map initialization, line864 get and line867 write in one cluster span<=80; otherwise stop with no retry. |
...
| 고비용 검증 권한 | none; 17CV allows only the bounded `digests` cache-ownership location query; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNERSHIP-DISCOVERY` | 17CV | ACTIVE | locate attachment digest cache initialization/get/write ownership | exact `digests` query once; exit0, 3–12 unique lines/1 file; one local empty-map init, line864 get, line867 write in one cluster span<=80; parameter/import/global/nonempty init, alias/additional writer/multiple cluster/cap STOP/no retry; location-only |
...
### 17CV. W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNERSHIP-DISCOVERY (ACTIVE)
...
Gate: exact once, exit0, 3–12 unique lines from exactly 1 file; exactly one local empty-map initialization, line864 get, and line867 write must form one cluster span<=80. Parameter/import/global/nonempty initialization, alias assignment, additional writer/mutation, multiple cluster, comments/strings-only, or cap/span failure stops with no retry. Location-only; no body read. Asset-name meaning, islands, ownership, stamps, profile, replacement, nullspace, Zii, production, accuracy, and release remain excluded.

...
- 문서 버전: **1.129**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.128
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNER-BOUNDARY-DISCOVERY` (17CW) |
| 다음 권장 묶음 | `17CW` exact once-only defs-boundary location query |
| current authorization | 17CV consumed at clean evidence HEAD `90ae22d410433b27383386281ea4e700bc7d0733`; exact `digests` query ran once, exit0, 11 lines/1 file. Relevant 858/864/867 cluster and separate 1951–2118 init/writer/membership/update/items cluster failed the one-cluster/no-additional-mutation gate; no retry/body read. 17CW permits exactly one defs-boundary command below; exit0 exactly 2 lines/1 file, first definition boundary <=858 and next >858, owner span <=200, unique top-level owner for line858 cluster ending before/separate from 1951+ cluster; otherwise BLOCKED/location STOP, no retry/body read. |
...
| 고비용 검증 권한 | none; 17CW allows only the bounded defs-boundary location query; additional source reads, body reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNER-BOUNDARY-DISCOVERY` | 17CW | ACTIVE | locate unique top-level owner boundary around line858 | exact PowerShell defs-boundary command once; exit0 exactly 2 lines/1 file, first boundary <=858 and next >858, positive owner span<=200, unique enclosing owner for line858 cluster ending before/separate from 1951+ cluster; parse/path/order/count/nested/dynamic ambiguity STOP/no retry; location-only, no body read |
...
### 17CW. W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNER-BOUNDARY-DISCOVERY (ACTIVE)
...
Gate: exact once, exit0, exactly 2 lines/1 file; first definition boundary <=858 and next >858, with positive owner span<=200. A unique enclosing top-level owner must positionally own the line858 cluster and end before/separate from the 1951+ cluster. Parse/path/order/count/span errors, nested/dynamic ambiguity, or any cap failure stops with no retry. Location-only; no body read. Asset-name meaning, islands, ownership, stamps, profile, replacement, nullspace, Zii, production, accuracy, and release remain excluded.

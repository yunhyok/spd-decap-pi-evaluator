...
- 문서 버전: **1.122**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.121
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-FIELD-BINDING-TRACE` (17CP) |
| 다음 권장 묶음 | `17CP` exact once-only 16-line asset-field binding body read |
| current authorization | 17CP body gate: exact read once lines803-818 (16 lines); complete missing lookup fail-closed, record asset/asset_sha256 validation and unchanged transfer to primitive source fields, and line818 expression; endpoint/delegated validation/alias/default/synthesis STOP/no expansion/retry. |
...
| 고비용 검증 권한 | none; 17CP allows only the bounded 16-line body read; additional source reads, code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-FIELD-BINDING-TRACE` | 17CP | ACTIVE | trace primitive source asset fields | exact body read once lines803-818 (16 lines); require missing lookup fail-closed, asset/asset_sha256 validation and unchanged transfer to primitive source fields, complete line818; endpoint/delegated validation STOP/no expansion |
...
### 17CP. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-FIELD-BINDING-TRACE (ACTIVE)

Standing preapproval authorizes exactly this body read once:

```powershell
$path = 'src/spd_decap_pi/raw_spatial_contact_compiler.py'
$lines = @(Get-Content -LiteralPath $path)
if ($lines.Count -lt 818) { exit 2 }
$output = for ($line = 803; $line -le 818; $line++) {
    '{0}:{1}:{2}' -f $path, $line, $lines[$line - 1]
}
if ($output.Count -ne 16) { exit 3 }
$output
```

PASS only if missing lookup fail-closed, record['asset'] and ['asset_sha256'] existence/format validation, unchanged transfer to primitive source fields, and complete line818 expression are present. Earlier/delegated validation, alias/default/synthesis, or an open endpoint => BLOCKED/STOP; no expansion or retry. Body scope only; islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.

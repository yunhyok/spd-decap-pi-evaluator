...
- 문서 버전: **1.134**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.133
...
| 현재 active work item | `W7-PHYS-W6-PLANE-GEOMETRIES-ASSIGNMENT-OWNER-BOUNDARY-DIAGNOSTIC` (17DB) |
| 다음 권장 묶음 | `17DB` exact once-only assignment-owner boundary diagnostic (canonical command fenced only in this section) |
| current authorization | 17DA consumed at clean main HEAD `28439758c9c7c5875fd174ec37b15fdc7e440982`; exact boundary command ran once, exit6, zero output, so BLOCKED/location STOP with no retry/body read. 17DB permits exactly one fenced diagnostic command: exit0 with exactly two structured records and two formatted lines/1 file, each line declaring path, target, Owner.Number+Owner.Raw, Next.Number+Next.Raw, and Span; owner numbers must be distinct, target order 3226 then 9090, owner<=target<next and positive span; no span limit, otherwise BLOCKED/location STOP with no retry/body read. |
...
| 고비용 검증 권한 | none; 17DB allows only the bounded assignment-owner boundary diagnostic; additional source search/read, artifact/test/import/build/code/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-GEOMETRIES-ASSIGNMENT-OWNER-BOUNDARY-DIAGNOSTIC` | 17DB | ACTIVE | independently diagnose owner/next boundaries for assignments at lines3226 and9090 | exact fenced command once in Work §17DB; exit0 exactly 2 structured records/2 formatted lines/1 file declaring path, target, Owner.Number+Owner.Raw, Next.Number+Next.Raw, Span; distinct owner numbers; ordered owner<=target<next with positive span, no span limit; parse/path/order/count failures STOP/no retry/body read; separate diagnostic, not 17DA retry |
...
### 17DB. W7-PHYS-W6-PLANE-GEOMETRIES-ASSIGNMENT-OWNER-BOUNDARY-DIAGNOSTIC (ACTIVE)

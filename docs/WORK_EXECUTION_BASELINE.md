...
- 문서 버전: **1.56**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.55
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-MERGE-OWNER-BOUNDARY-DISCOVERY` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-MERGE-OWNER-BOUNDARY-DISCOVERY` (ACTIVE; one frozen owner-boundary gate) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AB is DONE at its clean-main query boundary; 17AC is BLOCKED/source-classified STOP after one read, with v3 semantic validation and compiler/caller ownership unproven/delegated. 17AD is DONE at clean main HEAD `7537c8ebee9109071db54cc84f5562e375bfc07f`: exact frozen query once, exit 0, 2 output lines/1 file; definition `src/spd_decap_pi/spd_adapter.py:131` and exactly one selected non-definition caller `src/spd_decap_pi/spd_adapter.py:7868`, location evidence only, no files opened or compiler/delegate inference. 17AE permits only the exact fenced PowerShell command below; `--with-filename` is mandatory, exactly 2 output lines/1 file, first line <=7868 and second >7868; command error, parse failure, wrong count/file count, or nonbracketing => STOP. Output is only top-level owner/signature boundary location evidence; no implementation read/file open/code/test/profile/accuracy/production claim; later contiguous read requires separate authorization. |
...
| 고비용 검증 권한 | one frozen 17AE owner-boundary gate + one Sol review are authorized; exact command/bracketing STOP rules above apply; no file open, code/test/production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-MERGE-OWNER-BOUNDARY-DISCOVERY` | 17AE | ACTIVE | bracket the selected merge caller with top-level owner/signature definitions | exact fenced PowerShell gate with mandatory `--with-filename`; exactly 2 output lines/1 file, first line <=7868 and second >7868; command error, parse failure, wrong count/file count, or nonbracketing => STOP; no implementation read/file open |
...
17Z는 DONE으로 닫고, 17AA와 17AA-S1은 historical BLOCKED로 남기며 17AA-S2는 test-gate PASS DONE으로 닫는다. 17AB는 DONE, 17AC는 source-classified STOP/BLOCKED로 닫고 17AD는 DONE으로 닫는다. ACTIVE는 `W7-PHYS-W6-RAW-SPATIAL-V3-MERGE-OWNER-BOUNDARY-DISCOVERY`(17AE)로 지정한다.
...
delegate inference. 17AE is now the sole ACTIVE read with the exact fenced PowerShell gate below;
`--with-filename` is mandatory, output must be exactly 2 lines/1 file, first line <=7868 and second
>7868, otherwise STOP. The output is only top-level owner/signature boundary location evidence; no
implementation read, file open, code/test/profile/accuracy/production claim, or automatic contiguous
read is authorized. v2 constants/API는
...
### 17AE. W7-PHYS-W6-RAW-SPATIAL-V3-MERGE-OWNER-BOUNDARY-DISCOVERY (ACTIVE)

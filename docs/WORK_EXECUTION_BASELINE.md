...
- 문서 버전: **1.58**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.57
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-CALLSITE-TRACE` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-CALLSITE-TRACE` (ACTIVE; one bounded callsite read) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AD is DONE at clean main HEAD `7537c8ebee9109071db54cc84f5562e375bfc07f`, exact query once exit0, 2 lines/1 file; definition `src/spd_decap_pi/spd_adapter.py:131` and selected non-definition caller `src/spd_decap_pi/spd_adapter.py:7868`, location evidence only. 17AE is DONE as source-location boundary evidence at clean main HEAD `c5f3b1a234c8deeffa26b0fa6cf60dabc4d576d9`: exact frozen PowerShell once exit0, exactly 2 lines/1 file; bracketing span 1601 lines not reasonably bounded, so no contiguous/implementation read or compiler/delegate inference. 17AF is DONE at clean main HEAD `e4a6704247770cce084fd70fa3d5307f95a3a64a`: exact frozen query once exit0, 4 lines/1 file (compiler import 45, merge definition 131, actual compiler call 7860, known merge call 7868); after import/definition filtering, nearest span is 8 lines <=120, location evidence only. 17AG permits exactly one read of `src/spd_decap_pi/spd_adapter.py` lines 7848-7885 (38 lines), no whole-file/search/second range/code/test; answer immediate guard/provenance, complete compiler and merge calls/arguments/results, adjacent downstream use/return, and structural v3 manifest/generated passage; endpoint truncation => source-classified STOP without extension. No profile/solver/accuracy/production proof. |
...
| 고비용 검증 권한 | one bounded 17AG callsite read + one Sol review are authorized; exact range/STOP rules above apply; no whole-file/search, code/test/production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-CALLSITE-TRACE` | 17AG | ACTIVE | read the bounded compiler/merge callsite and adjacent control boundary | exactly one read of `src/spd_decap_pi/spd_adapter.py` lines 7848-7885 (38 lines); no whole-file/search/second range/code/test; endpoint truncation => source-classified STOP without extension |
...
17Z는 DONE으로 닫고, 17AA와 17AA-S1은 historical BLOCKED로 남기며 17AA-S2는 test-gate PASS DONE으로 닫는다. 17AB는 DONE, 17AC는 source-classified STOP/BLOCKED로 닫고 17AD는 DONE으로 닫는다. 17AE는 source-location boundary evidence DONE으로 닫고 17AF는 DONE으로 닫는다. ACTIVE는 `W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-CALLSITE-TRACE`(17AG)로 지정한다.
...
### 17AG. W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-CALLSITE-TRACE (ACTIVE)
...
immediate control boundary is truncated at an endpoint, source-classify STOP without extension.

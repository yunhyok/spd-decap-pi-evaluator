...
- 문서 버전: **1.120**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.119
...
| 현재 active work item | `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-CONSTRUCTION-DISCOVERY` (17CN) |
| 다음 권장 묶음 | `17CN` exact once-only records/key construction discovery query |
| current authorization | 17CN location gate: exact query once; exit0 2-8 unique lines/1 file all770-801; exactly one records iteration and one key assignment in one cluster span<=24; missing/loop/assignment/generic/duplicate/multiple/span/cap STOP/no retry; no body read. |
...
| 고비용 검증 권한 | none; 17CN allows only the bounded location query, with source body/code/test/build/import/production execution forbidden |
...
| `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-CONSTRUCTION-DISCOVERY` | 17CN | ACTIVE | locate source-record key construction | exact query once; exit0 2-8 unique lines/1 file all770-801; one records iteration and key assignment in one cluster span<=24; missing/loop/assignment/generic/duplicate/multiple/span/cap STOP/no retry; no body read |
...
### 17CN. W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-CONSTRUCTION-DISCOVERY (ACTIVE)

Standing preapproval authorizes exactly this location query once:

```powershell
$path = 'src/spd_decap_pi/raw_spatial_contact_compiler.py'
$hits = @(rg -n --with-filename --no-heading --color never `
    '^\s*(?:for\s+[A-Za-z_]\w*\s+in\s+records\s*:|key\s*=)' `
    $path)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$selected = @($hits | Where-Object {
    $_ -match '^[^:]+:(\d+):' -and
    [int]$Matches[1] -ge 770 -and
    [int]$Matches[1] -le 801
})
if ($selected.Count -lt 2 -or $selected.Count -gt 8) { exit 2 }
$selected
```

Gate: exit0, 2-8 unique lines/1 file all770-801; exactly one actual records iteration and one key assignment must form one cluster span<=24. Missing loop/assignment, generic/duplicate/multiple clusters, span/cap STOP/no retry. Location-only; no body read/requery/expansion. Islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.

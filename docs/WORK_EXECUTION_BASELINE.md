...
- 문서 버전: **1.71**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.70
...
- 17AS DONE-negative authority: clean main `9dba3076b6648e7aeb874c96fa721997e1629bb3`, exact authorized query once, exit0, 6 lines/1 file (`raw_spatial_contact_asset.py`); all matches were definitions/signature/guards/export, so actual `require_plane_sheet_payload=True` source caller count is zero. Producer active/no solver consumer; no rerun/read/code/test/production/PowerSI/accuracy/causal claim.
- Sole ACTIVE 17AT: `W7-PHYS-W6-RAW-SPATIAL-V3-SOLVER-CONSUMER-INTEGRATION-DESIGN`.
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-SOLVER-CONSUMER-INTEGRATION-DESIGN` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-SOLVER-CONSUMER-INTEGRATION-DESIGN` (ACTIVE; one frozen-evidence design pass) |
| current authorization | 17AS DONE-negative at clean main `9dba3076b6648e7aeb874c96fa721997e1629bb3` (query once, exit0, 6 lines/1 file, zero actual caller, no rerun); sole ACTIVE 17AT frozen-evidence-only design; zero source/search/code/test/production/PowerSI/accuracy/causal execution. |
...
| 고비용 검증 권한 | none; 17AT is frozen-evidence design only, with zero source/search/code/test/production execution |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-SOLVER-CONSUMER-INTEGRATION-DESIGN` | 17AT | ACTIVE | design the solver-consumer integration contract from frozen evidence | frozen evidence only; candidates limited to `raw_spatial_contact_asset.py`, `evaluation.py`, `_core/solver/profiles.py`, `_core/solver/layer_surface_network.py`; `surface_patch_plane.py`/`mfdm.py` reuse findings without reread; answer owner, hash-bound handoff, new-profile replacement/no-double-counting, differential/nullspace/rail-order, worker/cache, and minimal whitelist/V1/V2 STOP; missing contract => max2-file successor whitelist; no code/test/production/PowerSI/accuracy/causal claim |
...
### 17AT. W7-PHYS-W6-RAW-SPATIAL-V3-SOLVER-CONSUMER-INTEGRATION-DESIGN (ACTIVE)

One design pass is authorized from frozen evidence only; no new source/search/read is permitted. Candidate
owners are limited to `raw_spatial_contact_asset.py`, `evaluation.py`, `_core/solver/profiles.py`, and
`_core/solver/layer_surface_network.py`; `surface_patch_plane.py` and `mfdm.py` findings are reused without
reread. The pass must resolve: (1) exact owner for opt-in/no-fallback profile and loader `require_plane_sheet_payload=True`;
(2) source/project/certificate/topology/geometry hash-bound handoff into solve input; (3) new-profile-only
replacement disabling existing Maxwell C and ideal same-layer plane stamp while retaining finite Via+termination once;
(4) differential/nullspace/gauge and stable rail-order Zii mapping; (5) single-worker immutable payload and bounded
cache/resource contract; (6) minimal production/test whitelist with V1/V2 once-only STOP. If any symbol/data handoff
is not fixed by frozen evidence, implementation is not approved; return missing contract and max2-file successor
whitelist. No code/test/production/PowerSI/accuracy/causal/release claim.
## 20. 변경 기록

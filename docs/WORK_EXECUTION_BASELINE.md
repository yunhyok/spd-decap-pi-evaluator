...
- 문서 버전: **1.43**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.42
...
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-PAYLOAD-CONTRACT-TRACE` (NOT ACTIVE; explicit user approval required) |
| current authorization | 17T consumed at clean `main` HEAD `57aae22531d481623d34b6fb1b68bc0a201702c9`; no further search/read/expansion authority; no code/test/artifact/production/edit |
...
| 고비용 검증 권한 | none; 17T BLOCKED. Proposed successor requires explicit approval for one raw-spatial compiler read + one Sol review; no automatic whitelist creation |
...
| `W7-PHYS-W6-RAW-SPATIAL-PAYLOAD-CONTRACT-TRACE` | next candidate | BLOCKED / NOT ACTIVE | compile input/output/units/limits, complete polygon/material/dielectric/provenance/hash payload, bounded solve-time contract, fallback/delegates/resources | requires explicit user approval for one complete read of `src/spd_decap_pi/raw_spatial_contact_compiler.py` + one Sol review; any outside dependency or missing group fails closed |

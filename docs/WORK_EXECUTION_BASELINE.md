...
- 문서 버전: **1.60**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.59
...
| 현재 active work item | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DISCOVERY` |
| 다음 권장 묶음 | `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DISCOVERY` (ACTIVE; one frozen owner-boundary gate) |
| current authorization | S2 ran the approved focused node once: exit 0, `1 passed in 1.09s`; no rerun, broad-suite, or production run. The v3 emission goal is DONE. 17AH is DONE at clean main HEAD `d45a23f62ebd417a50498724bdea79ba77d8d041`: exact query once, exit0, 22 lines/1 file; `import_spd_scenario` import line 70; non-import calls lines 509, 587, 618, 647, 698, 791, 863, 965, 1207, 1208, 1229, 1249, 1272, 1291, 1409, 1450, 2298; `RAW_SPATIAL_CONTACT_ASSET_METADATA_KEY` import line 45 and checks lines 1750, 1752, 2487; no compiler/include flag matches. This is bounded location evidence only; no test owner selected from location alone. 17AI permits only the exact once-only PowerShell owner-boundary gate below, with `$targets = @(1750, 2487)` and top-level `test_` definition bracketing; it must produce exactly 4 lines/1 file, then use only the frozen 17AH call list for interval membership and select at most one owner only if exactly one bracket contains at least one observed non-import `import_spd_scenario` call; zero/both/ambiguous/overlap => STOP. No file open, code, test, profile, solver, accuracy, or production proof.
...
| 고비용 검증 권한 | one frozen 17AI owner-boundary PowerShell gate + one Sol review are authorized; exact 4-line/1-file and bracketing/count/ambiguity caps apply; no file open, code, test, or production work |
...
| `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DISCOVERY` | 17AI | ACTIVE | bracket observed metadata checks with one top-level adapter test owner | run exactly once the frozen PowerShell gate below; exactly 4 output lines/1 file; `$targets = @(1750, 2487)`; parse/count/file/bracketing failures or nonbracketing order => STOP; use only frozen 17AH call-line list for interval membership and select at most one owner only if exactly one bracket contains at least one observed non-import `import_spd_scenario` call; zero/both/ambiguous/overlap => STOP; no files opened, no code/test/inference |
...
### 17AI. W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DISCOVERY (ACTIVE)
...
$numbered = @(
    foreach ($line in $defs) {
        ...
        [pscustomobject]@{ File = $Matches[1]; Number = [int]$Matches[2]; Text = $line }
    }
)
if ($numbered.Count -ne $defs.Count -or @($numbered.File | Sort-Object -Unique).Count -ne 1) { exit 2 }
$out = @(
    foreach ($target in $targets) {
        ...
        if ($null -eq $before -or $null -eq $after) { exit 2 }
        $before.Text
        $after.Text
    }
)
...
The gate must produce exactly four output lines from exactly one file, with both target brackets
valid and ordered; parse/count/file errors or nonbracketing order => STOP. Use only the frozen 17AH
call-line list for interval membership and select at most one owner only if exactly one bracket
contains at least one observed non-import `import_spd_scenario` call; zero, both, ambiguous, or
overlapping ownership => STOP. Location evidence only; no file open, code, test, or inference.

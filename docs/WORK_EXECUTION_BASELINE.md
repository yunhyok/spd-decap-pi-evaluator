...
- 문서 버전: **1.42**
- 상위 기준: [목적·기술 기준](PRODUCT_PURPOSE_AND_TECHNICAL_BASELINE.md) v1.41
...
| 현재 active work item | `W7-PHYS-W6-PLANE-SHEET-BOUNDED-PATH-DISCOVERY` |
| 다음 권장 묶음 | 17T exact sequential bounded path-discovery queries A→B→C; one Sol review; no file open or implementation |
| current authorization | clean `main` HEAD `d5f489ba28000cd13bfaef719b17e5b4559d1d23`; run only the three exact commands below sequentially, each once; no other search/open/import/Python/test/artifact/solver/production/edit |
...
| 고비용 검증 권한 | 17T three sequential `rg` queries + one Sol review only; each <=40 lines and <=12 unique files; zero/over-budget/split/unrelated results STOP and no remaining query; no fourth query or automatic expansion |
...
| `W7-PHYS-W6-PLANE-SHEET-BOUNDED-PATH-DISCOVERY` | 17T | ACTIVE | identify bounded producer/profile/worker/differential integration candidates without opening files | clean `d5f489ba28000cd13bfaef719b17e5b4559d1d23`; commands A→B→C exactly once, sequential; each <=40 lines and <=12 unique files; zero/over-budget/split/unrelated results STOP; no fourth query or automatic whitelist expansion |

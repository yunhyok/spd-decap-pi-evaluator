...
- 현재 권위 상태: sole ACTIVE 17CF `W7-PHYS-W6-RAW-SURFACE-LAYER-MATCH-CONTROL-BOUNDARY-DISCOVERY`; exact once-only control-header query over compiler.py, exit0 6-32 unique lines/1 file all935-1040, max2 nonoverlap clusters, possible later split total<=100, otherwise STOP/no retry. Location-only; no direct body approval, and decode/SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release remain excluded.
...
17CF authorizes only the exact once-only control-header location query for `layer_match`/`pending` in
`raw_spatial_contact_compiler.py`; gate exit0, 6-32 unique lines/1 file all935-1040, control headers/continue
boundaries governing both pending updates and layer_match with indentation, max2 nonoverlap clusters, possible
later split total<=100, otherwise STOP/no retry. Location-only; no direct body approval. Decode/SHA/source hash/islands/ownership/stamp/profile/replacement/

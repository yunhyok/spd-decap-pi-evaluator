...
`W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S1`, limited to `raw_spatial_contact_compiler.py` and
`tests/test_raw_spatial_contact_compiler.py`; its static gate must inspect actual typed
`StackupLayer` fields (`discriminator`, `thickness_um`, `material_name`, canonical
`conductivity_s_per_m`) and exact dielectric-point epsilon/loss types, with no guessed aliases,
ad-hoc metadata, or monkeypatching. Sol reviews before the same focused node is run once;
failure stops without rerun. No production, accuracy, or version claim follows.

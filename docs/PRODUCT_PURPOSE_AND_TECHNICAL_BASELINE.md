...
- 현재 권위 상태: sole ACTIVE 17CO `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-POPULATION-BODY-TRACE`; exact once-only body read compiler.py lines780-802, 23 lines, with records source/container, raw-row key/blank-duplicate guard, asset validation, record_by_key population, and geometry lookup key; open/delegated/truncated or unproven key-domain semantics STOP/no expansion. Bounded body read only; no source expansion/requery, islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.
...
17CN is BLOCKED/location STOP at clean HEAD `55424cb09610260cf46699a5b6b6c9004ab8f56f`: the exact query ran once with exit0, 3 unique lines/1 file, span16 at lines785/788/801; two semantic key assignments were present, so normalization/equality/intermediate validation were not inferred. No body read or retry occurred. Sole ACTIVE successor is 17CO, a source-record key-population body trace.
17CO authorizes one exact 23-line body read:

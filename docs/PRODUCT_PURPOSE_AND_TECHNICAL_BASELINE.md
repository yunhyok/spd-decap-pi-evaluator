...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); Actual-P0/R1/R2와 R2-MULTI-TOPOLOGY-DIAG-01 및 Recovery-01은 DONE/STOP, FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`은 DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-02`는 `STOP_PROVENANCE_INCOMPLETE`로 영구 종료됐다.
  `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-03`만 ACTIVE이며 W7 production physics는 아직 BLOCKED다.
...
- Sole ACTIVE item (current): **`W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-03`** — DSU-01은 `STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`, DSU-02는 `STOP_PROVENANCE_INCOMPLETE`로 영구 종료했으며, DSU-03 lean producer-time observer만 진단 목적으로 유지한다. production wiring, topology 승격과 P12는 열지 않으며, 실제 source invocation은 별도 contract/self-check/Sol ACCEPT gate 이후에만 가능하다.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. P12 prospective review는 NO-GO다. docs contract commit, fixed +09:00/atomic-writer self-check와 Sol ACCEPT 뒤에만 한 번의 high-level original-SPD import/analyze invocation을 ephemeral observer/report로 허용한다. 제품 code/test/schema 변경은 0이다. observer는 `spd_adapter._layer_surface_connectivity_certificate` 호출부에서 authoritative project/source/rail_anchor_bindings/contact_seeds/landing_by_pin/reachability만 target-only bounded evidence로 읽는다. target은 3개 device branch의 6개 power/ground contact이며, primary report와 별도 sibling launcher receipt를 남긴다. original full-v4 builder/materialize/hash, compiled/raw production 또는 payload load, ownership IR build, save/load/reload, solve/Touchstone/P0-P12/fitting, candidate/ZIP/Recovery artifact 재읽기와 retry는 금지한다. 실제 runtime guard counter는 compiled-raw/ownership/externalizer에만 적용하고 나머지 금지 항목은 `not_requested_by_coordinator` 정적 제어흐름 주장으로 기록한다.
- coordinator freeze: DSU-01 `source_anchor_lean_dsu_evidence.py`는 contract `fb45d2fdb9c7e447935073e082ac557209a317f6`에서 `STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`로 영구 종료했다. DSU-02 `source_anchor_lean_dsu_evidence_v2.py`는 `STOP_PROVENANCE_INCOMPLETE`로 영구 종료했으며, coordinator 110,605 bytes/SHA-256 `b9d56f959b9a7795b6dbb83fd6e33c515c91d19c97c845f5614d31e33c792092`, report는 1,048 bytes/SHA `17fdb08c7bade28090909ec846d1041e6ecea4490ce08725e35fd3fb5b575e1c`, sibling receipt는 2,238 bytes/SHA `12e7ab7e6a8f429be7aef624bfc4c468d8d5c26575bd6d49c46d9813a37fcb16`다. sole ACTIVE DSU-03은 `D:\SPD-Decap-PI-Evaluator-W7\_coordinator_temp\source_anchor_lean_dsu_evidence_v3.py` (115,960 bytes, SHA-256 `fb6dc30adabd8c93453dcc0791bf99f2464d49b8c90c76c8ce74f9dbd1a93402`)이며 final exact-hash self-check PASS, measured `195.79 B/target vertex <=256 B`, Sol ACCEPT를 기록한다. DSU-03 contract는 `4f6e063be4b16f261eaa2fdd7ff3927dd1de92f5`의 단일 docs-only child여야 한다. 원본 SPD는 `D:\S4LB002-2Para_260729_1_injected.spd` (1,116,717,287 bytes)이며 observer source SHA는 `40cb44b2376f59d6b606eb9b4d138204fe51b2dc6b3332d3b7c0e7d4202866d2`와 exact 일치해야 한다. 정상 DSU-03 observer-reached 경로는 import/analyze/observer/report `1/1/1/1`, retry 0이고 runtime guard만 계수한다.
...
| 현재 작업 상태 | Actual-P0/R1/R2, DIAG-01과 Recovery-01은 영구 DONE/STOP했다. Recovery report는 `D:\SPD-Decap-PI-Evaluator-W7\96b0a6235b8139671c90be94dceea4a9a573f6d2\260729-r2-storage-metadata-recovery-01\r2_storage_metadata_recovery_report.json` (3,092 bytes, SHA-256 `fbd4f9d4593af4d3b58828f6ca7c17a85b4d6d1fc62c7afcd29b6b7c9c9319e3`)다. FIX-01/FIX-02는 DONE/ACCEPT다. DSU-01은 `STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`, DSU-02는 `STOP_PROVENANCE_INCOMPLETE`로 영구 종료했다. |
| 현재 ACTIVE | DSU-03 `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-03`만 ACTIVE다. |
| 현재 gate | Recovery-01은 scenario `760,816,272` bytes/SHA-256 `15115693d43bdfe69bfcf2d17faeb465da0adac640072183ee24eeea414fa9d9`의 exact descriptor(499 bytes)를 stream해 compiled-only storage를 확인했고, full surface identity `8,437,824,570` bytes/SHA-256 `1c3bfb03aa3201f47e831824e8acb1a9d95f6cfd5c2a782467e8c5019f33c17a`는 absent였다. execution PASS/scientific `STOP_FULL_CERTIFICATE_UNAVAILABLE`; Recovery candidate/validation/DIAG/central/manifest/scenario reads는 각 1, descriptor validator 1, retry 0이다. successor에서는 candidate/ZIP/Recovery artifact rereads와 full-v4/raw/ownership/save/load/solve/Touchstone/P0-P12/fitting/artifact reads를 요청하지 않으며, 정상 완료 시 runtime guard(compiled-raw/ownership/externalizer)만 0으로 확인하고 강제 종료 시 child call 값은 `not_evaluated_after_termination`으로 남긴다. topology·production 승격과 raw-v3 재구성은 금지한다. |

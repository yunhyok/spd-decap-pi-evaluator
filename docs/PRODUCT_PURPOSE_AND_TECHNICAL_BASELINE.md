...
- 현재 평가: 17DG는 standalone mathematical foundation이고, 17DR–17DS review에서 source/provenance/ownership feasibility 기준 **surface-patch plane current-spreading R/L**만 credible한 다음 물리 방향으로 남았지만 global port/owner seam 부재로 **NOT READY/STOP**이다. 17DT는 raw-spatial v3 내부 재현은 PASS했으나 W6 비교는 `STOP_IDENTITY_DRIFT`이며, 17DU local seam audit만 ACTIVE로 연다. 17DU도 local seam만 증명하고 W7은 BLOCKED 상태다.
- Sole ACTIVE item (current): **17DU**. candidate result는 **PASS_LOCAL_SEAM_ONLY** 범위일 때만 기록하며 W7은 BLOCKED, production physics/`Zii`는 unchanged다.
- 후속 권한: 17DU의 저장 candidate read-only local seam audit와 focused test만 승인하며, raw SPD/W6/Touchstone/correlation/solver/GUI/physics/profile 변경, production rerun, build/release는 여전히 미승인이다.
...
| 17DU local seam | Read-only current-lineage audit of `ADC_VDD_180_VQPS_SYS_1_AON/0` (3 PWR + 3 return Device pins) is scoped to prove anchor/contact → Via → raw owner → compiled first edge/landing vertex → raw node and target-layer pad-shape proper containment in source-bound ordered copper. The four-file whitelist is this Product/Work pair plus `scripts/audit_current_lineage_rail_footprint.py` and `tests/test_audit_current_lineage_rail_footprint.py`; `accuracy_parse.py` remains preserved and untouched. Success is `PASS_LOCAL_SEAM_ONLY`; W7 remains BLOCKED because old-plane owner-off/global assembly/W6 causal comparison/Zii improvement are not proven. |
...
| 현재 권한 | `ACTIVE 17DU` only for one read-only local seam audit and focused test; no raw SPD/W6/Touchstone/correlation/solver/GUI/physics/profile/build/release execution. |

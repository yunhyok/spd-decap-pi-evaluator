...
and no production W6/accuracy claim. Sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-V3-PRODUCTION-IMPORT-CALLER-DISCOVERY` (17AO), authorizing exactly once the
frozen query `rg -n --with-filename --no-heading --color never --glob '*.py' '\bimport_spd_scenario\b' 'src/spd_decap_pi'`.

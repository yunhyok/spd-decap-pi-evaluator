...
- Sole ACTIVE item (current): **NONE**. 17DU는 DONE/STOP 상태이며 production physics/`Zii`는 unchanged다.
- 후속 권한: 17DU one-shot closure를 보존하며 새 work는 아직 승인하지 않는다. raw SPD/W6/Touchstone/correlation/solver/GUI/physics/profile 변경, production rerun, build/release는 미승인이다.
...
| 현재 권한 | `ACTIVE NONE`; 17DU is a preserved DONE/STOP one-shot contract. No new candidate read, raw SPD/W6/Touchstone/correlation/solver/GUI/physics/profile/build/release execution is authorized. |

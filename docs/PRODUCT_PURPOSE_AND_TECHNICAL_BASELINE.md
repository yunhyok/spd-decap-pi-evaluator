...
- 현재 권위 상태: sole ACTIVE 17CN `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-RECORD-KEY-CONSTRUCTION-DISCOVERY`; exact once-only records/key query in compiler.py lines770-801, gate exit0 2-8 unique lines/1 file, one records iteration and key assignment in one cluster span<=24; missing/loop/assignment/generic/duplicate/multiple/span/cap STOP/no retry. Location-only; no source read/requery/expansion, islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.
...
17CM is BLOCKED/location STOP at clean HEAD `45be04069c3723627bcd9e61cc75e2d0deb98cda`: the exact record_by_key query ran once with exit0, 4 unique lines/1 file, span18 (init784, guard789, population792, lookup802), but records iteration and key construction were absent. No retry or body read occurred. Sole ACTIVE successor is 17CN, a source-record key-construction discovery query.
17CN authorizes only this exact once-only query:
...
Gate: exit0, 2-8 unique lines/1 file all770-801; exactly one actual records iteration and one key assignment must form one cluster span<=24. Missing loop/assignment, generic/duplicate/multiple clusters, span/cap STOP/no retry. Location-only; no body read.

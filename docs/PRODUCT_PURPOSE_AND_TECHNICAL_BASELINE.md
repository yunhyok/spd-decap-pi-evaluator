...
- 현재 권위 상태: sole ACTIVE 17BR `W7-PHYS-W6-RAW-SURFACE-PRODUCER-BOUNDARY-DISCOVERY`, location evidence only. Exact once-only PowerShell definition query must exit0 with exactly 2 lines/1 file, first top-level boundary <=2575 and next >2575, inclusive span <=160; otherwise STOP/no retry. PASS only authorizes a later separately approved full-function read for source/artwork SHA and net/layer normalization/validation. No source open/code/test/physical ownership/stamp/production/accuracy execution.
...
Sole ACTIVE successor is 17BR, a compiler-owner boundary query; no source open or ownership claim follows.

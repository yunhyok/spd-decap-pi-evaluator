...
source-classified BLOCKED; that proves only filename absence. Sole ACTIVE 17Z
...
Budget is <=40 lines and <=6 files; over-budget fails closed without a second query/open. If exactly
one file references the compiler symbol, Sol selects it; builder/validator-only or ambiguous results
remain BLOCKED. If zero compiler references are found, the minimal new test path
`tests/test_raw_spatial_contact_compiler.py` is the only defensible candidate because 17Y found no
conflicting filename; no further search is allowed. The proposed production successor
`W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION` remains NOT ACTIVE until this mapping is documented.

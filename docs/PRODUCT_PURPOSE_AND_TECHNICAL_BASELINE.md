...
- 현재 권위 상태: sole ACTIVE 17CG `W7-PHYS-W6-RAW-SURFACE-LAYER-PENDING-CONTROL-BODY-TRACE`; exact once-only numbered read of compiler.py lines941-1037 (97 lines), with frozen continue boundaries at940 and1038. PASS requires complete two-cluster control flow, pending create/update/consume, and layer_match create/branch binding; mid-statement/branch, omitted mutation, or external helper semantics STOP/no extension/no retry. Location/body trace only; decode/SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release remain excluded.
...
17CG authorizes only the exact once-only numbered read of `raw_spatial_contact_compiler.py` lines941-1037 (97 lines);
the frozen predecessor line940 and successor line1038 are continue boundaries already read in17CD and are not reread.
...
extension/no retry. Decode/SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/
accuracy/release remain excluded.

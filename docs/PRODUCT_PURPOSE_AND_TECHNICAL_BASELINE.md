...
- Sole ACTIVE item (current): **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION**. The capture fix closed the two read-only wrappers and the sole run reached the intended fail-closed Node-layer `SpdImportError`, but the same first test's separate `tamper_node` connection remained open and WinError 32 masked that expected exception. The successor closes only that last test connection before one fresh fail-fast 18-node run. Improvement remains 0.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. Luna는 `tests/test_source_plane_ownership_ir_producer.py`의 남은 `with sqlite3.connect(evidence.spool.path) as connection:` 한 문장만 기존 stdlib `closing(...)`으로 감싼다. Sol static review 뒤 [작업 기준 12.59](WORK_EXECUTION_BASELINE.md#1259-w7-phys-owner-join-ownership-streamed-ir-streamed-handoff-tamper-connection-fix-01--frozen--ready_for_implementation)의 정확한 18개 node를 원인 노드 우선 순서로 `-x -vv --tb=long` Python312 invocation 1회(외부 300 s 상한) 실행한다. 모든 product/source IR/other test bytes, schema/caps, pressure/exact-300K, original SPD/full suite/solver/production/PowerSI/release는 금지한다.
...
| 현재 ACTIVE | **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION** |
| 현재 gate | producer test의 마지막 `tamper_node` SQLite connection 한 곳을 명시적으로 close한 뒤 원인 노드 우선 18-node lightweight scope를 단일 300 s invocation으로 확인; product/schema/caps/other tests와 accepted pressure/exact-300K 불변 |
...
## 35. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 — FROZEN / READY_FOR_IMPLEMENTATION

...
provenance remains external. Sole ACTIVE successor is 17CL, a source-record origin discovery query.

17CL authorizes only the exact once-only record/source origin query in `raw_spatial_contact_compiler.py` lines700-861;
gate exit0, 2-24 unique lines/1 file, record container/iteration plus asset/asset_sha256 derivation and line818 transfer
in one clear cluster span<=120. Missing/generic/multiple/span/cap STOP/no retry. Location-only; no source read/requery/expansion.

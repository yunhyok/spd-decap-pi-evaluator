...
- Sole ACTIVE item (current): **`W7-PHYS-R2-MULTI-TOPOLOGY-DIAG-01`** — R2가 확정한 multiple candidate가 valid one-to-many topology인지 partition/provenance defect인지 frozen 17DT artifact에서 한 번만 판정한다. full certificate가 없으면 재구성하지 않고 STOP한다. original SPD, production wiring/broadband와 P12는 열지 않는다.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. P12 prospective review는 NO-GO다. 허용 범위는 세 기준 문서와 frozen 17DT candidate/validation report의 artifact-only one-shot, 새 bounded report root뿐이다. Actual-P0/R1/R2 재실행·부분 재사용, original SPD, 코드/test/schema 수정, raw-v3 단독 재구성, first-match/dedupe/fallback, solve/Touchstone/P0-P12, production wiring, broadband, Distribution, fitting·comparison과 release는 금지한다.
...
| 현재 작업 상태 | Actual-P0/R1/R2는 scenario 없이 영구 DONE/STOP했다. FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-R2-MULTI-TOPOLOGY-DIAG-01`만 ACTIVE다. |
| 현재 gate | R2 report(23,894 bytes, SHA-256 `b55ca2fa…44fe8`)는 import/save/load `1/0/0`과 exact multiple STOP을 확정했다. 다음 gate는 frozen 17DT artifact의 certificate 저장 형태와 exact candidate/component/reachability/owner provenance만 읽는다. compiled-only이면 `STOP_FULL_CERTIFICATE_UNAVAILABLE`이며 raw-v3 재구성은 금지한다. |

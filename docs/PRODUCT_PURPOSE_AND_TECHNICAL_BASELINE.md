...
- 현재 권위 상태: sole ACTIVE 17BT `W7-PHYS-W6-RAW-SURFACE-FIELD-ORIGIN-DISCOVERY`, exact once-only PowerShell hit query restricted to compiler.py lines2363-2549; gate exit0, 1-20 unique lines, exact1 file, one clear cluster locating net/layer/asset_sha/islands creation and source_hashes binding, otherwise STOP/no retry. PASS only authorizes a later <=120-line bounded read; `_canonical_sha`, islands semantics, and ownership remain separate. No source expansion/code/test/physical ownership/stamp/production/accuracy execution beyond this bounded query.
...
source hash before2550 plus `_canonical_sha` delegated; no expansion/retry. Sole ACTIVE successor is 17BT,
a bounded field-origin discovery query; no islands/electrical ownership/stamp/accuracy claim follows.

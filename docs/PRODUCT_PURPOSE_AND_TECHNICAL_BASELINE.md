...
17CX is the sole ACTIVE docs-only frozen-evidence judgment. It requires direct binding of primitive/source and surface asset fields, attachment names and exact-byte SHA domain, net/layer casefold keys, and cross-row `(asset_sha, casefold(net), casefold(layer))` equality across the frozen evidence. Any inference yields BLOCKED/design REJECT; no new reads or implementation are authorized.

...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); Actual-P0/R1/R2와 R2-MULTI-TOPOLOGY-DIAG-01 및 Recovery-01은 DONE/STOP, FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`은 DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  ...
  W7 production physics는 BLOCKED이고 sole ACTIVE는 **NONE**이다.
...
- Sole ACTIVE item (current): **NONE** — DSU-05의 선택된 6-contact source boundary에서 복수 selector 후보가 재현되지 않았고 source partition/finite reachability 결함도 관찰되지 않았다. DSU-06, FIX-03, P12, production wiring과 원본 SPD 재실행을 열지 않는다.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. DSU-05 one-shot은 종료됐고 제품 code/test/schema 변경은 0이다. 같은 원본 SPD 실행, DSU-06, FIX-03, P12, production wiring, solve/Touchstone/fitting과 release 승격은 금지한다. 다음 물리 작업은 새로 식별된 단일 admissible causal block과 frozen 검증 계약이 있을 때만 별도 gate로 연다.
...
| 현재 ACTIVE | **NONE** |
| 현재 gate | 선택된 3개 device branch/6개 contact에서 source-boundary candidate가 모두 singleton이고 all/complete graph component가 동일했다. R2 multiple-selector 가설은 재현되지 않았으므로 DSU-06/FIX-03/P12/production wiring을 열지 않는다. 새 작업은 PowerSI 오차를 설명할 별도의 source-derived causal block과 독립 frozen gate가 제시될 때만 재개한다. |

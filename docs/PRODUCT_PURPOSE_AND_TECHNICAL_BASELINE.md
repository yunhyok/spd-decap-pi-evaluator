...
- 현재 권위 상태: sole ACTIVE 17CC `W7-PHYS-W6-RAW-SURFACE-PARSE-LAYERS-MAP-MUTATION-DISCOVERY`; exact once-only mutation query over compiler.py for rows/by_fold initialization, insertion/update, `RawSpatialLayerRow`, and final return; gate exit0, 4-24 unique lines/1 file all872-1067, all mutation clusters present and a <=120-line bounded/split successor permitted, otherwise STOP/no retry. Decode/SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release remain excluded.
...
constructor918 and return1065 present but casefold meanings multiple; no body read. Sole ACTIVE successor is 17CC,
a map-mutation discovery query.

17CC authorizes only the exact once-only mutation query over `raw_spatial_contact_compiler.py` for rows/by_fold
initialization, insertion/update, `RawSpatialLayerRow`, and final return; gate exit0, 4-24 unique lines/1 file
all872-1067, all mutation clusters present, otherwise STOP/no retry. PASS only authorizes one later <=120-line
bounded/split read. Decode/SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/

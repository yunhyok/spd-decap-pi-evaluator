...
- 현재 권위 상태: sole ACTIVE 17BW `W7-PHYS-W6-RAW-SURFACE-KEY-DOMAIN-LOCAL-TRACE`, exact once-only numbered read restricted to compiler.py lines2388-2435 (48 lines); require complete RHS/uses of both key assignments, direct net/layer/asset SHA origins, casefold tuple domain and relation2396↔2425 in range. STOP/no expansion/no retry if endpoints or semantics leave range; source hash helper, canonical SHA, islands, ownership/stamp/accuracy remain separate.
...
Sole ACTIVE successor is 17BW, a bounded key-domain local trace.

...
is location evidence only with no file open or inference beyond placement. Sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-V3-COMPILER-MERGE-CALLSITE-TRACE` (17AG), limited to exactly one read of
`src/spd_decap_pi/spd_adapter.py` lines 7848-7885 (38 lines). Questions are immediate guard/provenance
setup; complete compiler assignment/call, arguments/flags/returned tuple; complete merge assignment/
call, arguments/results; locally adjacent downstream use/return; and whether v3 manifest/generated is
structurally passed to merge. Endpoint truncation => source-classified STOP without extension. No
whole-file/search/second range/code/test/profile/solver/accuracy/production proof.

...
- 현재 권위 상태: sole ACTIVE 17CI `W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-CONTRACT-TRACE`; exact once-only numbered read of compiler.py lines402-414 (13 lines). PASS requires full `_decode_token` signature/return/error/whitespace/encoding/normalization and returned token in the same string domain for later casefold mapping; delegated helper/out-of-range/truncation STOP/no expansion/retry. Body trace only; SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release remain excluded.
...
17CI authorizes only the exact once-only numbered read of `raw_spatial_contact_compiler.py` lines402-414 (13 lines).
PASS requires full signature/return/error/whitespace/encoding/normalization and returned token in the same string domain
for later casefold mapping; delegated helper/out-of-range/truncation STOP/no expansion/retry. Body trace only; SHA/source
hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release claim excluded.

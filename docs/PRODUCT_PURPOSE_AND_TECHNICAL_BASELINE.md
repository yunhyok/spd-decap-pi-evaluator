...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); Actual-P0/R1/R2와 R2-MULTI-TOPOLOGY-DIAG-01 및 Recovery-01은 DONE/STOP, FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`은 DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  ...
  W7 production physics는 BLOCKED이고 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-02** (FROZEN / READY_TO_EXECUTE)다. W7-PHYS-OWNER-JOIN-QUOTIENT-SCOPE-FIX-01은 DONE/ACCEPT로 닫혔고, 이전 -01 gate는 DONE/STOP_IMPORT_OR_OWNERSHIP로 영구 닫혔다.
...
- Sole ACTIVE item (current): **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-02** — **FROZEN / READY_TO_EXECUTE**. FIX-01 is DONE/ACCEPT at `1765731b411bd2a3c0a4a425b8e4c74232c6b289`; old -01 gate is permanent DONE/STOP retry 0/no rerun. Frozen coordinator `D:\SPD-Decap-PI-Evaluator-W7\_coordinator_temp\production_owner_join_original_spd_evidence_02.py` is 52,836 bytes, SHA-256 `3fe3b6057ae51986ddf83d98cac5222cb7a538d8e850e45427c488b5acde0c98`; exact-six-line static ACCEPT, source compile exactly once exit 0 (`SOURCE_COMPILE PASS`), self-check exactly once exit 0 with matching SHA, Sol `EXECUTION_ACCEPT_FOR_DOCS_FREEZE`, P0/P1 none. Base/source-before is `1765731b411bd2a3c0a4a425b8e4c74232c6b289`; execution contract is the new docs-only direct child and its runtime hash is not pre-stated. Original SPD input/rail/frequency/geometry is unchanged; output root/report/receipt are new -02 names. No production owner-off/wiring/solve/`Y_global`/`Zii`/PowerSI promotion; improvement remains 0.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. -02 coordinator/docs freeze가 완료되어 정확한 parent one-shot 1회만 허용한다. Input source/rail/frequency/cell, 7200 s timeout, report/receipt bounds와 claim ceiling을 유지하며 retry 0/no patch/reuse/rerun이다. Timeout/interrupt 미확정은 STARTED receipt와 `not_evaluated_after_termination`을 보존한다. Production owner-off/wiring/solve/`Y_global`/`Zii`/PowerSI 승격과 accuracy claim은 금지한다.
...
| 현재 ACTIVE | **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-02** — **FROZEN / READY_TO_EXECUTE**; coordinator 52,836 bytes/SHA `3fe3b6057ae51986ddf83d98cac5222cb7a538d8e850e45427c488b5acde0c98`, Sol `EXECUTION_ACCEPT_FOR_DOCS_FREEZE` |
| 현재 gate | **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-02** — exact parent one-shot 1회, retry 0; output root/report/receipt are new -02 names and no production/PowerSI promotion is allowed. |
...
## 17. W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-02 — FROZEN / READY_TO_EXECUTE

Base/source-before is `1765731b411bd2a3c0a4a425b8e4c74232c6b289`; the execution contract is the new docs-only direct child
changing exactly these three canonical docs (Product, Source-derived physical IR, and Work execution baseline), and its
actual commit hash is supplied at runtime, not invented here. Frozen coordinator
`D:\SPD-Decap-PI-Evaluator-W7\_coordinator_temp\production_owner_join_original_spd_evidence_02.py` is 52,836 bytes,
SHA-256 `3fe3b6057ae51986ddf83d98cac5222cb7a538d8e850e45427c488b5acde0c98`; exact six-line static ACCEPT, source
compile exactly once exit 0 (`SOURCE_COMPILE PASS`), self-check exactly once exit 0 with matching SHA, Sol
`EXECUTION_ACCEPT_FOR_DOCS_FREEZE`, P0/P1 none. Compile/self-check are consumed and must not rerun.

Fixed input is `D:\S4LB002-2Para_260729_1_injected.spd` (1,116,717,287 B, expected SHA-256
`40cb44b2376f59d6b606eb9b4d138204fe51b2dc6b3332d3b7c0e7d4202866d2`), rail
`ADC_VDD_180_VQPS_SYS_1_AON/0`, frequency 1e9 Hz, cell 1000.0 um, timeout 7200 s. The parent launches exactly once,
retry 0, with no patch/reuse/rerun. Root is
`D:\SPD-Decap-PI-Evaluator-W7\<contract>\260729-production-owner-join-original-spd-evidence-02`; report is
`production_owner_join_original_spd_evidence_02_report.json`; sibling receipt is
`260729-production-owner-join-original-spd-evidence-02.launcher-receipt.json`. Root/report/receipt are absent before
launch; report and receipt bounds and claim ceiling remain unchanged. Timeout/interrupt uncertainty preserves STARTED
receipt and calls=`not_evaluated_after_termination`.

The exact parent command is:
...
Only PASS/STOP evidence-gate outcomes are allowed. No production owner-off/wiring/solve/`Y_global`/`Zii`/PowerSI
promotion or accuracy claim is made; improvement remains **0**. The old -01 gate is permanent DONE/STOP and never rerun.

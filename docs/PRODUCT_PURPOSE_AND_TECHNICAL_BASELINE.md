...
17S is the sole ACTIVE read-only contract trace at clean main HEAD
`78cd240dff6b704595574d888da4b9c089ae39ab`. Its exact whitelist is
`src/spd_decap_pi/_core/services.py`,
`src/spd_decap_pi/_core/solver/surface_certificate_asset.py`, and
`src/spd_decap_pi/_core/solver/layer_surface_termination.py`, each to be read completely once.
The trace must jointly bind profile construction/application and no fallback, hash-bound
solve-ready polygon/material/dielectric inputs without raw-SPD/full-Scenario hydration, balanced
`N^H B=0` gauge/Kron/nullspace with stable rail/Zii, old Maxwell C/ideal disable with Via/
termination exactly once, and worker/cache fencing. Any external delegate or missing group,
including the unknown `compile_raw_spatial_contact_asset` module, closes 17S as
source-classified BLOCKED; no fourth read, automatic whitelist expansion, code, test, artifact,
or production execution is authorized.

...
- 현재 권위 상태: 17T는 historical source-classified BLOCKED; 17U는 compiler trust/resource/determinism는 proven했지만 payload assembly/certificate envelope 및 required completeness가 delegated/unproven으로 BLOCKED; `W7-PHYS-W6-RAW-SPATIAL-ASSET-CERTIFICATE-CONTRACT-TRACE` 17V가 사용자의 standing preapproval 아래 유일한 ACTIVE read-only trace다.
...
solve-time payload are delegated/unproven; 17U is source-classified BLOCKED. The user then activated

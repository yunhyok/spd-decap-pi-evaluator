...
compiler/include flag matches. Location evidence only; no test owner selected. Sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DISCOVERY` (17AI), with the exact frozen
PowerShell gate in the Work document: run once, exactly 4 lines/1 file, bracket targets 1750 and
2487 with top-level test definitions, and use only the frozen 17AH call list for interval membership;
select at most one owner only when exactly one bracket contains an observed non-import call, otherwise
zero/both/ambiguous/overlap => STOP. No file open, code, test, or inference.

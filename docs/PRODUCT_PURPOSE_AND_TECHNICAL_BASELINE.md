...
no files opened and no compiler/delegate inference. Sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-V3-MERGE-OWNER-BOUNDARY-DISCOVERY` (17AE), limited to the exact fenced
PowerShell gate in the Work document: `--with-filename` is mandatory, output must be exactly 2 lines/
1 file, first line <=7868 and second >7868, otherwise STOP. The output is only top-level owner/
signature boundary location evidence; no implementation read, file open, code/test/profile/accuracy/
production claim, or automatic contiguous read is authorized.

...
Sole ACTIVE successor 17CV `W7-PHYS-W6-PLANE-PRIMITIVE-ATTACHMENT-DIGEST-CACHE-OWNERSHIP-DISCOVERY` authorizes exactly one cache-ownership location query:
...
Gate: exact once, exit0, 3–12 unique lines from exactly 1 file; one local empty-map initialization, line864 get, and line867 write must form one cluster span<=80. Parameter/import/global/nonempty initialization, alias assignment, additional writer/mutation, multiple cluster, comments/strings-only, or cap/span failure stops with no retry. Location-only; no body read. Asset-name meaning, islands, ownership, stamps, profile, replacement, nullspace, Zii, production, accuracy, and release remain out of scope.

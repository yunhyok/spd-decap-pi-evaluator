...
Sol design ACCEPT. Exact whitelist: `scripts/validate_island_finite_via_gate.py`, `tests/test_validate_island_finite_via_gate.py`. Reject `distribution._distribution_plane_geometries`; use `core_services.spd_plane_geometry_record_payload()` per persisted record fail-closed, existing `SimpleNamespace`, and a context manager for `_retained_surface_artwork(project, attachments, plane_geometries, ...)` with `finally close()`. Map lifecycle fields and production callbacks exactly. Focused node only after Sol static ACCEPT; no production run. Future 17DM audit is NOT ACTIVE until 17DL PASS/clean commit/new docs/Usage Guard/new root.

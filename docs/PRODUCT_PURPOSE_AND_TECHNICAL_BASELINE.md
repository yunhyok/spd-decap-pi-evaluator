...
- 현재 평가: 17DG는 standalone mathematical foundation이고, 17DR–17DS review에서 source/provenance/ownership feasibility 기준 **surface-patch plane current-spreading R/L**만 credible한 다음 물리 방향으로 남았지만 global port/owner seam 부재로 **NOT READY/STOP**이다. 17DT는 원본 SPD의 source plane/stackup/material 데이터를 normalized/hash-bound raw-spatial v3 attachment로 재현하는 import-save-only 작업이며 physics/profile/GUI를 바꾸지 않는다. 17DK–17DP는 validation/metadata only이며 production `Y_global` 또는 `Zii`를 변경하지 않았다.
- Sole ACTIVE item (current): **17DT**. 17DS candidate gate는 **NOT READY/STOP**이고 W7은 BLOCKED 상태다.
- 후속 권한: 17DT의 fresh import-save-only v3 generation 및 focused evidence만 승인하며, artifact-only ablation, owner-manifest/schema, physics/profile 변경, production rerun, release는 여전히 미승인이다.
...
| 17DT activation | Raw SPD contains source plane/stackup/material data. The W6 frozen candidate lacks the newer normalized/hash-bound raw-spatial v3 tables. Active scope is one fresh `import-save-only` reproduction using source `D:\S4LB002-2Para_260729_1_injected.spd` (1,116,717,287 bytes; SHA-256 `40cb44b2376f59d6b606eb9b4d138204fe51b2dc6b3332d3b7c0e7d4202866d2`) to planned output `D:\SPD-Decap-PI-Evaluator-W7\<technical_commit>\260729-17dt-raw-spatial-v3`; generation must persist and validate v3 manifest/table/count/hash/full-loader evidence. The four-file whitelist is this Product/Work pair plus `scripts/benchmark_raw_spd_powersi_correlation.py` and `tests/test_benchmark_raw_spd_powersi_correlation.py`; untracked `accuracy_parse.py` is explicitly preserved and untouched. This changes only the runner/test seam and raw-SPD evidence path: no `src`/GUI/physics/profile/frequency solve/Touchstone/correlation/build/release. This does not select a physical owner or change `Y_global`/`Zii`; exact rail footprint join and old-plane owner-off remain a later bounded gate. Sole ACTIVE **17DT**. |
...
| 현재 권한 | `ACTIVE 17DT` only for the fresh import-save-only v3 reproduction/evidence scope; only the runner/test seam and raw-SPD evidence path are allowed, with no `src`/GUI/physics/profile/frequency solve/Touchstone/correlation/build/release. |

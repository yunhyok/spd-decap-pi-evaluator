...
17CY `W7-PHYS-W6-PLANE-GEOMETRIES-CANONICAL-PRODUCER-DISCOVERY` is the sole ACTIVE location-only query using the canonical exact fenced command in Work §17CY; gate is exit0, 1–12 unique lines across at most 3 files, with exactly one actual `spd_import.plane_geometries` producer/normalizer required. Consumers, annotations, zero/multiple/dynamic/ambiguous hits or caps STOP without retry/read. No asset/electrical/production/accuracy inference.

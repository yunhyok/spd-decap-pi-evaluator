...
- Sole ACTIVE item (current): **`W7-PHYS-R2-STORAGE-METADATA-RECOVERY-01`** — 실패한 DIAG-01을 재실행하지 않고 frozen ZIP의 exact JSON path에서 certificate storage descriptor 하나만 bounded stream으로 복구한다. storage availability만 판정하며 topology 의미, original SPD, production wiring/broadband와 P12는 열지 않는다.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. P12 prospective review는 NO-GO다. 허용 범위는 세 기준 문서와 frozen candidate/validation report/DIAG-01 failure report의 metadata-stream one-shot, 새 bounded report root뿐이다. `load_scenario_bundle`, hydrate/raw load, Actual-P0/R1/R2/DIAG-01 재실행, original SPD, 코드/test/schema 수정, raw-v3 재구성, solve/Touchstone/P0-P12, production wiring, broadband, Distribution, fitting·comparison과 release는 금지한다.
...
| 현재 작업 상태 | Actual-P0/R1/R2와 DIAG-01은 영구 DONE/STOP했다. FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-R2-STORAGE-METADATA-RECOVERY-01`만 ACTIVE다. |
| 현재 gate | DIAG-01은 bundle load 뒤 report timestamp의 `ZoneInfoNotFoundError`로 storage 결과를 잃었다. report는 3,283 bytes/SHA-256 `725ec8b4…a53c`다. successor는 full load 없이 exact descriptor만 stream한다. compiled-only이면 `STOP_FULL_CERTIFICATE_UNAVAILABLE`; raw-v3 재구성은 금지한다. |

...
- 현재 권위 상태: sole ACTIVE 17BV `W7-PHYS-W6-RAW-SURFACE-KEY-ORIGIN-DISCOVERY`, exact once-only PowerShell assignment query restricted to compiler.py lines2363-2459; gate exit0, 4-16 unique lines, exact1 file, actual assignments for net/layer/asset_sha/key in one clear cluster, otherwise STOP/no retry. PASS only authorizes a later <=100-line bounded read; helpers, canonical SHA, islands, and ownership remain separate. No physical stamp/accuracy execution.
...
outside the range; no expansion/retry. Sole ACTIVE successor is 17BV, a bounded key-origin query.

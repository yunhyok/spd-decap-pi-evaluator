...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); Actual-P0/R1/R2와 R2-MULTI-TOPOLOGY-DIAG-01 및 Recovery-01은 DONE/STOP, FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`은 DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  DSU-02/DSU-03/DSU-04는 각각 `STOP_PROVENANCE_INCOMPLETE`로 영구 종료됐다.
  `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-05`만 ACTIVE이며 W7 production physics는 아직 BLOCKED다.
...
- Sole ACTIVE item (current): **`W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-05`** — DSU-01은 `STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`, DSU-02/DSU-03/DSU-04는 `STOP_PROVENANCE_INCOMPLETE`로 영구 종료했다. DSU-05 path-aware lean producer-time observer만 진단 목적으로 유지하며 production wiring, topology 승격과 P12는 열지 않는다.
...
- coordinator freeze: sole ACTIVE DSU-05는 `D:\SPD-Decap-PI-Evaluator-W7\_coordinator_temp\source_anchor_lean_dsu_evidence_v5.py` (119,869 bytes, SHA-256 `601e7404f85d4ce64c43203a1af94b28144c82e516651f6a241d75b3f6c8d9bb`)이며 in-memory compile/self-check 각 1회 PASS, 195.51 bytes/target vertex와 Sol final ACCEPT를 기록한다. DSU-05 contract는 `28845c0bd718d61fb611414005d1b325b19a9eca`의 단일 docs-only child여야 한다. direct first-edge와 trace `source-node:*` identity를 분리하고 optional immediate row는 형식만 검사하며, remote selected L30/L29 proof/projection과 기존 3V+2E graph는 바꾸지 않는다.
- 원본 SPD는 `D:\S4LB002-2Para_260729_1_injected.spd` (1,116,717,287 bytes)이며 observer source SHA는 `40cb44b2376f59d6b606eb9b4d138204fe51b2dc6b3332d3b7c0e7d4202866d2`와 exact 일치해야 한다. 정상 DSU-05 observer-reached 경로는 import/analyze/observer/report `1/1/1/1`, retry 0이고 runtime guard만 계수한다.

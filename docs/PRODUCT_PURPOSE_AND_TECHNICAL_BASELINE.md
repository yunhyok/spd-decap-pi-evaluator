...
Sole ACTIVE successor is `W7-PHYS-W6-RAW-SPATIAL-V3-ADAPTER-TEST-OWNER-BOUNDARY-DIAGNOSTIC` (17AJ),
with the exact frozen diagnostic PowerShell gate in the Work document: success exit0, exactly 4 lines
for one expected file, complete top-level test-definition scan, and final EOF sentinel only when no
later test definition exists; any STOP/nonzero/count/path mismatch => STOP. Use only frozen 17AH
non-import call lines for interval membership; zero/both/overlap/ambiguity => STOP. No file open,
code, test, or inference.

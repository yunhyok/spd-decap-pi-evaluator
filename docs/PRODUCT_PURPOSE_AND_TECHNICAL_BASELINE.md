...
- 현재 권위 상태: sole ACTIVE 17CB `W7-PHYS-W6-RAW-SURFACE-PARSE-LAYERS-NORMALIZATION-CLUSTER-DISCOVERY`; exact once-only query over compiler.py for `RawSpatialLayerRow`, `.casefold()`, and return lines, exit0 with 3-24 unique lines/1 file all within872-1067; construction, casefold normalization, and return/map binding must form one clear flow, otherwise STOP/no retry. PASS only authorizes a later <=120-line bounded/split read; decode2292-2362 remains separate frozen candidate, and source hash/canonical SHA/islands/ownership/stamp/profile/physical/accuracy remain separate.
...
next header_material1068, span196; no body/context read. Sole ACTIVE successor is 17CB, a normalization-cluster
discovery query.

17CB authorizes only the exact once-only query over `raw_spatial_contact_compiler.py` for `RawSpatialLayerRow`,
`.casefold()`, and return lines; gate exit0, 3-24 unique lines/1 file all872-1067, with construction, casefold
normalization, and return/map binding in one clear flow, otherwise STOP/no retry. PASS only authorizes one later
<=120-line bounded/split read. Decode2292-2362 remains a separate frozen candidate; source hash helper, canonical
SHA, islands, ownership, stamp, profile, replacement, nullspace/Zii, production, PowerSI accuracy, and release
remain excluded.

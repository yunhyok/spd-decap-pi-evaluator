...
- 현재 권위 상태: 17T는 historical BLOCKED; 17U/17V는 source-classified BLOCKED (17U compiler trust/resource/determinism proven; 17V structural schema/hash/bounds proven but full polygons/material/dielectric payload absent); `W7-PHYS-W6-RAW-PLANE-MATERIAL-SOURCE-DISCOVERY` 17W가 standing preapproval 아래 유일한 ACTIVE read-only query다.
...
and a solve-ready payload remain absent, so 17V is source-classified BLOCKED. The user activated
`W7-PHYS-W6-RAW-PLANE-MATERIAL-SOURCE-DISCOVERY` (17W) under standing preapproval at clean main HEAD
`bf0dc6be265eb3375a64f510d5e51d98cd199074`. It is the sole ACTIVE one-query read-only trace. Its
exact query must find exactly one `SpdAnalysis` definition and one `ProjectSpec` definition in no more
than two files; missing/duplicate/nested/non-class aliases or over-cap results fail closed. Future
owner files are limited to one or two unread definitions, each read once. No payload/accuracy/causal
or production claim follows from a passing definition query. Conditional storage direction only: if
the owner trace passes, version/extend the existing raw-spatial asset to schema v3 with its SQLite/
hash/resource helpers; use no second attachment or new physics solver, make a new opt-in profile reject
v2/missing physical tables fail-closed, and leave the historical path unchanged.

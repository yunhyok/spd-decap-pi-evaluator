...
production. The uncommitted technical diff remains exactly
`src/spd_decap_pi/raw_spatial_contact_asset.py`, `src/spd_decap_pi/raw_spatial_contact_compiler.py`,
and `tests/test_raw_spatial_contact_compiler.py` at clean-main base
`4af8ae120469be2fc3bf99ff30f07b1a5e961565`. The sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S1` is BLOCKED at its test gate. The approved command
ran once: exit 1, `1 failed in 1.51s`; `raw_spatial_contact_asset.py::_plane_sheet_rows` (line 1389)
raised `KeyError: 0` from direct `circles_by[row["primitive_ordinal"]]` lookup for a polygon with
no circle rows. This is source-classified as an asset grouping bug; no rerun. The retained
uncommitted technical diff remains the same three paths. Sole ACTIVE successor S2 is
`W7-PHYS-W6-RAW-SPATIAL-ASSET-V3-EMISSION-S2`, limited to `src/spd_decap_pi/raw_spatial_contact_asset.py`;
use `.get(primitive_ordinal, [])` for both grouped maps, preserving the existing union check and
`_validate_plane_primitive` authority. Sol reviews before the same focused node once; failure
stops. No production, accuracy, or version claim follows.
...
Under standing W6 reimplementation authority, after 17Z either selects the uniquely mapped existing
compiler test or freezes the named new `tests/test_raw_spatial_contact_compiler.py` path, the proposed
v3 successor may be activated; it is NOT ACTIVE now. If activated, it may version/extend the existing raw-spatial

...
- 현재 권위 상태: sole ACTIVE 17CQ `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-DIGEST-VALIDATOR-BOUNDARY-DISCOVERY`; exact once-only definition-boundary query targets `_digest_hex` and its immediate next top-level definition in compiler.py; gate exit0, exactly 2 lines/1 file, positive span<=80, otherwise STOP/no retry. Location-only; no source body read/expansion/requery, islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.
...
17CP is BLOCKED/source-classified STOP at clean HEAD `652ee17f39b34a2c6f5ba93cb27cfe5e6944fd42`: the exact 16-line body read ran once with exit0 and proved missing/non-Mapping lookup and non-str asset/SHA failure, primitive-order validation, and unchanged transfer in complete line818; str-only values allow empty/nonhex/non64 SHA, so format validation was absent. No expansion or retry occurred. Sole ACTIVE successor is 17CQ, an asset-SHA digest validator-boundary discovery query.
17CQ authorizes one exact definition-boundary query:
...
17CQ is location-only; gate exit0 with exactly 2 lines/1 file, unique `_digest_hex` definition and immediate next top-level boundary, positive span <=80. Missing/duplicate/parse/path/order/over-span => BLOCKED/STOP; no body read or retry. Asset-name meaning/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.

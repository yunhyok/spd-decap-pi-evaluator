...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); Actual-P0/R1/R2와 R2-MULTI-TOPOLOGY-DIAG-01 및 Recovery-01은 DONE/STOP, FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`은 DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  ...
  W7 production physics는 BLOCKED이고 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-EVIDENCE-01**이다.
...
- Sole ACTIVE item (current): **W7-PHYS-OWNER-JOIN-EVIDENCE-01** — global contact의 복수 component 후보는 서로 다른 required rail layer에 걸쳐 유효할 수 있어 exact-one은 rail별 속성이다. ownership-private selector의 global singular identity 요구를 Phase A에서 expected `(net, layer)` projection exact-one으로 교정하고 Phase B에서 hash-bound source-to-production owner join을 read-only로 관찰한다.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. 이 gate의 변경은 Phase A/B에 명시한 두 producer/consumer 파일과 두 focused test 및 세 기준 문서로 제한하며 physics/solver/`Y_global`/`Zii`는 변경하지 않는다. 원본 SPD, scenario save/load, numerical solve, Touchstone, P12, owner-off production wiring, fitting, release/build/version bump는 금지한다. PowerSI numerical improvement는 0이다.
...
- DSU-05 count/SHA-only evidence, compiled-only Recovery와 full certificate/ownership IR 부재로 exact join key가 없으므로 이 문서 개정에서 W7-PHYS-OWNER-JOIN-EVIDENCE-01을 그 evidence를 만드는 유일한 ACTIVE gate로 연다. DSU-06/FIX-03/P12/production/rerun은 여전히 금지하며 PowerSI numerical improvement는 0이다.
...
| 현재 ACTIVE | **W7-PHYS-OWNER-JOIN-EVIDENCE-01** |
| 현재 gate | Global contact 복수 후보와 per-rail exact-one의 구분을 Phase A selector projection으로 고정하고 Phase B hash-bound source-to-production owner join을 read-only로 관찰한다. DSU-06/FIX-03/P12/production wiring은 열지 않으며 physics/solver/`Y_global`/`Zii`와 PowerSI numerical improvement는 0으로 유지한다. |
...
## 13. W7-PHYS-OWNER-JOIN-EVIDENCE-01 — sole ACTIVE

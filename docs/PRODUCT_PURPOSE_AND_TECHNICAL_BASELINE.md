...
- Sole ACTIVE item (current): **`W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`** — old component-evidence gate는 `STOP_RESOURCE_OR_CANCELLED`로 영구 종료했으며, lean DSU producer-time observer만 진단 목적으로 유지한다. production wiring, topology 승격과 P12는 열지 않으며, 실제 source invocation은 별도 contract/self-check/Sol ACCEPT gate 이후에만 가능하다.
...
- coordinator freeze: old `source_anchor_component_evidence.py`는 contract `95fd4d940019515aaef9dd43c38f4fb5301d7aeb`에서 target census cap으로 `STOP_RESOURCE_OR_CANCELLED` 영구 종료했다. sole ACTIVE는 `D:\SPD-Decap-PI-Evaluator-W7\_coordinator_temp\source_anchor_lean_dsu_evidence.py` (109,333 bytes, SHA-256 `ff2ce1af1528033ec378354988bd8cc7cf1fc381e7e2f511629e140bcadb7d35`)이며 self-check/in-memory compile PASS와 Sol ACCEPT를 기록한다. Recovery contract `96b0a6235b8139671c90be94dceea4a9a573f6d2`는 `e9c0e545d17e4707122dfce0e16b1377f9b50c49`의 단일 docs-only child이고, future lean contract는 `95fd4d940019515aaef9dd43c38f4fb5301d7aeb`의 단일 docs-only child여야 한다. 원본 SPD는 `D:\S4LB002-2Para_260729_1_injected.spd` (1,116,717,287 bytes)이며 observer source SHA는 `40cb44b2376f59d6b606eb9b4d138204fe51b2dc6b3332d3b7c0e7d4202866d2`와 exact 일치해야 한다. 정상 lean observer-reached 경로는 import/analyze/observer/report `1/1/1/1`, retry 0이고 runtime guard만 계수한다.

...
accuracy claim. Sole ACTIVE successor is
...
one actual source caller bound to `require_plane_sheet_payload=True` to select a later bounded trace; zero
actual caller => DONE-negative producer/no solver consumer and move to separately documented integration
design; multiple/ambiguous/error/over budget => STOP. No file open/code/test/production/profile/solver/

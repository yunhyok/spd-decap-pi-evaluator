...
`layerwise_network`. These matches are location evidence only. Sole ACTIVE successor is
...
or production/accuracy proof is authorized. Questions are the enclosing owner signature/return/
mutation, attachment/metadata normalization, v3 preservation/fail-closed behavior, and compiler
invocation versus delegated output; if a function boundary/delegate is outside the excerpt,

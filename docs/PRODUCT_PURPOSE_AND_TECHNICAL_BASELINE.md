...
- 현재 권위 상태: sole ACTIVE 17BU `W7-PHYS-W6-RAW-SURFACE-FIELD-ORIGIN-LOCAL-TRACE`, exact once-only numbered read restricted to compiler.py lines2460-2535 (76 lines); require complete item creation/return and direct origins+normalization/validation for net/layer/asset SHA/islands plus complete `_surface_source_hashes(...)` call/args in range. STOP/no expansion/no retry if endpoints or dependencies leave range; helper internals, canonical SHA, island/electrical ownership remain separate. No physical stamp/accuracy execution.
...
ACTIVE successor is 17BU, a bounded local field-origin read; no islands/electrical ownership/stamp/accuracy
claim follows.

...
- Sole ACTIVE item (current): **`W7-PHYS-SOURCE-ANCHOR-COMPONENT-EVIDENCE-01`** — Recovery-01이 보존한 compiled-only certificate/storage identity를 context-only로 사용해 producer-time source anchor/component evidence를 bounded observer로 수집한다. production wiring, topology 승격과 P12는 열지 않으며, 실제 source invocation은 별도 contract/self-check/Sol ACCEPT gate 이후에만 가능하다.
...
- coordinator freeze: `D:\SPD-Decap-PI-Evaluator-W7\_coordinator_temp\source_anchor_component_evidence.py` (69,919 bytes, SHA-256 `3bf566baeceac42a14416f72c91950a4e3d830801928eb4dd77344ee0340e015`)의 self-check와 in-memory compile은 PASS였고 Sol ACCEPT를 받았으며, P0/P1은 없다. 원본 source/import/artifact/output은 읽지 않았다. Recovery contract `96b0a6235b8139671c90be94dceea4a9a573f6d2`는 `e9c0e545d17e4707122dfce0e16b1377f9b50c49`의 단일 docs-only child이며, future successor는 이를 부모로 하는 단일 docs-only child여야 한다. 원본 SPD는 `D:\S4LB002-2Para_260729_1_injected.spd` (1,116,717,287 bytes)이며 observer source SHA는 `40cb44b2376f59d6b606eb9b4d138204fe51b2dc6b3332d3b7c0e7d4202866d2`와 exact 일치해야 한다. 정상 observer-reached 경로는 import/analyze/observer/report `1/1/1/1`, retry 0이고 runtime guard만 계수하며, timeout/interrupt/unexpected communicate의 child call은 `not_evaluated_after_termination`이다.

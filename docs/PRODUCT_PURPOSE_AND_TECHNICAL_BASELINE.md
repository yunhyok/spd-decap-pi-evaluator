...
- Sole ACTIVE item (current): **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION**. The tamper fix closed all three producer-test SQLite handles and the sole run passed the first 13 nodes, then the historical provisional-request test leaked its synthetic cap `48` into the now-authoritative streamed final-spool count. The product correctly failed closed. The successor restores the authoritative cap after capturing the request and before real compilation. Improvement remains 0.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. Luna는 `tests/test_source_plane_ownership_ir_producer.py`의 `capture_compile`에서 request 포착 직후, `original_compile` 직전에 `monkeypatch.setattr(spd_adapter, "MAX_SOURCE_PLANE_OWNERSHIP_IR_ROWS", AUTHORITATIVE_IR_ROW_CAP)` 한 문장만 추가한다. Sol static review 뒤 [작업 기준 12.60](WORK_EXECUTION_BASELINE.md#1260-w7-phys-owner-join-ownership-streamed-ir-streamed-handoff-provisional-cap-oracle-fix-01--frozen--ready_for_implementation)의 provisional 노드 우선 18개 node를 `-x -vv --tb=long` Python312 invocation 1회(외부 300 s 상한) 실행한다. 모든 product/source IR/other test bytes, schema/caps, pressure/exact-300K, original SPD/full suite/solver/production/PowerSI/release는 금지한다.
...
| 현재 ACTIVE | **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION** |
| 현재 gate | provisional request 포착 뒤 synthetic `48`을 authoritative final cap으로 복원하고 provisional 노드 우선 18-node lightweight scope를 단일 300 s invocation으로 확인; product/schema/caps/other tests와 accepted pressure/exact-300K 불변 |
...
## 28. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-COLLECTION-FIX-01 — DONE / STOP_TEST_FAILURE_AND_RUNTIME_BUDGET
...
The only product delta was deletion of the stale first one-line docstring; the resulting ownership IR SHA-256 is
`59C85203333561158C1C91699EEAB02C6BD24734EF726E844D81B0E4604906B7`. Tests remained byte-identical and Sol
statically reconstructed the predecessor bytes and accepted that one-line delta.

The sole fresh 15-node invocation emitted `.....F...`. The pressure node failed, but pytest had not emitted its
traceback before later work began, so its exact exception and cause are unknown and must not be inferred. The run then
entered the `source_count=150000, surface_count=149977, expected_total=300000` parameter. It remained CPU-active with
about 120 MiB working set until the parent enforced the fixed 5,400 s runtime budget. The last parameter, cleanup and
producer nodes were not reached. Ctrl-C ended the process with exit 1 and no final pytest summary. No PASS claim is
made from progress dots, and this invocation is immutable/no-rerun.

Static SQL review identified a deterministic independent defect: the surface component check performed an unindexed
correlated `lower(surface_id)` scan for every one of 149,977 surfaces, with a worst-case 149,977 squared comparison
plan. The pressure path also had a folded primitive/source cross-product and ledger coverage whose bound depended on an
optional automatic index rather than an explicit owner-leading index.
This is validation runtime debt, not PowerSI physics or evidence that the pressure failure had the same cause.

## 29. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-VALIDATION-RUNTIME-FIX-01 — DONE / ACCEPT_WITH_OUTPUT_LIMITATION

Base/docs contract is exact `ef93d05`. Freeze the six current candidate files by SHA-256: raw compiler
...
Luna may modify only `_validate_spooled_rows` in `source_plane_ownership_ir.py`: add TEMP indexes for
`_ir_primitives(source_fold)` and `_ir_ledger_members(owner_fold)`, replace the reverse Shape lookup with the indexed
folded record key, compare surface component counts from the outer `surfaces` row without a correlated rescan, and
replace the retained-primitive/edge cross-product with set difference. Preserve Python `casefold()` semantics; SQLite
`NOCASE`, permanent schema/index changes, new tables/abstractions/dependencies and every test edit are forbidden.
External v2 schema, hashes, caps, validation outcomes and all non-validator product bytes remain unchanged.

After Sol static review, run exactly one Python312 invocation with `-x -vv --tb=long`: first
`test_v2_pressure_vector_uses_bounded_generators`, then only
`test_v2_exact_section_and_total_caps[150000-149977-False-300000]`. Enforce a 600 s external wall limit. Pressure
failure must stop immediately and preserve the full traceback; exact-300K runs only if pressure passes. Any collection
error, failure, timeout or interrupt is a new DONE/STOP with no same-contract rerun. The old 15-node set, original SPD,
full suite, upstream recovery/certificate, solver, production, PowerSI and release work remain forbidden. Passing this
gate proves only those two synthetic ownership transport/validator cases; numerical improvement remains 0.

Luna changed only `_validate_spooled_rows`; the final ownership IR SHA-256 is
`1027E694F6994B4F6B109933617D149A7A5184CD6951050D2C93F67B145079DE`. Sol found no P0/P1 and accepted the indexed/set-based
plan. The exact ordered two-node Python312 process exited 0 within wrapper wall 22.8483887 s. Fresh node IDs and ordered
temporary databases identify both requested cases, but redirected stdout was unavailable; therefore literal `2 passed`,
pytest durations and any wider acceptance are not claimed. The invocation is consumed/no-rerun.

## 30. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-LIGHTWEIGHT-ACCEPTANCE-01 — DONE / STOP_TEST_ORACLE_COMPOUND_MUTATION

Freeze the same six candidate hashes listed in section 29 except for the accepted ownership IR hash above. No product or
test edit is allowed. Run exactly the 18 nodes listed in the work baseline once, cheap fail-closed checks first and the
two remaining large rejection boundaries last, with `-x -vv --tb=long` and a 300 s external wall limit. The already
accepted pressure and exact-300K cases, the redundant 150,000-section pass, static-only dataclass shape check, unchanged
tests and full suite are excluded. Failure or timeout is a new DONE/STOP with captured output and no rerun.

Passing proves only the named synthetic/MINI-SPD streamed SQLite transport, v2 logical parity, fail-closed
bounds/invariants/cleanup and adapter producer branches. Original SPD, the upstream 13.09 GiB path, production owner
join, solver/`Y_global`/`Zii`, PowerSI accuracy/performance and release remain unproved; improvement remains 0.

The sole run collected 18 nodes, reported seven explicit PASS results, then failed the eighth node with expected
`SOURCE_PLANE_OWNERSHIP_IR_ID_COLLISION` versus actual `SOURCE_PLANE_OWNERSHIP_IR_DATABASE_INVALID`; summary was
`1 failed, 7 passed in 6.39s`, child exit 1, wrapper wall 7.0349161 s. The failing node's 0-based mutation index 10
changed the parent ledger ID without changing its child member IDs. The FK-enabled destination therefore rejected the
...
## 32. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-MATERIAL-SELECTION-FIX-01 — DONE / STOP_PRODUCT_STREAMED_HANDOFF_REGRESSION
...
The sole run collected 18 nodes and stopped at the first producer component-layer node with no reusable PASS result.
The primary error was `sqlite3.IntegrityError: NOT NULL constraint failed: stackup_layers.raw_layer_ordinal`; pytest
reported `1 failed in 1.87s`, child exit 1, wrapper wall 2.6307 s. During unwind, Windows then raised WinError 32 while
deleting `raw-spatial-spool.sqlite`, masking that primary in the final exception. The Material branch was traversed, but
this does not accept the predecessor gate. The invocation is consumed/no-rerun.

Static tracing classifies two product regressions. First, the streamed migration replaced the predecessor's exact join
of raw-v3 stackup/dielectric rows with source provenance by unmodified request-draft rows, omitting the complete sibling
set: final contiguous ordinal, global raw ordinal and canonical raw-row SHA. Second, eager cursor-backed retained/member
generators stay live in the traceback, so connection close cannot release the Windows handle before temporary cleanup.
The fixture does not mutate these fields and the capture builder was never reached.

## 33. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-FIX-01 — DONE / STOP_TEST_CAPTURE_CONNECTION_LEAK

Restore the predecessor's proven raw-v3/provenance join inside the current spool writer: folded layer join for stackup,
folded layer plus point ordinal for dielectric rows, raw physical values overriding while provenance remains, compact
final ordinals, global raw ordinals and canonical row SHA-256. The adapter consumes those two spool sections and treats
them as streamed. Retained/member cursor iterators open lazily and the generic insert loop closes each owned iterator in
`finally` before rollback/connection close/rethrow. Do not suppress cleanup errors, reload the raw asset, change schema
or caps, or add helpers/tests. After Sol static acceptance, run the same exact 18-node order from work-baseline section
12.57 once under the same 300 s bound. The claim ceiling remains the named synthetic/MINI-SPD streamed transport and
producer paths; original SPD, production owner join, solver/`Y_global`/`Zii`, PowerSI and release remain unproved, and
numerical improvement remains 0.

The implementation reached final hashes `5B02CC51677FEB6BCBE537071C7A13342E2036C0F2D7910103118865C3A45BA2`
for the raw compiler and `F8E81D7B702E155C51766A8CC0CAC2B3FE3E189450BEB43DAC0811FA9A213D14` for the
adapter. Sol's final static review found no P0/P1. The sole run collected all 18 nodes, then the first node completed its
product callback/build with no application exception but failed when the outer raw TemporaryDirectory deleted
`raw-spatial-spool.sqlite`: WinError 32, `1 failed in 2.27s`, child exit 1, wrapper wall 2.8645396 s. No node passed and
the run is consumed/no-rerun. A policy-rejected wrapper attempt created no Python process and is not an execution.

The two test capture wrappers use `sqlite3.Connection` as a context manager, which commits or rolls back but does not
close the connection. The first wrapper caused this cleanup failure; the same defect exists in the later producer
roundtrip wrapper. Product callback/build connections and attached-spool ownership close in the correct order and no
product P0/P1 remains.

## 34. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-CAPTURE-CONNECTION-FIX-01 — DONE / STOP_TEST_TAMPER_CONNECTION_LEAK

Modify only `tests/test_source_plane_ownership_ir_producer.py`: import stdlib `contextlib.closing` and wrap both
read-only `sqlite3.connect(spool.path)` capture connections with `closing(...)`. Do not add GC, sleeps, retries or
product changes. After Sol static acceptance, run the same exact 18-node order from work-baseline section 12.58 once
under the same 300 s bound. The claim ceiling remains the named synthetic/MINI-SPD streamed transport and producer
paths; original SPD, production owner join, solver/`Y_global`/`Zii`, PowerSI and release remain unproved, and numerical
improvement remains 0.

The implementation changed only the producer test to final SHA-256
`44E82828827CF5D4C43D489FC51340884F791FB7ED4E806081115B11F1CBE83A`; removing the import and reverting the two
capture statements reconstructs exact pre-fix hash `D08714921FDBBB9577DCE4CD4568EB359331B56FCB3FC1CFEFD3DA6DBC6AA3C5`.
Sol's static review found no P0/P1. The sole run collected all 18 nodes and stopped at the first after reaching the
intended `SOURCE_PLANE_OWNERSHIP_IR_TERMINAL_INCOMPLETE` exception. Its separate `tamper_node` SQLite context manager
did not close the handle, so Windows cleanup masked that expected exception with WinError 32: `1 failed in 2.44s`,
child exit 1, wrapper wall 3.1867265 s, reusable PASS 0. Captured output was 249,289 bytes with SHA-256
`F02D5541266A4F3DFC5A987FB0812C93F8104F533030C2C7B8C72A160AD8AFD0`. The invocation is consumed/no-rerun.

All product connections close before cleanup. The producer file has exactly three `sqlite3.connect` sites: the two
capture sites now use `closing`, while the test-only tamper site is the sole remaining leak.

## 35. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-TAMPER-CONNECTION-FIX-01 — DONE / STOP_TEST_PROVISIONAL_CAP_SCOPE_LEAK

Modify only the remaining `tamper_node` statement in `tests/test_source_plane_ownership_ir_producer.py` to
`with closing(sqlite3.connect(evidence.spool.path)) as connection:`. Reuse the existing import; do not change the
explicit SQL commit, mutation, exception, product, schema, caps or any other test byte. After Sol static acceptance, run the same exact
18-node order from work-baseline section 12.59 once under the same 300 s bound. The claim ceiling remains the named
synthetic/MINI-SPD streamed transport and producer paths; original SPD, production owner join, solver/`Y_global`/`Zii`,
PowerSI and release remain unproved, and numerical improvement remains 0.

The implementation changed only the remaining tamper statement, producing final test SHA-256
`494468358C939CA25A0FA013616DD93702CDBE4051C47DB3CD9004C757CE7193`; exact reversal reconstructs
`44E82828827CF5D4C43D489FC51340884F791FB7ED4E806081115B11F1CBE83A`. Sol static review found no P0/P1. The sole
run collected 18 nodes and explicitly passed the first 13, then the provisional-request node failed the actual final
spool cumulative check with `SOURCE_PLANE_OWNERSHIP_IR_BOUND_EXCEEDED`. Pytest reported `1 failed, 13 passed in
8.79s`; child exit 1, wrapper wall 9.435489 s. Captured output was 244,178 bytes with SHA-256
`3031D61F5EA12AB5328CBDB02AD73627AF3B0F73F8C2C685A7AA89E624621B0B`. The invocation is consumed/no-rerun and the
13 partial PASS results are not acceptance evidence.

The product correctly applies its cap to the actual streamed final spool. The historical test patched adapter cap 48
only to exercise the removed provisional aggregate before the legacy builder, whose separate source-module constant
remained authoritative. After the streamed handoff moved final counting into the adapter, the synthetic cap outlived
its intended request-only scope.

## 36. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-STREAMED-HANDOFF-PROVISIONAL-CAP-ORACLE-FIX-01 — FROZEN / READY_FOR_IMPLEMENTATION

Modify only `capture_compile` in `tests/test_source_plane_ownership_ir_producer.py`: after the request snapshot is
captured and before `original_compile`, add
`monkeypatch.setattr(spd_adapter, "MAX_SOURCE_PLANE_OWNERSHIP_IR_ROWS", AUTHORITATIVE_IR_ROW_CAP)`. Request construction
still runs under synthetic 48, while real compile/final counting uses the authoritative cap. Preserve every assertion,
product byte and other test byte. After Sol static acceptance, run the provisional node first and the remaining exact
17 nodes in their prior relative order from work-baseline section 12.60, once under 300 s. The claim ceiling remains the
named synthetic/MINI-SPD streamed transport and producer paths; original SPD, production owner join,
solver/`Y_global`/`Zii`, PowerSI and release remain unproved, and numerical improvement remains 0.

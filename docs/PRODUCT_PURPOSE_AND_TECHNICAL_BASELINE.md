...
- 현재 권위 상태: sole ACTIVE 17CM `W7-PHYS-W6-PLANE-PRIMITIVE-RECORD-BY-KEY-POPULATION-DISCOVERY`; exact once-only `record_by_key` query in compiler.py, gate exit0 2-20 unique lines/1 file, actual init+records-based population+key construction+line802 lookup in one cluster span<=120; annotation/lookup-only/no population/multiple/span/cap STOP/no retry. Location-only; no source read/requery/expansion, islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release excluded.
...
17CL is BLOCKED/source-classified STOP at clean HEAD `4c07698a84f03c5305b59a965061dff27a01d982`: the exact query ran once with exit0, 3 lines/1 file, span38; container line780, lookup802, and transfer818 were found, but record_by_key population and asset/asset_sha256 validation were unproven. No body read or retry occurred. Sole ACTIVE successor is 17CM, a record-by-key population discovery query.
17CM authorizes only this exact once-only query:
...
Gate: exit0, 2-20 unique lines/1 file; actual initialization + records-based population + key construction + line802 lookup must form one clear cluster span<=120. Annotation/lookup-only, no population, multiple/span/cap => STOP/no retry. Location-only; no body read.

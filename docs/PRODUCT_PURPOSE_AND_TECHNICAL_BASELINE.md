...
- Sole ACTIVE item (current): **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-03 / FROZEN / READY_TO_EXECUTE** at base `2174a3891158a7d210e57ceb86a61937424386e4`. Its distinct 52,836-byte coordinator SHA is `5aef286c9bb936c7d748a8f1b5396f3a27f2bdc3922b076c74e2fb1b90eca0d7`; source-only compile and self-check were each consumed once with exit 0/PASS and must not repeat. The runtime contract must be the base's direct child changing exactly these three docs, and one parent launch/retry 0 may use only its new empty EVIDENCE-03 root. Claim ceiling is import/compile/P1/shadow owner-join evidence; no production/PowerSI promotion or improvement.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. EVIDENCE-03 exact docs-only direct-child contract가 committed/tracked-clean이고 Sol execution acceptance를 받은 뒤에만 frozen parent launcher를 정확히 1회 실행한다. Retry, patch, rerun, old root/gate reuse, compile/self-check 반복은 금지한다. Production owner-off/wiring/solve/`Y_global`/`Zii`/PowerSI 승격과 accuracy claim도 금지한다.
...
| 현재 ACTIVE | **W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-03 / FROZEN / READY_TO_EXECUTE**; distinct coordinator/direct-child docs/new empty root |
| 현재 gate | exact docs-only contract commit + Sol execution acceptance 뒤 frozen parent launch 1회/retry 0; no patch/rerun |
...
## 20. W7-PHYS-OWNER-JOIN-ORIGINAL-SPD-EVIDENCE-03 — FROZEN / READY_TO_EXECUTE

...
no-v3-regression claims remain unproven. Sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-V3-IMPORT-API-SIGNATURE-TRACE` (17AM), authorizing exactly once reading
`src/spd_decap_pi/spd_adapter.py` lines6423-6465 (max43), no search/second range; truncated signature/
defaults at6465 => STOP. No code/test authority. Frozen later candidate only: two-file kw-only
`include_plane_sheet_payload: bool=False` forwarding at7860, selected test
`import_spd_scenario(source, include_plane_sheet_payload=True)` at2298 and loader
call2500 require True, then Sol diff + one focused node; other calls default-v2.

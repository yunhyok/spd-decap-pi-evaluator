...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); `W7-PHYS-ACTUAL-P0`와 successor R1은 scenario 없이 DONE/STOP, FIX-01과 FIX-02는 각각 commit `b0b90db`, `6fc06dd`에서 DONE/ACCEPT다. `W7-PHYS-ACTUAL-P0-R2`만 ACTIVE이고 W7 production physics는 아직 BLOCKED다.
...
- Sole ACTIVE item (current): **`W7-PHYS-ACTUAL-P0-R2`** — FIX-02가 구분한 target contact candidate의 실제 cardinality를 같은 원본 SPD에서 정확히 한 번 분류하는 successor diagnostic gate다. zero/multiple은 그대로 STOP하고 singular one일 때만 기존 scenario save/reload를 진행한다. production wiring/broadband와 P12는 열지 않는다.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. P12 prospective review는 NO-GO다. 허용 범위는 세 기준 문서에 동결한 R2 coordinator one-shot과 그 새 output root뿐이다. R1 재시도·부분 재사용, 코드/test/schema 수정, first-match/dedupe/fallback, synthetic MINI spectral/nullspace 분석, threshold/gauge/reordering/fixture capacitance 변경, solve/Touchstone/P0-P12, production wiring, broadband, Distribution, W6/PowerSI fitting·comparison과 release는 금지한다.
...
| 현재 작업 상태 | `W7-PHYS-ACTUAL-P0`와 successor R1은 scenario 없이 영구 DONE/STOP했다. FIX-01은 commit `b0b90db`, FIX-02는 commit `6fc06dd`와 focused PASS/Sol ACCEPT로 DONE이다. `W7-PHYS-ACTUAL-P0-R2`만 ACTIVE다. |
| 현재 gate | R1 report(22,879 bytes, SHA-256 `3d009c61…76b5`)와 root는 불변 보존한다. R2는 별도 exact-main commit/new empty root에서 import 1회, 조건부 save/reload 각 최대 1회, retry 0으로 실제 candidate가 zero/one/multiple인지 분류한다. solve와 정확성 주장은 금지한다. |

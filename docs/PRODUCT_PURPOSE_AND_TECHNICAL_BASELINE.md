...
`4af8ae120469be2fc3bf99ff30f07b1a5e961565`. 17AA and 17AA-S1 remain historical BLOCKED failed
attempts. S2 is DONE: the approved command
...
The v3 emission goal is achieved via S2. Sole ACTIVE successor is
`W7-PHYS-W6-RAW-SPATIAL-V3-PERSISTENCE-OWNER-DISCOVERY` (17AB), a read-only one-query discovery
bounded to `rg -n --no-heading --color never --glob '*.py' '\b(?:build_compiled_topology_asset|COMPILED_TOPOLOGY_ASSET_METADATA_KEY|RAW_SPATIAL_CONTACT_ASSET_METADATA_KEY)\b' 'src/spd_decap_pi'`;

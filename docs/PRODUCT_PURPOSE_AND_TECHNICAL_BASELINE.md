...
- 현재 권위 상태: sole ACTIVE 17CA `W7-PHYS-W6-RAW-SURFACE-PARSE-LAYERS-OVERSIZE-BOUNDARY-DIAGNOSTIC`; exact distinct no-span-cap PowerShell definition query is authorized once, exit0 exactly 2 lines/1 file with unique `_parse_layers` definition then immediate next top-level boundary, positive line ordering; parse/duplicate/no-next/output mismatch STOP/no retry. No span cap/body/context or full owner/decode read is authorized; source hash helper, canonical SHA, islands, ownership/stamp/accuracy remain separate.
...
with zero output because the positive target-to-next boundary span exceeded120; no retry/body read. Sole ACTIVE
successor is 17CA, an oversize parse-layers boundary diagnostic.

17CA authorizes only the exact distinct no-span-cap PowerShell definition query over `raw_spatial_contact_compiler.py`:
exit0, exactly 2 unique lines/1 file, first `_parse_layers` definition then immediate next top-level boundary with
positive line order; parse/duplicate/no-next/output mismatch is STOP/no retry. No span cap/body/context or full
owner/decode read is authorized; decode2292-2362 is only a frozen candidate. Source hash helper, canonical SHA,
islands, ownership, physical stamp, and accuracy remain separate.

...
- 상태: **G0 기준 문서** — W5 DONE; W6-BASE 260729 numerical FAIL; W7-SOURCE-IR Phase 1/2/3/4와 W7-PHYS-PROSPECTIVE-P0/P1/P2/P4/P5/P6/P7/P8/P9/P10/P11 DONE, P3/P11 DONE/STOP (모두 shadow/prerequisite 범위); Actual-P0/R1/R2와 R2-MULTI-TOPOLOGY-DIAG-01 및 Recovery-01은 DONE/STOP, FIX-01/FIX-02는 DONE/ACCEPT다. `W7-PHYS-SOURCE-ANCHOR-LEAN-DSU-EVIDENCE-01`은 DONE/`STOP_TARGET_CONTACT_MISSING_OR_AMBIGUOUS`다.
  ...
  W7 production physics는 BLOCKED다. OWNERSHIP-COUNT-DIAG-01/-02는 DONE/`STOP_RESOURCE_OR_CANCELLED`로 영구 닫혔고 sole ACTIVE는 **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION**이다. Original-SPD EVIDENCE-01/-02/-04/-05는 DONE/STOP_IMPORT_OR_OWNERSHIP, EVIDENCE-03은 DONE/STOP_UNEXPECTED로 영구 닫혔고 quotient-scope FIX-01, terminal pad-layer test-oracle FIX-01과 compact-request-bound FIX-01은 DONE/ACCEPT다.
...
- Sole ACTIVE item (current): **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION** at base `f2c98040870b30bf21d778099cc4cb6d2a553ed5`. It reuses the existing raw-spatial SQLite spool for ownership selection/source/context/final-section handoff, removes input-sized Python materialization in that handoff, and separates 150,000 selection/section bounds from the 300,000 final-total bound while preserving external v2 schema and owner semantics. Improvement remains 0.
...
- 현재 권한: main-only와 `accuracy_parse.py` 보존을 유지한다. Luna는 `raw_spatial_contact_compiler.py`, `spd_adapter.py`, `source_plane_ownership_ir.py`와 대응 지정 test 3개만 수정한다. Sol static review 뒤 지정 node 1회만 허용하며 original SPD, full suite, certificate/recovery redesign, solver/P1/`Y_global`/`Zii`, production/PowerSI와 release 작업은 금지한다.
...
| 현재 ACTIVE | **W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-FIX-01 / FROZEN / READY_FOR_IMPLEMENTATION** |
| 현재 gate | 기존 raw-spatial SQLite spool을 ownership handoff까지 확장하고 selection/section 150,000·final total 300,000을 byte/scratch bound와 함께 fail-closed 적용; 외부 v2 schema 불변 |
...
## 27. W7-PHYS-OWNER-JOIN-OWNERSHIP-STREAMED-IR-300K-FIX-01 — FROZEN / READY_FOR_IMPLEMENTATION

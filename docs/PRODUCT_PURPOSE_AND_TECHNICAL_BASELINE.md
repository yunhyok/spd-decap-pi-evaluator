...
- 현재 권위 상태: sole ACTIVE 17CK `W7-PHYS-W6-PLANE-PRIMITIVE-SOURCE-ASSET-SHA-PRODUCER-DISCOVERY`; exact once-only source_asset_sha256 query in compiler.py, gate exit0 1-20 unique lines/exact1 file, actual assignment/row construction plus source-origin reference in one cluster span<=120; import/annotation/schema/string/comment-only, multiple meaning, span/cap STOP. Location-only; no source read/requery/expansion, and islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release remain excluded.
...
17CK authorizes only the exact once-only `source_asset_sha256` query in `raw_spatial_contact_compiler.py`; gate exit0,
1-20 unique lines/exact1 file, actual assignment/row construction plus source-origin reference in one clear cluster
span<=120. Import/annotation/schema/string/comment-only, multiple meaning, span/cap STOP; no source read/requery/expansion.

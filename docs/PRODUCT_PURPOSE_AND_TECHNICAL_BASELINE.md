...
- 현재 권위 상태: sole ACTIVE 17CH `W7-PHYS-W6-RAW-SPATIAL-DECODE-TOKEN-BOUNDARY-DISCOVERY`; exact once-only definition query for `_decode_token` in compiler.py, gate exit0 exactly2 lines/1 file, unique def plus immediate next top-level positive boundary, span<=120; any fail/duplicate/no boundary/span>120 STOP/no retry. Location-only; decode body/SHA/source hash/islands/ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release remain excluded.
...
17CH authorizes only the exact once-only definition query for `_decode_token` in `raw_spatial_contact_compiler.py`;
gate exit0, exactly2 lines/1 file, unique `def _decode_token(` plus immediate next top-level positive boundary,
span<=120. Any fail/duplicate/no boundary/span>120 STOP/no retry. Location-only; no decode body/SHA/source hash/islands/
ownership/stamp/profile/replacement/nullspace/Zii/production/accuracy/release claim.

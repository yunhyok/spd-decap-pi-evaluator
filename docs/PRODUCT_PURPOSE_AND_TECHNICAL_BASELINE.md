...
creators. Sole ACTIVE successor is 17BZ, a parse-layers boundary discovery query.

17BZ is authorized only for the exact once-only PowerShell definition query over `raw_spatial_contact_compiler.py`:
exit0, exactly 2 unique lines/1 file, first `_parse_layers` definition then immediate next top-level boundary,
span1-120; any error/duplicate/no-next/over-span is STOP/no retry. PASS only authorizes later decode-owner2292-2362
and parse-layers-owner reads, each <=120 lines. Source hash helper, canonical SHA, islands, ownership, physical stamp,
and accuracy remain separate.

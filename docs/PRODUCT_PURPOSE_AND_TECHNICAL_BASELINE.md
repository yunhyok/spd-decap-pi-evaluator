...
- Sole ACTIVE item (current): 17DN `W7-PHYS-COMPILED-ANCHOR-BINDING-CONCRETE-ROW-ADAPTATION`; 17DM is permanently BLOCKED/execution STOP and 17DL is DONE/PASS.
...
Sol design ACCEPT. Exact whitelist: `scripts/validate_island_finite_via_gate.py`, `tests/test_validate_island_finite_via_gate.py`. `_retained_rail_anchor_bindings()` returns `list(bindings)` from compiled frozen `external_port_proof_view`; inner `payload["rail_anchor_bindings"][i]` remains frozen. Minimum change: annotation `list[dict[str, Any]]` and `return [dict(row) for row in bindings]`; no thaw helper/new serializer/shared production change. Focused node `test_compiled_only_anchor_bindings_use_external_port_proof_view` is strengthened with compact frozen-view fixture, exact dict and canonical digest equality; run exactly once after Sol static ACCEPT. No production run. 17DO is NOT ACTIVE; separately authorize only after 17DN PASS/clean commit/new docs/Usage Guard/fresh commit-bound root.

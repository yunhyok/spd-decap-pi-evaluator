...
- 현재 평가: 17DG는 standalone mathematical foundation이고, 17DK–17DP는 validation/metadata only다. 어느 작업도 production `Y_global` 또는 `Zii`를 변경하지 않았다.
- Sole ACTIVE item (current): **NONE**. 17DP는 `BLOCKED/source-owner-gap STOP`이다.
...
| W7의 한계 | 17DG–17DP는 production network에 결속되지 않았고 `Y_global`/`Zii` 및 PowerSI 오차를 개선하지 않았다. 17DP는 count conservation만 확인했으며 identity-level owner join은 증명하지 못했다. |

...
| 현재 단계 | P0 reference contract 진행 중, P1 baseline 증거 일부 확보 |
| 제품 코드 변경 | 없음 |
| GitHub 원격 변경 | 없음 |
| 정확성 승격 | 미달성 |
| 8 GB 성능 승격 | 미달성 |
...
| D-004 | exact route-graph reduction은 물리 근사 전 독립적으로 검증한다. | 잠정 채택; O(V+E), parity 검증 가능 |
...
| D-007 | pair 3/4는 독립 holdout이 아니라 controlled perturbation pair로 사용한다. | 확정; topology와 port/grid가 매우 유사 |
| D-008 | pair 1의 전체 Z curve 분석은 후보 동결 전까지 보류한다. | 확정; reserved transfer/scale holdout 보존 |
...
| 단계 | 산출물 | 종료 조건 | 상태 |
|---|---|---|---|
| P0 Reference contract | 8개 파일 manifest, explicit port map, PowerSI 조건 | identity/hash/grid/state 및 reference quality 기록 | 진행 중 |
| P1 Frozen baseline | 현행 solver의 전체 timing/memory/error/numerical report | 대표 pair와 rail별 재현 가능한 baseline | 일부 완료 |
| P2 Model attribution | 물리 block별 ablation matrix | 예상 port/band 개선을 원인별로 구분 | 대기 |
| P3 Local oracle | via/trace/pad/plane canonical corpus | reciprocity/passivity/conservation/mesh convergence | 대기 |
| P4 Hybrid global | condensed domains + passive MNA 후보 | 네 pair provisional accuracy gate | 대기 |
| P5 Acceleration | exact reduction, MOR, adaptive sweep | 추가 오차 budget + 8 GB gate | 대기 |
| P6 Blind validation | 동결 후 새 PowerSI pair | 사전 정의 기준 통과 | 대기 |
| P7 Final report/GitHub | 방법·결과·한계·재현법 | 사용자 검토 및 원격 반영 | 대기 |
...
- cold raw-SPD compile 목표는 P2/P3 prototype profiling 뒤 동결한다. 잠정 연구 목표는 30 min 이하이나 아직 약속 또는 승격 기준이 아니다.
...
1. 네 PowerSI export의 정확한 PowerSI 버전, mesh/adaptive convergence 설정, material/roughness 모델, port de-embedding 조건은 무엇인가?
2. pair 3과 4의 SPD 구조 차이가 의도한 `power_plane_mod`만인지, component state와 solver option도 같은가?
...
1. PowerSI 설정 및 component-state metadata를 확보해 P0의 unknown을 닫는다.
2. pair 2를 최소 canonical development case로 정하고, full diagonal/transfer/eigen/passivity 기준 report schema를 동결한다.
3. pair 3/4의 SPD semantic diff를 streaming 방식으로 작성해 실제 perturbation 영역을 식별한다.
4. 현행 solver baseline을 pair 2에서 단계별 time/RSS/conditioning과 함께 측정한다. 8 GB에서 위험한 full solve는 preflight 없이 실행하지 않는다.
5. local transition ablation용 canonical case 목록과 oracle 허용오차를 사전 등록한다.

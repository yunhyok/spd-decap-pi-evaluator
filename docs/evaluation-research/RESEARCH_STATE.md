...
| 현재 단계 | R0 source/external-port/return-owner contract 진행 중; M1 collocation negative/G1 exterior-only 결과 동결; G2 pair 제한 통과 후 100 kHz circle reciprocity/cancellation fail; independent `AV-BS1-CIRCLE` H0는 pre-factor sparse gate fail, H1 coarse `h`와 H2 refined `h2`는 각각 stage-only gate 통과. 최신 상태 `passed_AV_BS_h2_stage_only_pending_h4_preregistration`; H2 token consumed·factor/physics 실행 완료; fine analytic/mesh convergence/final circle은 `null`, H4는 별도 preregistration 전 미승인 |
| 제품 코드 변경 | 없음 |
| GitHub 원격 변경 | 없음 |
| 정확성 승격 | 미달성 |
| 8 GB 성능 승격 | 미달성 |
...
3. independent A–v FEM boundary-Schur의 H0 pre-factor negative, H1 coarse-h와 H2 `passed_AV_BS_h2_stage_only_pending_h4_preregistration` artifacts, consumed tokens와 P0/P1 commits를 보존한다. H2 result digest와 consumed tombstone을 commit한 뒤 H4 mesh/assembly/resource/result schema를 별도로 사전등록한다. clean H4 preregistration과 새 one-use token 전에는 h4 factorization/physics를 실행하지 않는다.

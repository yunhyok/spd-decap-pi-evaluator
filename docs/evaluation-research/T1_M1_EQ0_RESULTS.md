...
5. 다음 독립 reference candidate는 two-DtN subtraction이 없는 A–v volume-FEM boundary-Schur다. [`T1_AV_BOUNDARY_SCHUR_RESULTS.md`](T1_AV_BOUNDARY_SCHUR_RESULTS.md)의 H0는 pre-factor sparse gate에서 실패했지만 H1 coarse `h`는 stage-only pass다. 별도 h2/h4와 두 circle radius가 모두 통과한 뒤에만 EQ0 crop `2/4/8Deff`를 사전 등록한다.

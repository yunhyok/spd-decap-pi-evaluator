...
The sixth P1 attempt (`82775327...`) reached claim/factor child PID `55380`, but
direct child evidence stopped before `_factor_one`/`splu` on
`claimed preflight payload mismatch`. It records attempted/performed
`false/false`, zero completed/certified factors, and no RHS, solve, H4 physics,
or EQ0 input. Independent outer close `d2777207...` exhausted max3 for PID
`53404`, retained `30/30` retry events without truncation, verified cleanup,
and wrote no seal. Resource ceilings were not approached.

Emergency record `43bb34b2...` is honest but strict-validator-invalid because
its nested bindings omit `resource_policy_sha256`; commits `06061a2...` and
`c001b44...` preserve then delete-retire the token. Retry-v6 only removes a
redundant three-line Python comparison and adds the missing runner binding key.
Frozen Python/runner/tests hashes are `2373a13f...` / `852ce8a0...` /
`f1d0b044...`; focused `8/8` passed and full no-cache `330/330` passed in
`81.83 s`. Immutable M1/G1/G2 results and `next_stage_authorized=false` are
unchanged.
...
`66211ef...`. Native `splu` returned for `A_background_II`; attempted/performed
are `true/true` with one completed name. Combined non-canonical L/U storage left
zero certificates and prefixes, and `A_conductor_II`, RHS, solve, and physics
never started. The resource artifact passed, but independent outer/control
monitoring falsely exhausted across distinct confirmed-dead helpers. No terminal
seal or published result exists. Seven temporary evidence files are byte-identical
ignored-quarantine copies. Valid provisional consumption `8fce704...` and D-only
retirement `7964464...` leave the token absent.
...
5. 다음 독립 reference candidate는 A–v volume-FEM boundary-Schur다. H1/H2 stage-only pass와 H4 parents를 보존한다. 아홉 번째 P1은 producer PID `51184`에서 two-factor certificates/prefixes와 resource gates를 통과했지만 published finalizer PID mismatch와 strict-invalid raw seal 때문에 authoritative factor fit은 여전히 미증명이다. retry-v9 no-token provisional contract의 first full exact-document suite는 `397/397` in `92.95 s`, exit `0`, no failure로 통과했다. Exact-byte/audit와 별도 승인한 뒤에만 열 번째가 될 factor-only pilot을 검토하며, authoritative result audit/H4 physics/two-radius pass 전에는 EQ0 crop을 열지 않는다.

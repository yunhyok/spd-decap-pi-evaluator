...
5. 다음 독립 reference candidate는 two-DtN subtraction이 없는 A–v volume-FEM boundary-Schur다. 첫 `primary-h`는 [`T1_AV_BOUNDARY_SCHUR_RESULTS.md`](T1_AV_BOUNDARY_SCHUR_RESULTS.md)의 H0 pre-factor sparse-pattern gate에서 실패했다. H1 cyclic-diagonal correction의 새 fixture/token clean commit 뒤 동일 h만 재실행하며, circle 두 radius가 통과한 뒤에만 EQ0 crop `2/4/8Deff`를 사전 등록한다.

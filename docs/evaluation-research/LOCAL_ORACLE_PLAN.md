...
## Current H4-P1 manifest gate

The non-executable H4-P1 manifest is now the current gate. It has no token,
runner, factor, RHS, solve, sidecar, or physics path. The historical eleventh
factor-only seal is parent evidence only. Before any future execution, a new
runner/lifecycle contract must implement every frozen raw accuracy and resource
gate and receive independent explicit authorization. Selected old-plus-new H4
...
`111.82 s`, exit `0`, with no failure. This is the **FINAL DOC FREEZE**; do not

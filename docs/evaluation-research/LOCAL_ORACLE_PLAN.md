...
2. M0 periodic 1-D volume pass, failed collocation result, G1 exterior-only, G2 pair-only와 100 kHz circle reciprocity/cancellation failure를 동결한다. 독립 `AV-BS1-CIRCLE` H0 negative와 H1 `passed_AV_BS_h_stage_only_pending_h2_review` artifact를 함께 보존한다. 다음은 H1 결과 독립 review와 별도 h2 mesh/fixture/token preregistration이며, 그 clean commit 전 h2를 실행하지 않는다. 이후에도 h2→h4와 withheld radius review token을 순차 통과해야 EQ0 mesh/crop으로 확장한다. Hamiltonian Schur/four-operator Calderón SAO는 그 다음 후보로 비교한다.

...
2. M0 periodic 1-D volume pass, failed collocation result, G1 exterior-only, G2 pair-only와 100 kHz circle reciprocity/cancellation failure를 동결한다. 독립 `AV-BS1-CIRCLE` H0 primary-h는 pre-factor sparse-pattern contract에서 실패했다. 1,920개 cyclic triangulation diagonal을 exact-zero owner로 태그하는 H1 fixture/token을 새 clean commit으로 고정한 뒤에만 h 한 mesh를 재실행한다. 그 뒤에도 h→h2→h4와 withheld radius review token을 순차 통과해야 EQ0 mesh/crop으로 확장한다. Hamiltonian Schur/four-operator Calderón SAO는 그 다음 후보로 비교한다.

...
2. M0 periodic 1-D volume pass, failed collocation result, G1 exterior-only, G2 pair-only와 100 kHz circle reciprocity/cancellation failure를 동결한다. 독립 `AV-BS1-CIRCLE` H0 negative, H1 coarse-h와 H2 `passed_AV_BS_h2_stage_only_pending_h4_preregistration` artifacts, consumed tokens를 함께 보존한다. H4-P0 mesh/assembly certificate는 별도 manifest-only로 고정했으며 다음은 one-use H4-P0R factorization-only resource audit다. 그 clean commit/audit/token 전에는 factor를 만들지 않고, P0R 뒤 별도 H4-P1 전에는 h4 physics를 실행하지 않는다. 이후에도 withheld radius review chain을 통과해야 EQ0 mesh/crop으로 확장한다. Hamiltonian Schur/four-operator Calderón SAO는 그 다음 후보로 비교한다.

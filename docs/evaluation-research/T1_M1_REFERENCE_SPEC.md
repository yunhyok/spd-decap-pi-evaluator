...
다음 실행 후보는 G2 수치를 보정하는 continuation이 아니다. two-DtN subtraction이 없는 independent A–v volume-FEM boundary Schur reference candidate는 H0 pre-factor negative를 보존한 뒤 H1 17.5 µm/100 kHz coarse `h`와 H2 refined `h2`에서 각각 stage-only gate를 통과했다. 최신 상태는 `passed_AV_BS_h2_stage_only_pending_h4_preregistration`이고 h→h2 RMS/max `0.462796%/0.868015%`는 trend-only다. fine analytic/mesh convergence/final circle은 `null`, token은 consumed/next=false다. 이후 별도 `h4`, final circle과 crop/withheld-radius 단계를 순차 확장한다. production SAO 후보는 Hamiltonian Schur 또는 four-operator symmetric Calderón/Steklov–Poincaré discretization으로 별도 비교하며 post-symmetrization, negative-eigenvalue clipping, higher-precision promotion과 gate 완화를 금지한다.

...
다음 실행 후보는 G2 보정 continuation이 아니라 independent A–v volume-FEM boundary Schur다. H1/H2 stage-only pass와 H4 parents를 보존한다. 아홉 번째 P1은 producer PID `51184`에서 two-factor certificates/prefixes와 resource gates를 통과했지만 published finalizer PID mismatch와 strict-invalid raw seal 때문에 authoritative factor fit은 미증명이다. 현재 retry-v9 provisional candidate/token absent/next=false이며 fine analytic/convergence/final circle은 `null`이다. retry-v9 no-token contract의 first full exact-document suite `397/397` in `92.95 s`, exit `0`, no failure와 FINAL DOC FREEZE를 보존하며, exact-byte/audit와 별도 승인/authoritative result audit 전에는 H4 physics를 금지한다.
...
outer close SHA-256 `73073432faa0525f560b6536ea2a5fd54329ba3a30f04ab05ab8cef59460e758`는 `OUTER_RESOURCE_EXCEPTION` / `NONROOT_DISAPPEARANCE_NOT_CONFIRMED`를 고정한다. attempt 1의 PID 40224는 metric `exited`, expected/observed birth `639223978396874096`, initial complete snapshot present였고 Win32 error는 없었다. cleanup은 terminal post-cleanup sample로 검증됐으며 live matching process와 `av-bs1-*` temp directory는 0이다. claim, factor child/prefix, RHS, solve, H4 physics, numerical result, tombstone, quarantine, seal은 생성되지 않았다. 따라서 네 번째 시도도 factor fit, 8 GB, PowerSI 정확도 증거가 아니다.

retry-v4는 caller가 이미 `MaximumAttempts>1`을 전달한 outer/control sample에만 적용한다. 위와 같은 same-birth `exited` + positive birth + initial complete snapshot present일 때만 25 ms 재검사를 최대 두 번 수행해 complete snapshot 총 3개로 제한한다. 중간 absent면 기존 `CONFIRMED_NONROOT_DISAPPEARANCE` event와 whole-sample retry를 그대로 사용하고, 세 번 모두 present면 기존 fatal을 유지한다. `not_found`/87 + present, default-one factor sample, live reappearance, PID reuse, invalid/mismatch birth, root loss/reuse, query/access/malformed/incomplete snapshot은 모두 즉시 fatal이다. schema, Python matrix/factor/physics는 바꾸지 않는다.
...
## Frozen retry-v5 execution boundary
...
`claimed preflight payload mismatch`로 차단됐다. direct child의
attempted/performed는 `false/false`, completed/certified factors는 0이다.
독립 outer close `d2777207...`는 PID `53404`의 max3 exhaustion으로 30/30
events를 truncation 없이 보존하고 cleanup을 검증했지만 seal은 쓰지
않았다. resource ceiling, RHS/solve/H4 physics/PowerSI/EQ0 결과는 없다.

emergency record `43bb34b2...`는 정직하지만 nested
`bindings.resource_policy_sha256` 누락으로 strict-validator-invalid다.
`06061a2...` provisional record와 `c001b44...` deletion retirement 뒤 token은
없다. retry-v6는 redundant Python 3줄 삭제와 runner binding 1키 추가뿐이다.
frozen Python/runner/tests SHA는 `2373a13f...` / `852ce8a0...` /
`f1d0b044...`; focused `8/8`과 full no-cache `330/330`이 통과했고 후자는
...
L/U storage stopped certificates and prefixes at zero; `A_conductor_II`, RHS,
solve, and physics never started. The resource artifact passed, while independent
outer/control monitoring falsely exhausted across distinct confirmed-dead
helpers. No seal or published result exists. Seven temporary evidence files were
copied byte-identically into ignored quarantine. Valid provisional consumption
...
## Current H4-P1 manifest-only reference boundary

The separate H4-P1 preregistration freezes the future A-v fine-mesh solve,
sidecar, raw invariants, H2 commitment, and resource policy without executing
them. It neither repairs the SAO/G2 failures nor promotes full M1/T1. Current
token and terminal evidence are absent. Selected old-plus-new H4 collection was
confirmed exactly at `432 tests collected in 0.56 s`, exit `0`. The first
successful full exact-document result records `432/432` passed in `111.82 s`,
exit `0`, with no failure. This is the **FINAL DOC FREEZE**; it does not promote
M1/T1, and no later closure runtime is recorded.

## Historical execution sequence and state transitions
...
6. 같은 geometry/current basis의 A–v는 **2 GHz 4Deff smoke와 H1/H2 stage-only:** H2 certificate/token을 보존한다. 아홉 public attempt 뒤 latest factor producer는 두 certified factors/prefixes와 resource gates를 통과했지만 published finalizer PID mismatch와 strict-invalid raw seal 때문에 authoritative factor fit/H4 physics는 미증명·금지 상태다. retry-v9 no-token provisional contract의 first full exact-document suite `397/397` in `92.95 s`, exit `0`, no failure와 FINAL DOC FREEZE를 보존한다. Exact-byte audit/reread 및 별도 승인 뒤 fresh token이 생긴 경우에만 열 번째 factor-only run을 검토하고 terminal chain을 독립 감사한다. 별도 H4-P1 계약/clean audit/fresh token 전에는 h4를 실행하지 않는다.

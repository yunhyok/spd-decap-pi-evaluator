...
6. 같은 geometry/current basis의 A–v는 **2 GHz 4Deff smoke와 H1 coarse `h` stage-only, H2-P0 assembly-only:** consistent mass power smoke 뒤 17.5 µm/100 kHz boundary-Schur `h`가 `passed_AV_BS_h_stage_only_pending_h2_review`를 얻었고 refined H2 lineage/hash/resource는 manifest-only로 사전등록됐다. 별도 H2-P1 executable/result/token freeze·review 뒤에만 `h2` physics를 실행하며, 그 뒤에도 별도 `h4`, final circle과 crop `2/4/8 Deff`를 순차 실행한다.

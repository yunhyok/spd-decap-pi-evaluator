...
6. 같은 geometry/current basis의 A–v는 **2 GHz 4Deff smoke와 H1 coarse `h`, H2 refined `h2` stage-only:** consistent mass power smoke 뒤 17.5 µm/100 kHz boundary-Schur `h`와 `h2`가 각각 local gate를 통과했다. H2 artifact는 signed-M9 PDE/power certificate와 consumed one-use token을 보존한다. H4-P0R parent와 P1 executable/static review는 통과했지만 두 public attempt는 pre-factor 중단/token 폐기됐고 현재 factor fit/H4 physics는 미증명·금지 상태다. retry-v2 clean contract와 fresh one-use token을 감사한 뒤 factor-only run을 한 번 수행하고, 그 result/resource/tombstone/seal을 독립 감사한다. 이어 별도 H4-P1 physics/result 계약, clean static audit와 새 one-use token을 모두 고정한 뒤에만 `h4`를 실행해 mandatory h2→h4/fine analytic을 처음 판정하며, 그 뒤에만 final circle과 crop `2/4/8 Deff`를 순차 실행한다.

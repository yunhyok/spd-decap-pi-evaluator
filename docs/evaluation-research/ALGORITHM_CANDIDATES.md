...
- T1의 Cohn lossless `C'`와 periodic plate `R/L`은 independent 1-D FEM까지 manufactured 범위에서 통과했다. finite/open 2-D 후보는 SAO–CIM이며 A–v/3-D PEEC로 교차검증한다. M1 collocation power failure는 immutable이고 G1은 `passed_exterior_galerkin_only`다. G2 pair screen은 `passed_pair_screen_only`지만 circle 100 kHz에서 N128 cancellation `2.91315e-8`, N256 raw `Yw` reciprocity/cancellation `1.41197e-8/1.63755e-7`로 fail-closed 됐다. two-DtN subtraction이 없는 독립 A–v volume-FEM boundary-Schur reference candidate는 [`T1_AV_BOUNDARY_SCHUR_SPEC.md`](T1_AV_BOUNDARY_SCHUR_SPEC.md)에 고정했다. 첫 primary-h는 `V+2E`를 K/M 공통 nnz로 잘못 둔 prereg gate 때문에 factor 전에 `BLOCKED_AV_BS_MESH_HASH`가 됐다. [`T1_AV_BOUNDARY_SCHUR_RESULTS.md`](T1_AV_BOUNDARY_SCHUR_RESULTS.md)는 이를 보존하고 cyclic trapezoid의 1,920개 exact-zero stiffness diagonal을 topology-tagged H1 correction으로 사전등록하며 physics solve는 아직 없다.

...
- T1의 Cohn body-fitted lossless `C'`와 periodic two-plate smooth-copper `R/L`은 independent normalized 1-D FEM까지 manufactured 범위에서 통과했다. lateral-periodic slab과 finite/open rectangle은 다른 경계값 문제이므로 free-space contour를 `coth`와 직접 gate하지 않는다. finite-width 기준 2-D 후보는 homogeneous-first SAO–CIM이며 independent A–v FEM/finite-length 3-D PEEC로 교차검증한다. 첫 M1-EQ0 collocation SAO의 boundary power `1.585e-4 > 1e-8` failure는 immutable이다. 후속 direct double-panel exterior Galerkin은 `N={144,288,576}`·7 frequencies의 structural/q/`r0`/power/terminal gate를 통과했지만 `passed_exterior_galerkin_only`다. interior `WYs` weighted reciprocity `1.70%–7.12%`와 100 kHz/1 MHz raw passivity failure가 남아, 다음 후보는 target-tested interior `P/U/Pout/Uout` Galerkin G2다.

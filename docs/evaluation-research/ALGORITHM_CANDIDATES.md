...
## H4-P0R retry-v6 lifecycle boundary

The sixth public invocation is another execution-only non-result and changes no
candidate ranking. Token-only commit `82775327...`, child of clean retry-v5
contract `bddbf9cb...`, created the claim and factor child PID `55380`. Direct
child evidence then failed on `claimed preflight payload mismatch` before
`_factor_one`/`splu`, fixing attempted/performed to `false/false` and completed
factors to zero. Independent outer close `d2777207...` exhausted max3 for PID
`53404`, retained `30/30` retry events without truncation, verified cleanup, and
wrote no seal. This is not factor-fit, accuracy, PowerSI, or 8 GiB evidence.

Emergency record `43bb34b2...`, preserved by `06061a2...`, is honest about
consumption but strict-validator-invalid because nested `bindings` omits
`resource_policy_sha256`; deletion retirement `c001b44...` leaves no token.
Retry-v6 only removes a redundant three-line Python comparison and adds that
runner binding key. Frozen Python/runner/tests SHA-256 are `2373a13f...` /
`852ce8a0...` / `f1d0b044...`; focused `8/8` passed and the full no-cache
...
for `A_background_II`, so attempted/performed are `true/true` and its completed
name is durable. Combined non-canonical L/U storage then stopped the certificate
path: certificates and prefixes are zero, while `A_conductor_II`, RHS, solve,
and physics never started. The resource artifact passed. Independent
outer/control monitoring instead falsely exhausted retry attempts across
distinct confirmed-dead helpers, so no terminal seal or published result exists.
Seven temporary evidence files were copied byte-identically into ignored
quarantine. Valid provisional emergency consumption commit `8fce704...`, then
...
## Current H4-P1 contract checkpoint

The next candidate has been narrowed to a manifest-only contract, not promoted
to an executable algorithm. It fixes exact H4 matrix/solve/raw-accuracy/
sidecar/resource requirements while leaving `primary-h4` unavailable. Static
fixture tests passed three `32/32` runs. Selected old-plus-new H4 collection was
...
exit `0`, with no failure. This is the **FINAL DOC FREEZE**; do not record any
later closure runtime. No candidate score, PowerSI accuracy, H4 physics, or
8 GB feasibility result follows.

...
6. 후속 P1 executable/runner/claim/finalizer/outer observer와 one-use lifecycle을 정적 감사했다. 첫 fresh-token 공개 시도는 token read 전 root-only collection strict-mode 오류, 두 번째는 ready 전 outer tree-sampling 예외로 모두 claim/factor 전에 중단됐다. 두 token은 재사용 없이 삭제-only retirement했고 현재 token은 없다. corrected close-v2는 confirmed non-root disappearance에만 최대 3회 전체 sample 재시작을 허용하며 partial 합계를 폐기한다. root/reuse/live/access/incomplete-snapshot failure와 exhaustion은 fatal이다. 새 token-absent contract와 fresh token-only child 뒤에만 `KII`와 `ApII` factor fill/resource를 한 번 측정한다. RHS, harmonic extension, boundary `Y`, modal response와 physics result는 만들지 않는다.
...
5. H4-P0R-P1의 두 pre-factor interruption, token retirement와 validation-output를 보존한다. 현재 whole-sample retry/typed close-v2 corrective candidate를 token-absent clean commit과 독립 audit에 고정한 뒤 fresh token-only child를 만든다. retired token commits는 재실행하지 않는다.
6. fresh token에서 factorization-only pilot 한 번으로 실제 fill/resource fit만 측정한다. RHS/solve/extension/Y/modal/physics는 금지하고, P0R 결과를 독립 감사한 뒤 H4 result schema와 h2→h4/fine analytic 판정을 별도 사전등록·clean commit·fresh one-use token으로 완료하기 전에는 h4 physics solve를 실행하지 않는다.

...
5. 별도 H4-P0R manifest-only fixture/runner/test/doc가 `KII/AbII/MII/ApII`, equilibration, factor-only result/resource/lifecycle schema와 forbidden operation을 고정했고 23개 정적 test 및 세 독립 감사를 통과했다. 이 단계에는 token, claim, finalizer, factorization 또는 physics 경로가 없다.
6. 다음 별도 clean cycle에서 H4-P0R executable/runner/claim/finalizer와 one-use token을 구현·감사한 뒤 `KII`와 `ApII` factor를 순차 생성해 fill과 process-tree resource만 한 번 측정한다. RHS, harmonic extension, boundary `Y`, modal response와 physics result는 만들지 않는다. pass/fail 모두 token을 consume한다.
...
5. 별도 H4-P0R executable/runner/claim/finalizer와 one-use token을 구현·감사한 뒤 factorization-only pilot 한 번으로 실제 fill/resource fit만 측정한다. 그 전에는 factor를 만들지 않는다.
6. P0R 결과를 독립 감사한 뒤 H4 result schema와 h2→h4/fine analytic 판정을 별도 사전등록하고 clean commit/one-use token을 완료하기 전에는 h4 physics solve를 실행하지 않는다.

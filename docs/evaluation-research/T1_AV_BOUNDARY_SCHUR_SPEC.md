...
## Historical retry-v9 exact-next sequence
...
## Historical retry-v10 eleventh-result exact-next target
...
## Current H4-P1 manifest-only contract freeze

The historical target above is now frozen as a non-executable manifest in
[`T1_AV_BOUNDARY_SCHUR_H4_P1_PREREG.md`](T1_AV_BOUNDARY_SCHUR_H4_P1_PREREG.md)
and
[`../../tools/research/av_bs1_boundary_schur_h4_p1.py`](../../tools/research/av_bs1_boundary_schur_h4_p1.py).
Fixture/test SHA-256 values are `6f527c76cbc0928a4b504e01abbb4c049fd39edc75ef865a3335f3a619863200`
and `6254e3d28b56d8ec640089c02fdf3308e33775e4c3cd2d6946932bfc1ed7ac9d`.
The parser exposes only required `--stage manifest`; no H4 runner exists.

The current manifest fixes exact matrix hashes, deterministic row-then-column
equilibration, one `COLAMD`/pivot-1.0/`Equil=False` factor per matrix, all
`512` raw RHS in batches `<=4`, original-system residuals, condition estimates,
raw assembly/reciprocity/passivity/power/analytic/convergence gates, and no
repair or averaging. It also fixes the `12923024`-byte `Y`/`Y_reverse`/
`M9_interior` sidecar and the `3470862055`-byte prospective resource envelope.

These are declared future requirements, not executed behavior. Current token
and terminal evidence are absent; no RHS, factor, solve, sidecar, H4 result,
PowerSI comparison, or 8 GB proof exists. Focused and independent static
`32/32` runs passed. Selected old-plus-new H4 collection was confirmed exactly
at `432 tests collected in 0.56 s`, exit `0`. The first successful full
exact-document result records `432/432` passed in `111.82 s`, exit `0`, with no

...
2. smallest equal-width M1-EQ0의 failed collocation, G1 exterior-only, G2 pair-only과 100 kHz circle raw failure를 보존한다. planned G2 2 GHz circle/N512/EQ0 seed로 확장하지 않는다. independent `AV-BS1-CIRCLE` H0 negative, H1 coarse-h와 H2 refined-h2 stage-only passes, consumed tokens, H4-P0/H4-P0R parent와 두 P1 pre-factor interruption/retirement를 함께 보존한다. 현재 token absent/factor fit unproven이며 retry-v2 clean contract/fresh token/result audit 전에는 H4 physics를 실행하지 않는다. circle 두 radius 통과 전 outer crop `{2,4,8}Deff` EQ0를 실행하지 않는다.

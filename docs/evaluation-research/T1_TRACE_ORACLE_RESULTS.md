...
2. smallest equal-width M1-EQ0의 failed collocation, G1 exterior-only, G2 pair-only과 100 kHz circle raw failure를 보존한다. planned G2 2 GHz circle/N512/EQ0 seed로 확장하지 않는다. independent `AV-BS1-CIRCLE` H0 negative, H1 coarse-h와 H2 refined-h2 stage-only passes, consumed tokens를 함께 보존한다. H4는 별도 preregistration/audit/token 전 실행하지 않고, circle 두 radius 통과 전 outer crop `{2,4,8}Deff` EQ0를 실행하지 않는다.

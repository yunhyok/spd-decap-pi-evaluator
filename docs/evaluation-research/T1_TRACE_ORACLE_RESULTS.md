...
2. smallest equal-width M1-EQ0의 failed collocation, G1 exterior-only, G2 pair-only과 100 kHz circle raw failure를 보존한다. planned G2 2 GHz circle/N512/EQ0 seed로 확장하지 않는다. independent `AV-BS1-CIRCLE` H0 negative와 H1 coarse h stage-only pass, consumed H1 token과 H2-P0 refined assembly manifest를 함께 보존한다. 별도 H2-P1 executable/result/token clean preregistration 전 h2 physics를 실행하지 않으며, circle 두 radius 통과 전 outer crop `{2,4,8}Deff` EQ0를 실행하지 않는다.

...
- T1의 lossless Cohn body-fitted `C'`, periodic two-plate smooth-copper identity, 독립 1-D slab FEM과 `C0-A1` 원형 interior DtN gate는 제한 범위에서 통과했다. M0의 lateral-periodic slab과 finite/open rectangle은 다른 경계값 문제이므로 free-space `H2` contour를 periodic `coth`와 직접 비교하지 않는다. 사전 등록했던 `C0-A0` 저주파 `10^6` switch는 작은 원 100 kHz에서 실패했다. 첫 finite/open M1-EQ0는 frozen full-contour `N={144,288,576}`에서 response/mesh/condition/current gate를 통과했지만 raw SAO boundary-power mismatch가 fine에서도 `1.585e-4 > 1e-8`라 `BLOCKED_SAO_BOUNDARY_POWER_IDENTITY`로 차단했다. independent A–v 2 GHz smoke의 consistent mass power는 통과했으나 mesh/crop convergence가 없다. 다음 후보는 같은 endpoint에서 direct double-panel Galerkin exterior를 사전 등록하는 M1-EQ0-G1이며, 3-D end audit, 실제 return polygon과 absolute/exact-core global operator도 계속 차단 상태다.
...
13. [`T1_M1_EQ0_RESULTS.md`](T1_M1_EQ0_RESULTS.md): frozen finite/open response pass, raw power failure, independent A–v smoke와 G1 next gate 확인

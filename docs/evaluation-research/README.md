...
- H2-P1은 H1/P0 lineage, token/claim/guard/finalizer/atomic tombstone, native process-tree 900 s guard, signed-M9 trend-only와 독립 modal PDE residual/volume-power certificate를 고정한 뒤 one-use `primary-h2`를 실행했다. artifact SHA-256은 `b890e4af13d97591b3134788984b3657f6f6b046e3043ce0a6f6792fe1ad7f55`이고 local gate가 통과했다. active token은 consumed tombstone SHA-256 `81574c1099bdd140004940b7cb768a20298b153445c1b16b9e21de95011c48c2`, `uses_remaining=0`, `next_stage_authorized=false`다. 별도 H4-P0는 32,001-node topology, 7,424 topology-owned cyclic tags, canonical `K/M/MΓ`와 H4 전용 `256uκ` certificate를 assembly-only로 고정했다. 이어 H4-P0R manifest 계약도 23개 정적 test와 세 독립 감사를 통과해 `KII/AbII/MII/ApII`, equilibration, one-factor resource policy와 향후 one-use lifecycle을 동결했다. 현재 상태는 `preregistered_H4_P0R_contract_only_no_factor`이며 executable token/claim/finalizer와 factorization은 아직 없고 `factor_fit_unproven=true`다. 따라서 별도 executable/static audit/clean commit/one-use token 전에는 factor pilot을, P0R 결과 audit과 별도 H4-P1 전에는 `h4` physics를 실행하지 않는다.
...
19. [`T1_M1_EQ0_RESULTS.md`](T1_M1_EQ0_RESULTS.md): immutable collocation failure, G1 exterior-only, G2 pair-only pass와 100 kHz circle reciprocity/cancellation failure 확인
20. [`T1_CIRCLE_DTN_RESULTS.md`](T1_CIRCLE_DTN_RESULTS.md): frozen `C0-A0` 실패, selected `C0-A1` circle-only 통과와 withheld/수치 범위 확인
21. [`T1_M0_SLAB_RESULTS.md`](T1_M0_SLAB_RESULTS.md): periodic slab의 independent 1-D FEM pass, withheld 결과와 free-space/periodic boundary mismatch 확인
22. [`ORACLE_REPRODUCTION.md`](ORACLE_REPRODUCTION.md): custom numerical table과 manifest의 exact 재현 명령 확인
23. [`SESSION_LOG.md`](SESSION_LOG.md)의 가장 최근 항목: 직전 세션의 증거와 중단 지점 확인

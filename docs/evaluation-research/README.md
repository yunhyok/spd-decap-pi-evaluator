...
- T1의 lossless Cohn body-fitted `C'`, periodic two-plate smooth-copper identity, 독립 1-D slab FEM과 `C0-A1` 원형 interior DtN gate는 제한 범위에서 통과했다. M0의 lateral-periodic slab과 finite/open rectangle은 다른 경계값 문제이므로 free-space `H2` contour를 periodic `coth`와 직접 비교하지 않는다. 사전 등록했던 `C0-A0` 저주파 `10^6` switch는 작은 원 100 kHz에서 실패했다. 첫 finite/open M1-EQ0 collocation은 frozen full-contour `N={144,288,576}`에서 response/mesh/condition/current gate를 통과했지만 raw SAO boundary-power mismatch가 fine에서도 `1.585e-4 > 1e-8`라 immutable `BLOCKED_SAO_BOUNDARY_POWER_IDENTITY` negative result로 보존한다. 후속 G1 direct double-panel exterior Galerkin은 같은 `N`과 7 frequencies에서 structural/q/`r0`/power/terminal gate를 통과했지만, fine interior `WYs` weighted-reciprocity가 `1.70%–7.12%`로 `1e-8` gate를 모두 넘고 100 kHz/1 MHz raw Hermitian passivity도 실패했다. 따라서 G1은 `passed_exterior_galerkin_only`이며 전체 M1/T1이 아니다. 다음은 target-tested interior `P/U/Pout/Uout` Galerkin인 G2 (`preregistered_not_run`)이고, 그 뒤에만 independent A–v mesh/crop convergence를 수행한다. 3-D end audit, 실제 return polygon, absolute/exact-core global operator, PowerSI correlation과 8 GB product status는 계속 차단 상태다.
...
13. [`T1_M1_EQ0_RESULTS.md`](T1_M1_EQ0_RESULTS.md): immutable collocation power failure, G1 exterior-only certificate, interior reciprocity/passivity blocker와 G2 next gate 확인
14. [`T1_CIRCLE_DTN_RESULTS.md`](T1_CIRCLE_DTN_RESULTS.md): frozen `C0-A0` 실패, selected `C0-A1` circle-only 통과와 withheld/수치 범위 확인
15. [`T1_M0_SLAB_RESULTS.md`](T1_M0_SLAB_RESULTS.md): periodic slab의 independent 1-D FEM pass, withheld 결과와 free-space/periodic boundary mismatch 확인
16. [`ORACLE_REPRODUCTION.md`](ORACLE_REPRODUCTION.md): custom numerical table의 exact 재현 명령 확인
17. [`SESSION_LOG.md`](SESSION_LOG.md)의 가장 최근 항목: 직전 세션의 증거와 중단 지점 확인

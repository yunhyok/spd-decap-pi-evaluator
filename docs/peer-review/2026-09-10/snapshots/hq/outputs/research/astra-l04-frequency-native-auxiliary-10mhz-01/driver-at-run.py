"""SPD Decap PI Evaluator v0.23.1: disabled 10 MHz hybrid-H auxiliary block."""
import argparse
import ctypes
...
R = base.R
RUN_RELEASED = True  # HQ reviewed target-frequency flow and verified all source receipts.
...
def self_check():
    ...
    assert not pending()
    print('PASS_10MHZ_AUXILIARY_SOURCE_PINS_AND_QUALIFICATION')
...
def launch(output: Path) -> int:
    ...
    with subprocess.Popen(command, stdin=subprocess.DEVNULL,
        ...
        print(f"bounded 10MHz auxiliary assembly watchdog: owned PID={process.pid},{EXTERNAL_SECONDS}s/{MEMORY_GIB}GiB", flush=True)

"""Solve the pinned L02/L14/L25 1 MHz mixed operator with bounded block GMRES.
...
three expanded sheets.  A field checkpoint is written before physical gates.
"""
...
def run(output: Path) -> dict:
    ...
    native = slice(0, NATIVE_SIZE-1)
    l14 = slice(NATIVE_SIZE-1, L14_SIZE-1)
    l25 = slice(L14_SIZE-1, L25_SIZE-1)
    l02 = slice(L25_SIZE-1, POTENTIAL_SIZE-1)
    currents = slice(potential_kept, total_kept)
    slices = {"native": native, "l14": l14, "l25": l25, "l02": l02,
              "currents": currents}
    partition = {
        "native_potential_count": native.stop-native.start,
        "l14_potential_count": l14.stop-l14.start,
        "l25_potential_count": l25.stop-l25.start,
        "l02_potential_count": l02.stop-l02.start,
        "current_count": CURRENT_SIZE,
        "total_kept_count": total_kept,
        "native_y_nnz": int(y_kept[native, native].nnz),
        "l14_y_nnz": int(y_kept[l14, l14].nnz),
        "l25_y_nnz": int(y_kept[l25, l25].nnz),
        "l02_y_nnz": int(y_kept[l02, l02].nnz),
    ...
    require(sum(partition[name] for name in ("native_potential_count", "l14_potential_count",
            ...
            == potential_kept and partition["native_b_nnz"] == 1
            and partition["l25_b_nnz"] == incidence.nnz-1,
    ...
    y25 = scaled_block(y_kept[l25, l25], potential_scale[l25], potential_scale[l25])
    ...
    for name, block in (("l02", l02), ("native", native), ("l14", l14)):
        local = scaled_block(y_kept[block, block], potential_scale[block], potential_scale[block])
    ...
    def base_apply(vector: np.ndarray) -> np.ndarray:
        ...
        result[l25] = local[:l25.stop-l25.start]
        result[currents] = local[l25.stop-l25.start:]
        return result
    ...
    coarse_report = {
        "mode_names": ["l14_appended_potential_constant", "l25_appended_potential_constant",
                       "l02_appended_potential_constant"],
        ...
        "native_harmonic_relative_denominator":
            "norm(native action from appended constant)+norm(native action from native correction)",
    ...
    with (np.load(PINS["l25_field"][0], allow_pickle=False) as l25_field,
          np.load(PINS["l02_field"][0], allow_pickle=False) as l02_field):
        prior_v25 = np.asarray(l25_field["active_voltage_v"], dtype=np.complex128)
        prior_q25 = np.asarray(l25_field["l25_branch_current_a"], dtype=np.complex128)
        prior_v02 = np.asarray(l02_field["active_voltage"], dtype=np.complex128)
        l02_sheet_active = np.asarray(l02_field["sheet_active_indices"], dtype=np.int64)
        l02_drive = (
            int(np.asarray(l02_field["source_positive_active_index"]).item()),
            int(np.asarray(l02_field["source_negative_active_index"]).item()),
            int(np.asarray(l02_field["source_gauge_active_index"]).item()),
        )
    require(prior_v25.shape == (L25_SIZE,) and prior_q25.shape == (CURRENT_SIZE,)
            and prior_v02.shape == (1_613_662,), "warm-start field dimensions differ")
    require(l02_drive == (POSITIVE, NEGATIVE, GAUGE)
            and l02_sheet_active.shape == (856_774,)
            and l02_sheet_active[0] == 349_710
            and np.array_equal(l02_sheet_active[1:],
                               np.arange(NATIVE_SIZE, 1_613_662, dtype=np.int64)),
            "L02 warm-start drive/shared-anchor coordinate contract differs")
    warm_v = np.empty(POTENTIAL_SIZE, dtype=np.complex128)
    warm_v[:L25_SIZE] = prior_v25
    warm_v[L25_SIZE:] = prior_v25[349_710] + (prior_v02[NATIVE_SIZE:] - prior_v02[349_710])
    warm_v -= warm_v[GAUGE]
    warm_physical = np.r_[warm_v[1:], prior_q25]
    ...
    require(np.all(np.isfinite(warm_scaled)) and np.isfinite(initial_relative),
            "stitched accepted-field warm start is invalid")
    events.emit("gmres_start", initial_scaled_residual_relative=initial_relative,
                restart=25, maxiter=4, inner_iteration_limit=100)

    history = []
    def callback(value):
        history.append(float(value))
        if len(history) == 1 or len(history) % 10 == 0:
            events.emit("gmres_iteration", inner_iteration=len(history),
                        preconditioned_residual=float(value))

    operator = LinearOperator((total_kept, total_kept), matvec=operator_apply,
                              dtype=np.complex128)
    preconditioner = LinearOperator((total_kept, total_kept), matvec=preconditioner_apply,
                                    dtype=np.complex128)
    tick = time.perf_counter()
    solved, info = gmres(operator, scaled_rhs, M=preconditioner, x0=warm_scaled,
                         callback=callback, callback_type="pr_norm", restart=25,
                         maxiter=4, rtol=1e-9, atol=0)
    gmres_elapsed = time.perf_counter()-tick
    ...
    atomic_npz(checkpoint, active_voltage_v=voltage, l25_branch_current_a=current,
               ...
               warm_start_l02_field_sha256_utf8=np.frombuffer(PINS["l02_field"][1].encode("ascii"), dtype=np.uint8),
               warm_start_l25_field_sha256_utf8=np.frombuffer(PINS["l25_field"][1].encode("ascii"), dtype=np.uint8),
    ...
    atomic_json(output / "unvalidated-field.json", {
        ...
        "status": "UNVALIDATED_COMBINED_BLOCK_GMRES_FIELD_BEFORE_PHYSICAL_GATES",
        "field": checkpoint_receipt, "inputs": inputs,
        "checkpoint_stage": "after GMRES return; before explicit source-category action, KCL, constitutive, passivity, power and backward-error gates",
        "gmres_info_before_physical_gates": int(info),
        "inner_iterations_before_physical_gates": len(history),
    })
    ...
    diagnostic = {
        "program": PROGRAM, "version": VERSION,
        "status": "UNVALIDATED_COMBINED_BLOCK_GMRES_FIELD_BEFORE_GATES",
        ...
        "gmres": {"info": int(info), "restart": 25, "maxiter": 4,
                  "inner_iteration_limit": 100, "inner_iterations": len(history),
                  "residual_history": history, "elapsed_s": gmres_elapsed,
    ...
    events.emit("unvalidated_field_saved", info=int(info), inner_iterations=len(history),
    ...
    require(np.all(np.isfinite(voltage)) and np.all(np.isfinite(current)),
            "combined GMRES field is nonfinite")
    ...
    require(backward <= 1e-9 and finite_action_relative <= 2e-12
            and source_action_relative <= 2e-12 and source_power_relative <= 2e-12,
    ...
    result = {
        "program": PROGRAM, "version": VERSION,
        "status": "COMPLETED_CONDITIONAL_L02_L14_L25_BLOCK_GMRES_1MHZ",
        ...
        "gmres": diagnostic["gmres"], "physical": diagnostic["physical"],
        ...
        "elapsed_s": time.perf_counter()-started,
        "scope": "One conditional 1MHz field of the pinned combined L02/L14/L25 Y/R/B operator using four exact diagonal-block factors only as a GMRES preconditioner and three balanced relative-sheet constant modes. The full mixed operator is not factorized.",
        "limitations": [
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path,
                        default=R / "astra-l02-l14-l25-combined-block-gmres-01")
    args = parser.parse_args()
    try:
        result = run(args.output.resolve())
        print(json.dumps({"status": result["status"], "gmres": result["gmres"],
        ...
        if args.output.exists() and not (args.output / "failure.json").exists():
            ...
            atomic_json(args.output / "failure.json", {
                "program": PROGRAM, "version": VERSION,
                "status": "STOP_CONDITIONAL_L02_L14_L25_BLOCK_GMRES_1MHZ",

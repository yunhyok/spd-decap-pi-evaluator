"""SPD Decap PI Evaluator v0.23.1: disabled saved-field LGMRES24x4 right-LGMRES research probe."""
import argparse
import ctypes
...
PINS.update({
 ...
 'predecessor_driver': (R/'astra-l04-hybrid-aux-restart48-continuation-01/driver-at-run.py', 'e84ba3e10e775248f6a87c412e188410b2dd0beedb8faecc93d899725a23753e'),
 'predecessor_result': (R/'astra-l04-hybrid-aux-restart48-continuation-01/result.json', '94c7740cc85eeed9f87e8785db435226b1f5fc8fd40c2a079a5f7438e87df945'),
 'predecessor_external': (R/'astra-l04-hybrid-aux-restart48-continuation-01/external-budget.json', 'a72299e3ff0913b817b9351a313dab2083507b129467a4ce75b2547d18d03ebd'),
 'warm_field': (R/'astra-l04-hybrid-aux-restart48-continuation-01/restart-2-unvalidated-field.npz', 'f27c64d0cbad6bf34b6356ec4467b6ca5829c09a9d6af83cd6e6fd78a40f5222'),
})
...
def self_check():
 ...
 print('PASS_RIGHT_LGMRES_RECOVERY_AND_COMPLETED_CALLBACKS')
...
def verify_inputs():
 ...
 assert result['best_relative']==9.537136237854355e-6 and result['rtol']==RTOL
...
def worker(output):
 ...
 try:
  ...
  # Reproduction tolerance only: absolute allowance is 1% of the unchanged solve target.
  assert np.isclose(initial_relative,9.537136237854355e-6,rtol=1e-8,atol=RTOL*.01)
  ...
  def checkpoint(z):
   ...
   base.atomic_json(output/'restart-history.json',dict(program=PROGRAM,version=VERSION,
                    status='UNVALIDATED_RIGHT_LGMRES_RESTARTS',driver=receipt(frozen),history=history))
  ...
  result=dict(program=PROGRAM,version=VERSION,status='MEASURED_BOUNDED_HYBRID_AUX_RIGHT_LGMRES',
              ...
              scope='At most four inner24/outer3 right-LGMRES cycles from the saved best field. '
  ...
  base.atomic_json(output/'failure.json',dict(program=PROGRAM,version=VERSION,status='STOP_BOUNDED_HYBRID_AUX_RIGHT_LGMRES',
...
def launch(output):
 ...
 with subprocess.Popen(command,stdin=subprocess.DEVNULL,creationflags=subprocess.CREATE_NO_WINDOW) as child:
  print(f'bounded right LGMRES guard: owned PID={child.pid},{EXTERNAL_SECONDS}s/32GiB',flush=True)
  try:

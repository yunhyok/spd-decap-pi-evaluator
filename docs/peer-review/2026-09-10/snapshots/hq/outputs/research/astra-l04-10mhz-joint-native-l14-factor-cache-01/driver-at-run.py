"""SPD Decap PI Evaluator v0.23.1: disabled 10MHz joint native+L14+L04 fixed-factor cache probe."""
...
RUN_RELEASED = True
SIZE, SECONDS, MEMORY_GIB, EXTERNAL_SECONDS = 2_602_745, 210.0, 24.0, 240.0
PINS = {
    'correction_helper': (Path(correction.__file__), '349abe08fbacbf1ad5b64061abb2514ec78ffb0b6f4f3673a9a358f23697a101'),
    'static_result': (R/'astra-l04-frequency-joint-native-l14-auxiliary-10mhz-01/result.json', '20aa09b4a62e4c06d28e3d7ec8e37fc34217e0b2bce3f77ea56df354c86cb835'),
    'static': (R/'astra-l04-frequency-joint-native-l14-auxiliary-10mhz-01/joint-native-l14-l04-hybrid-aux-block.npz', 'babe65c41ad7e33eb6752e96f5209994adb348105c6d7552793abbe88d26c4b6'),
    'static_driver': (R/'astra-l04-frequency-joint-native-l14-auxiliary-10mhz-01/driver-at-run.py', '1e15c3d4dd873c0179143c4926cc8c11fc3a9fecd8f0fa7d745e29793ec2b166'),
    'static_external': (R/'astra-l04-frequency-joint-native-l14-auxiliary-10mhz-01/external-budget.json', 'a905f3ef630905329bbd117d3e2ebf9a4781aa70b9be5ec315b82936e37e18c7'),
    'static_hq_check': (R/'astra-l04-frequency-joint-native-l14-auxiliary-10mhz-01/hq-block-check.json', '91b766a2ddbb60c65530009f151730734ab60965a12a48db45be8abe1a01cb10'),
...
def verify_released_inputs() -> dict:
    ...
    assert result['status'] == 'PASS_STATIC_L04_HYBRID_AUX_JOINT_NATIVE_L14_BLOCK_NO_FACTOR'
...
def worker(output: Path) -> None:
    ...
    try:
        ...
        with np.load(PINS["static"][0], allow_pickle=False) as archive:
            ...
            joint = archive['joint_native_l14_gauged_potential_indices']
            native = archive['native_gauged_potential_indices']; l14 = archive['l14_gauged_potential_indices']
            assert np.array_equal(joint, np.r_[native,l14]) and len(joint) == 903942
            assert np.array_equal(archive['frequency_hz'], [1e7]) and np.array_equal(archive['source_current_a'], [1.])
        assert matrix.shape == (SIZE, SIZE) and matrix.nnz == 12225369 and scale.shape == (SIZE,)
        ...
        started = perf_counter(); raw_reference_normal = factor.solve(rhs); raw_reference_normal_seconds = perf_counter()-started
        started = perf_counter(); raw_reference_transpose = factor.solve(rhs, trans="T"); raw_reference_transpose_seconds = perf_counter()-started
        started = perf_counter(); reference_normal = correction.corrected_solve(scaled, rhs, factor.solve); reference_normal_seconds = perf_counter() - started
        started = perf_counter(); reference_transpose = correction.corrected_solve(scaled.T, rhs, lambda value: factor.solve(value, trans="T")); reference_transpose_seconds = perf_counter() - started
        ...
        started = perf_counter(); raw_public_normal = compact.compact_solve(lower, upper, perm_r, perm_c, rhs); raw_public_normal_seconds = perf_counter()-started
        started = perf_counter(); raw_public_transpose = compact.compact_solve(lower, upper, perm_r, perm_c, rhs, transpose=True); raw_public_transpose_seconds = perf_counter()-started
        started = perf_counter(); public_normal = correction.corrected_solve(scaled, rhs, lambda value: compact.compact_solve(lower, upper, perm_r, perm_c, value)); public_normal_seconds = perf_counter() - started
        started = perf_counter(); public_transpose = correction.corrected_solve(scaled.T, rhs, lambda value: compact.compact_solve(lower, upper, perm_r, perm_c, value, transpose=True)); public_transpose_seconds = perf_counter() - started
        artifact = output / "compact-factor-probe.npz"
        base.atomic_npz(artifact, raw_reference_normal=raw_reference_normal, raw_reference_transpose=raw_reference_transpose, raw_public_normal=raw_public_normal, raw_public_transpose=raw_public_transpose, reference_normal=reference_normal, reference_transpose=reference_transpose, public_normal=public_normal, public_transpose=public_transpose, perm_r=perm_r, perm_c=perm_c)
        ...
        metrics = {"policy": POLICY, "rhs_count": 2, "fixed_residual_corrections": 1, "factor_seconds": factor_seconds, "L_nnz": int(lower.nnz), "U_nnz": int(upper.nnz), "L_storage_bytes": compact.storage(lower), "U_storage_bytes": compact.storage(upper), "permutation_storage_bytes": int(perm_r.nbytes + perm_c.nbytes), "pivot_finite": bool(np.all(np.isfinite(pivots))), "pivot_nonzero": bool(np.all(abs(pivots) > 0)), "reference_normal_seconds": reference_normal_seconds, "reference_transpose_seconds": reference_transpose_seconds, "public_normal_seconds": public_normal_seconds, "public_transpose_seconds": public_transpose_seconds, "normal_parity_relative": compact.finite_or_none(normal_parity), "ordinary_transpose_parity_relative": compact.finite_or_none(transpose_parity), "normal_scaled_residual_relative": compact.finite_or_none(normal_residual), "ordinary_transpose_scaled_residual_relative": compact.finite_or_none(transpose_residual), "n_parity_gate_lte_2e_8": bool(np.isfinite(normal_parity) and normal_parity <= 2e-8), "t_parity_gate_lte_2e_8": bool(np.isfinite(transpose_parity) and transpose_parity <= 2e-8), "n_residual_gate_lte_2e_8": bool(np.isfinite(normal_residual) and normal_residual <= 2e-8), "t_residual_gate_lte_2e_8": bool(np.isfinite(transpose_residual) and transpose_residual <= 2e-8), "physical_operator_replacement_accepted": False, "preconditioner_candidate_only": True}
        raw_metrics = dict(reference_normal_relative_residual=compact.rel(scaled@raw_reference_normal,rhs),
                           reference_transpose_relative_residual=compact.rel(scaled.T@raw_reference_transpose,rhs),
                           public_normal_relative_residual=compact.rel(scaled@raw_public_normal,rhs),
                           public_transpose_relative_residual=compact.rel(scaled.T@raw_public_transpose,rhs),
                           n_parity_relative=compact.rel(raw_public_normal,raw_reference_normal),
                           t_parity_relative=compact.rel(raw_public_transpose,raw_reference_transpose),
                           reference_normal_seconds=raw_reference_normal_seconds,reference_transpose_seconds=raw_reference_transpose_seconds,
                           public_normal_seconds=raw_public_normal_seconds,public_transpose_seconds=raw_public_transpose_seconds)
        raw_metrics['n_residual_gate_lte_2e_8'] = bool(raw_metrics['public_normal_relative_residual']<=2e-8)
        raw_metrics['t_residual_gate_lte_2e_8'] = bool(raw_metrics['public_transpose_relative_residual']<=2e-8)
        metrics['raw_inverse_before_fixed_correction'] = raw_metrics
        ...
        base.atomic_json(diagnostic, {"program": PROGRAM, "version": VERSION, "status": "MEASURED_FIXED_CORRECTION_HYBRID_AUX_FACTOR_PREACCEPTANCE", "driver": receipt(frozen), "inputs": inputs, "artifact": receipt(artifact), "metrics": metrics, "budget": budget.receipt()})
        ...
        # Persist the measured public factors so the following residual probe need not factor again.
        cache = output / 'qualified-public-factors.npz'
        temporary = cache.with_suffix('.npz.tmp')
        arrays = dict(perm_r=perm_r, perm_c=perm_c, diagonal_scale=scale,
                      joint_native_l14_gauged_potential_indices=joint,
                      frequency_hz=np.array([1e7]), source_current_a=np.array([1.]),
                      fixed_residual_corrections=np.array([1]),
                      static_artifact_sha256_utf8=np.array([PINS['static'][1]], dtype='U64'))
        for label, value in (('lower',lower), ('upper',upper)):
            arrays.update({label+'_data':value.data, label+'_indices':value.indices,
                           label+'_indptr':value.indptr, label+'_shape':np.array(value.shape)})
        with temporary.open('xb') as handle:
            np.savez(handle, **arrays)
        temporary.replace(cache)
        budget.check('qualified public factor cache saved')
        base.atomic_json(output / "result.json", {"program": PROGRAM, "version": VERSION, "status": "PASS_CACHED_10MHZ_JOINT_NATIVE_L14_L04_FIXED_CORRECTION_FACTOR", "physical_operator_replacement_accepted": False, "preconditioner_candidate_only": True, "driver": receipt(frozen), "inputs": inputs, "artifact": receipt(artifact), "public_factor_cache": receipt(cache), "frequency_hz": 1e7, "source_current_a": 1., "preserved_failed_gates": ["raw_hlambda_minus_direct_relative"], "diagnostic": receipt(diagnostic), "metrics": metrics, "budget": budget.receipt(), "scope": "One fixed residual correction B1=2B-BAB, with original 2e-8 gates. Auxiliary-only 10MHz native+L14+L04 hybrid factor; physical trace equivalence remains failed. Exact ContactNtD remains the true operator; no global solve, field, new Z, or accuracy claim."})
    except BaseException:
        base.atomic_json(output / "failure.json", {"program": PROGRAM, "version": VERSION, "status": "STOP_FIXED_CORRECTION_HYBRID_AUX_FACTOR", "traceback": traceback.format_exc(), "budget": budget.receipt()})
        raise

...
FULL_PROGRESS_CAP = 1.1171139319683658
GAP_PROGRESS_CAP_OHM = 0.00016061933005263093
...
def preflight():
    ...
    return {"program": PROGRAM, "version": VERSION,
            ...
            "run_released": RUN_RELEASED, "inputs": inputs, "third_helper_preflight": chain,
...
def worker(output):
    ...
    try:
        ...
        def m_apply(residual):
            ...
            inherited, _ = joint.preflight(); action = joint.ntd.load_action(inherited["qualified_stream_result"])
            assert np.array_equal(action.h_scale, sh)
            dpsi = -action.solve_h(residual[TOTAL:] / sh)
            h_dpsi = action.ct_apply(action.resistance @ action.c_apply(dpsi))
            h_relative = two.relative(h_dpsi, -residual[TOTAL:] / sh); answer = np.r_[base, dpsi / sh]
            record = {"index": counts["M"], "metrics": metrics, "inverse_gate": bool(inverse_gate),
                      "identity_gate": bool(identity_gate), "h_rhs_relative": h_relative,
            ...
            np.savez_compressed(artifact, input_residual=residual, direction=answer,
                                base_direction_scaled=base, psi_direction_physical=dpsi)
            record["artifact"] = receipt(artifact); save(sub / "receipt.json", record)
            del action; gc.collect(); record["rss_after_h_release"] = joint.ntd.recon._rss_bytes()
            m_records.append(record)
            assert inverse_gate and identity_gate and h_relative <= 2e-8 and record["finite"]
            assert counts["B1"] <= 14 and counts["Q"] <= 12 and counts["R"] <= 2
            assert record["rss_after_h_release"] <= 8 * 2**30

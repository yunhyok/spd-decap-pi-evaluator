"""SPD Decap PI Evaluator v0.23.1: disabled two-cycle right-GMRES research probe."""
import argparse
import ctypes
...
INTERNAL_SECONDS, EXTERNAL_SECONDS, MEMORY_GIB = 390., 420., 32.
RTOL, RESTART, MAX_CYCLES = 1e-9, 12, 2
PINS = dict(measured.PINS)
PINS.update({
 'measured_helper': (Path(measured.__file__), '0ee80a0d19ec1efc8a1480f742d7948840e9afae41a96e274e5c391d34654c1b'),
 'one_action_result': (R/'astra-l04-hybrid-aux-native-preflight-01/result.json', '73b2ae82f542f59fd610f2a6daab1605fab52d32cb6e722b7127186701c10933'),
 'one_action_external': (R/'astra-l04-hybrid-aux-native-preflight-01/external-budget.json', '8db1f23390001a894748f9a2121c770d623edfa265dab9c9a7aa304d8134cc7e'),
 'one_action_vectors': (R/'astra-l04-hybrid-aux-native-preflight-01/unvalidated-one-step-action.npz', '5b33db6339b2ec1796c3d20db527d2d66579d8b821753ab699d376460e86f83e'),
})
...
def self_check():
 ...
 print('PASS_TWO_CYCLE_RIGHT_CORRECTION_RECOVERY')

def verify_inputs():
 inputs={key:receipt(path,digest) for key,(path,digest) in PINS.items()}
 inputs['measured_contract']=measured.verify_inputs()
 result=json.loads(PINS['one_action_result'][0].read_bytes())
 external=json.loads(PINS['one_action_external'][0].read_bytes())
 assert result['status']=='MEASURED_HYBRID_AUX_NATIVE_PRECONDITIONER_ONE_STEP'
 assert result['driver']['sha256']==PINS['measured_helper'][1]
 assert result['measurement_artifact']['sha256']==PINS['one_action_vectors'][1]
 ...
 assert result['measurement']['initial_scaled_residual_relative']==219.8892996994003
 assert np.isclose(result['measurement']['optimized_residual_ratio'],.9416407186932461,rtol=1e-14)
 return inputs
...
def worker(output):
 ...
 try:
  ...
  assert np.isclose(initial_relative,219.8892996994003,rtol=1e-10,atol=0)
  ...
  def checkpoint(z):
   ...
   item=dict(cycle=len(history)+1,relative_true_residual=relative,ratio_to_restart6=relative/initial_relative,
  ...
  result=dict(program=PROGRAM,version=VERSION,status='MEASURED_BOUNDED_HYBRID_AUX_RIGHT_GMRES',
              ...
              best_field=best_field if best_field is not None else receipt(*PINS['restart6']),
              ...
              scope='At most two restart12 cycles of right GMRES from the fixed restart6 field. '

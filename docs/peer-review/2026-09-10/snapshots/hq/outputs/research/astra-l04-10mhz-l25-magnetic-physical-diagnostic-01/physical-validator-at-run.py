...
INTERNAL_SECONDS, EXTERNAL_SECONDS, MEMORY_GIB = 300.0, 330.0, 8.0
...
RECOVERY_DIR = R / 'astra-l04-10mhz-l25-magnetic-checkpoint-recovery-01'
FUTURE_OUTPUT_NAME = 'astra-l04-10mhz-l25-magnetic-physical-diagnostic-01'
RECOVERY_KEYS = {'recovery_result', 'recovery_driver', 'recovery_external', 'recovery_field'}
...
PINS.update({
    ...
    'recovery_result': (RECOVERY_DIR / 'result.json', '8824a89b08d18970f65799241b9b5b8a2309395886c546f9e1f8c7e01719c394'),
    'recovery_driver': (RECOVERY_DIR / 'driver-at-run.py', '783d526bb18610c46385c56504fae6a6098a490bb76feac6fa3ec0859edb6ef4'),
    'recovery_external': (RECOVERY_DIR / 'external-budget.json', '2cad57b4504627775bed45a0aca07ec91d1f30056c808907629a1896e2902fa5'),
    'recovery_field': (RECOVERY_DIR / 'recovered-unvalidated-full-contact-l04-10mhz-field.npz', 'f5def3cb8846f94e2bfc18a6992690eef1e9c54bc1402b86421a3271ffca2ba8'),
...
def pending(include_recovery=True):
    return [name for name, (_, digest) in PINS.items()
            if digest.startswith('PENDING_') and (include_recovery or name not in RECOVERY_KEYS)]

def _assert_pinned_inputs():
    assert not pending(False), pending(False)
    inputs = {k:receipt(p, h) for k,(p,h) in PINS.items() if k not in RECOVERY_KEYS}
...
def _require_recovery_contract(result_path, result_sha256, field_path, field_sha256):
    ...
    assert result['status'] == 'RECOVERED_UNVALIDATED_L25_MAGNETIC_FULL_CONTACT_L04_CHECKPOINT'
    ...
    assert solver['status'] == 'UNVALIDATED'
    assert raw['raw_gcrotmk_info'] == solver['raw_gcrotmk_info']
    assert raw['warm_changed_model_relative'] == solver['warm_changed_model']['relative']
    assert raw['final_changed_model_relative'] == solver['final_relative']
    assert raw['field_changed_model_relative'] == solver['final_relative']
    assert solver['artifact']['sha256'] == result['inputs']['solver_field']['sha256']
    numerical_gate = raw['raw_gcrotmk_info'] == 0 and raw['field_changed_model_relative'] <= 1e-9
    assert solver['actual_numerical_acceptance'] is numerical_gate
    assert result['original_numerical_gate'] == numerical_gate
    magnetic = result['magnetic_provenance']
    assert magnetic['physical_operator_replacement_accepted'] is False
    assert magnetic['physical_validation_performed'] is False
    assert magnetic['model_acceptance'] is False
...
def diagnose_checkpoint(solver_result_path, solver_result_sha256, field_path, field_sha256, output):
    ...
    budget = recon._Budget.create(INTERNAL_SECONDS, MEMORY_GIB); budget.check('full-contact validator start')
    with np.load(field_path, allow_pickle=False) as field:
        ...
        lq25 = np.asarray(field['l25_magnetic_flux_linkage_wb'], dtype=np.complex128)
        assert voltage.shape == (N,) and q25.shape == lq25.shape == (L25_BRANCHES,) and voltage[GAUGE] == 0
        assert np.all(np.isfinite(voltage)) and np.all(np.isfinite(q25)) and np.all(np.isfinite(lq25))
    ...
    r25q = r25@q25; inductive25 = 1j*(2*np.pi*FREQUENCY_HZ)*lq25; constitutive25 = r25q+inductive25-b.T@voltage
    power['l25_rt0_resistance'] = complex(np.vdot(q25, r25q)); power['l04_rt0_resistance'] = l04['joule']
    power['l25_magnetic'] = complex(np.vdot(q25, inductive25))
    ...
    matrix_power = complex(np.conj(np.vdot(voltage, matrix_potential)) + np.vdot(q25, r25q) + np.vdot(q25, inductive25))
    ...
    diagnostic = {'program': PROGRAM, 'version': VERSION,
        'status': 'DIAGNOSTIC_UNVALIDATED_L25_MAGNETIC_FULL_CONTACT_L04_CHECKPOINT',
        ...
        'physical': {**l02_metrics, 'zdd_ohm': _pair(zdd), 'matrix_kcl_max_abs_a': _max_abs(matrix_kcl),
          ...
          'l25_constitutive_max_abs_v': _max_abs(constitutive25), 'q_hermitian_lq_h_a2': _pair(np.vdot(q25, lq25)),
          'l25_magnetic_inductive_power_ohm': _pair(power['l25_magnetic']), 'l04_bq_scatter_max_abs_a': l04['bq_kcl_max_abs_a'], 'l04_bq_divergence_kcl_max_abs_a': l04['bq_divergence_kcl_max_abs_a'],
        ...
        'scope': 'Diagnostic only; no acceptance even if all physical gates pass. The provisional L25 magnetic term uses saved final Lq and excludes global return, full FMM qualification, and quadrature validation. No PSD, accuracy, model, broad-band, mesh, or PowerSI acceptance claim.'}
...
def self_check():
    ...
    assert "'status': 'DIAGNOSTIC_UNVALIDATED_L25_MAGNETIC_FULL_CONTACT_L04_CHECKPOINT'" in current
...
def main():
    ...
    if args.self_check:
        self_check(); return
    if args.preflight:
        inputs = _assert_pinned_inputs()
        print(json.dumps({'status':'PASS_DISABLED_L25_MAGNETIC_PHYSICAL_STATIC_INPUTS',
                          'future_recovery_pins':pending(), 'inputs':inputs}, sort_keys=True)); return

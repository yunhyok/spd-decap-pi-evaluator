...
import qualify_astra_l04_forward_closed_magnetic as qualifier
...
FULL_PROGRESS_CAP = 0.551854476798042
...
FORWARD = RESEARCH / "astra-l04-forward-closed-magnetic-qualifier-01"
PINS = {
    ...
    "forward_source": (Path(qualifier.__file__), "1e0c4afff1b34cba0cb973e9b22fc9b664bbf5aea3f830439ec8562cfc4c7780"),
    "forward_result": (FORWARD / "result.json", "4b617a1f5159d8e011585e65c72887e9886b421553456f3787df03787fc0ead5"),
    "forward_guard": (FORWARD / "external-budget.json", "f926c9c9d6de6aae27ae2ebd2d9b5f2038ce7bc7d225e7bfb6181162cf4e42bc"),
    "forward_artifact": (FORWARD / "forward-closed-first-direction.npz", "b1d1238281260952694481249d81c5f0d60f351f239b91ef71620d2e116654cd"),
}
...
def preflight():
    ...
    forward_result = json.loads(PINS["forward_result"][0].read_bytes())
    forward_guard = json.loads(PINS["forward_guard"][0].read_bytes())
    forward_chain = qualifier.preflight()
    ...
    assert forward_result["artifact"]["sha256"] == PINS["forward_artifact"][1]
    assert forward_result["qualified"] and all(forward_result["gates"].values())
    assert forward_result["original_numerical_gate"] is False
    assert forward_guard["status"] == "COMPLETED_NATIVE_WORKER" and forward_guard["exit_code"] == 0
    assert forward_guard["driver_sha256"] == qualifier.PINS["guard_helper"][1]
    assert forward_result["driver"]["sha256"] == PINS["forward_source"][1]
    return {"program": PROGRAM, "version": VERSION,
            "status": "PASS_HELD_COMPLETE_CURRENT_GCROTMK_PREFLIGHT",
            "run_released": RUN_RELEASED, "inputs": inputs, "third_helper_preflight": chain, "auxiliary_preflight": aux_chain, "forward_qualifier_preflight": forward_chain,
...
def worker(output):
    ...
    try:
        ...
        def m_apply(residual):
            ...
            del resources; gc.collect(); rss_after_base = joint.ntd.recon._rss_bytes(); phase_started = time.monotonic()
            action = joint.ntd.load_action(provenance["auxiliary_preflight"]["qualified_stream_result"])
            assert np.array_equal(action.h_scale, sh); dg = sc * base[two.legacy.NX:]; field = action.apply(dg, return_field=True)
            pg_relative = two.relative(action.b_apply(field.branch_current_a), field.target_bq_a)
            pg_gates = {"finite": bool(np.isfinite(field.branch_current_a).all()), "kcl": bool(pg_relative <= 1e-7),
                        "stationarity": bool(field.metrics["energy_scaled_stationarity_relative"] <= 1e-7), "dual": bool(field.metrics["dual_rq_relative"] <= 1e-7)}
            lself = aux.load_lself(action); cross = -1j*aux.OMEGA*sh*action.ct_apply(lself @ field.branch_current_a)
            p_metrics = {"pg_kcl_relative": pg_relative, "cross_norm": float(np.linalg.norm(cross)), "phase_seconds": time.monotonic()-phase_started, "gates": pg_gates}
            assert all(pg_gates.values()) and np.isfinite(cross).all(); del lself, field, action; gc.collect()
            ...
            effective_rhs = residual[TOTAL:] - cross; dpsi_scaled = qualifier.forward_solve(factor, residual[TOTAL:], cross); dpsi = sh * dpsi_scaled
            closed_rhs_norm = max(float(np.linalg.norm(effective_rhs)), np.finfo(float).tiny)
            homega_relative = float(np.linalg.norm(effective_rhs+scaled @ dpsi_scaled) /
                                     closed_rhs_norm)
            answer = np.r_[base, dpsi_scaled]
            record = {"index": counts["M"], "metrics": metrics, "inverse_gate": bool(inverse_gate),
                      "identity_gate": bool(identity_gate), "homega_rhs_relative": homega_relative, "p_metrics": p_metrics,
            ...
            np.savez_compressed(artifact, input_residual=residual, direction=answer,
                                ...
                                contact_direction_a=dg, forward_cross=cross, effective_closed_rhs=effective_rhs)
        ...
        result = {"program": PROGRAM, "version": VERSION,
            ...
            "closed_auxiliary": {"operator": "H+jw*C.T*L04self*C",
                "scaled_rhs_rule": "dpsi_scaled=-factor.solve(r_closed_scaled-forward_cross_scaled); no extra D on the RHS",
                "forward_cross_scaled": "-jw*Dpsi*C.T*L04self*P(Dg*z_g)",
                "one_nonclosed_m_per_application": True,
                "contact_feedback_left_to_flexible_outer": True,
                "fresh_real_h_p_action_per_m_application": True,

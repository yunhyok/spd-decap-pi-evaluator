...
RUN_RELEASED = True
OUTPUT_NAME = 'astra-l04-sheet-aware-native-preflight-02'
...
PINS = {
 ...
 'compact_source': (Path(compact.__file__),'eed904035db4aa0dbcca7c2b85ff8b5d2c0e9b61acfd86527b9d0725e3786acf'),
 'compact_result': (R/'astra-l04-sheet-aware-native-compact-factor-01/result.json','cc764850174f5c6e94b8f1341e62f6dff04d35e87d3e5d26d58ae8a6f2a588b0'),
 'compact_external': (R/'astra-l04-sheet-aware-native-compact-factor-01/external-budget.json','d94ce6e8e29e98ea1a411dd0ffff58cb965024b9c4188eab1c7f26a2cdd3b474'),
 'compact_diagnostic': (R/'astra-l04-sheet-aware-native-compact-factor-01/compact-factor-diagnostic.json','4f0875c78f4579a72bbef6defaeef54f66d69fc16c6469eb1e6e9cd9ba1b0458'),
}
...
def factor_primal(name,matrix,report,budget,diagnostic,frozen,inputs):
 """Use factor01's policy, then retain only owned public L/U/permutations."""
 ...
 lower,upper=f.L.copy(),f.U.copy(); perm_r,perm_c=f.perm_r.copy(),f.perm_c.copy()
 live_rss=rss(); del f; gc.collect(); released_rss=rss()
 started=perf_counter(); compact_x=compact.compact_solve(lower,upper,perm_r,perm_c,probe); compact_apply_seconds=perf_counter()-started
 started=perf_counter(); compact_xt=compact.compact_solve(lower,upper,perm_r,perm_c,probe,True); compact_transpose_seconds=perf_counter()-started
 normal_parity=rel(compact_x,x); transpose_parity=rel(compact_xt,xt)
 compact_normal_residual=rel(matrix@compact_x,probe); compact_transpose_residual=rel(matrix.T@compact_xt,probe)
 finite_pivots=abs(pivots)[np.isfinite(pivots)]; current_rss=rss()
 values={'size':matrix.shape[0],'nnz':int(matrix.nnz),'permc_spec':'MMD_AT_PLUS_A','diag_pivot_thresh':0.,'SymmetricMode':True,
         'factor_seconds':factor_seconds,'L_nnz':int(lower.nnz),'U_nnz':int(upper.nnz),'pivot_finite':finite,'pivot_nonzero':nonzero,
         ...
         'compact_apply_seconds':compact_apply_seconds,'compact_transpose_apply_seconds':compact_transpose_seconds,
         'compact_witness_rhs_count':1,
         'compact_normal_parity_relative':finite_or_none(normal_parity),'compact_transpose_parity_relative':finite_or_none(transpose_parity),
         'compact_normal_residual_relative':finite_or_none(compact_normal_residual),'compact_transpose_residual_relative':finite_or_none(compact_transpose_residual),
         'compact_normal_parity_gate_lte_2e_8':bool(np.isfinite(normal_parity) and normal_parity<=2e-8),
         'compact_transpose_parity_gate_lte_2e_8':bool(np.isfinite(transpose_parity) and transpose_parity<=2e-8),
         'compact_normal_residual_gate_lte_2e_8':bool(np.isfinite(compact_normal_residual) and compact_normal_residual<=2e-8),
         'compact_transpose_residual_gate_lte_2e_8':bool(np.isfinite(compact_transpose_residual) and compact_transpose_residual<=2e-8),
         'L_storage_bytes':compact.storage(lower),'U_storage_bytes':compact.storage(upper),
         'permutation_storage_bytes':int(perm_r.nbytes+perm_c.nbytes),
         'factor_live_max_private_or_working_bytes':live_rss,'post_superlu_delete_max_private_or_working_bytes':released_rss,
         'superlu_delete_delta_bytes':released_rss-live_rss,'max_private_or_working_bytes':current_rss}
 report[name]=values
 ...
 assert (finite and nonzero and values['probe_gate_lte_2e_8'] and values['transpose_gate_lte_2e_8']
         and values['compact_normal_parity_gate_lte_2e_8'] and values['compact_transpose_parity_gate_lte_2e_8']
         and values['compact_normal_residual_gate_lte_2e_8'] and values['compact_transpose_residual_gate_lte_2e_8'])
 calls={'normal_count':0,'normal_seconds':0.,'transpose_count':0,'transpose_seconds':0.}
 def solve(value,trans='N'):
  assert trans in ('N','T')
  started=perf_counter(); result=compact.compact_solve(lower,upper,perm_r,perm_c,value,trans=='T'); elapsed=perf_counter()-started
  key='transpose' if trans=='T' else 'normal'; calls[key+'_count']+=1; calls[key+'_seconds']+=elapsed
  return result
 budget.check(name+' compact factor'); return solve,calls
...
def verify_inputs():
 ...
 assert prior_external['status']=='STOP_EXTERNAL_MEMORY_BUDGET' and prior_external['exit_code']==1
 assert prior_external['driver_sha256']==PINS['preflight01_driver'][1] and prior_external['sampled_peak_private_bytes']==35398475776
 assert prior_diagnostic['status']=='FACTORS_AND_PRIMAL_COARSE_READY_BEFORE_NTD_OR_MEASUREMENT'
 assert prior_diagnostic['rss_bytes']==32182890496 and prior_diagnostic['budget']['peak_rss_bytes']==31350837248
 compact_result=json.loads(PINS['compact_result'][0].read_text()); compact_external=json.loads(PINS['compact_external'][0].read_text()); compact_diagnostic=json.loads(PINS['compact_diagnostic'][0].read_text())
 assert compact.scipy.__version__=='1.18.1'
 assert compact_result['status']=='PASS_COMPACT_PUBLIC_NATIVE_L04_FACTOR_SINGLE_PROBE'
 assert compact_result['driver']['sha256']==PINS['compact_source'][1] and compact_result['diagnostic']['sha256']==PINS['compact_diagnostic'][1]
 assert compact_result['artifact']['sha256']=='ac97411339050e5458d709d88c8cb858341deaa85d479b2faf28f7fa50ca2fa8'
 assert compact_external['status']=='COMPLETED_NATIVE_WORKER' and compact_external['exit_code']==0
 assert compact_diagnostic['status']=='MEASURED_PUBLIC_FACTORS_BEFORE_ACCEPTANCE'
 assert compact_diagnostic['driver']['sha256']==PINS['compact_source'][1]
 compact_metrics=compact_diagnostic['metrics']
 assert compact_metrics['rhs_count']==2 and compact_metrics['pivot_finite'] and compact_metrics['pivot_nonzero']
 assert all(compact_metrics[key] for key in ('n_parity_gate_lte_2e_8','t_parity_gate_lte_2e_8','n_residual_gate_lte_2e_8','t_residual_gate_lte_2e_8'))
 return inputs

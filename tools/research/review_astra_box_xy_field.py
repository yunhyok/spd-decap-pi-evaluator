"""SPD Decap PI Evaluator v0.23.1 saved 124-current q10/q14 review.
...
PINS = {
    ...
    "tools/research/diagnose_astra_box_xy_field.py":
        "de992467be1f33e0d911ca036760844f097c91894e02bc688164c6c1e4350448",
    ...
    "outputs/research/astra-box-xy-q10-field-01/result.json":
        "62092181c37b74aeb63436e57da6beacfafba6db34a25d6a1998b2edc9c0d025",
    "outputs/research/astra-box-xy-q10-field-01/fields.npz":
        "1d83a9115ffbcaae3a19d6b8f4c26e0d83565d1b2a2115643192495d83b85344",
...
def main():
    ...
    assert qualification["frozen_charge_symmetry_qualified"] is False
    charge_covariance = qualification["checks"][
    ...
    expected_artifacts = {
        "q10": {
            "result": PINS["outputs/research/astra-box-xy-q10-field-01/result.json"],
            "fields": PINS["outputs/research/astra-box-xy-q10-field-01/fields.npz"],
            "kernels": PINS[
                "outputs/research/astra-box-xy-symmetry-enrichment-03/kernels-q10.npz"
            ],
        },
        "q14": {
            "result": PINS["outputs/research/astra-box-xy-q14-field-01/result.json"],
            "fields": PINS["outputs/research/astra-box-xy-q14-field-01/fields.npz"],
            "kernels": PINS[
                "outputs/research/astra-box-xy-symmetry-enrichment-03/kernels-q14.npz"
            ],
        },
    ...
    for label in ("q10", "q14"):
        base = ROOT / f"outputs/research/astra-box-xy-{label}-field-01"
        results[label] = json.loads((base / "result.json").read_bytes())
        fields[label] = load_npz(base / "fields.npz")
        kernels[label] = load_npz(
            ROOT / f"outputs/research/astra-box-xy-symmetry-enrichment-03/kernels-{label}.npz"
        )
        result = results[label]
        expected = expected_artifacts[label]
        assert result["status"] == "COMPLETE_BOX_XY_FIELD"
        ...
        assert result["symmetry_helper_sha256"] == qualification["script_sha256"]
    ...
    expected_order = np.r_[np.arange(73), np.arange(120, 124), np.arange(73, 120)]
    assert mass.shape == (124, 124)
    ...
    split = 77
    old_indices = np.argsort(order)[:120]
    new_indices = np.arange(73, 77)
    ...
    with np.load(
        ...
        old_moment_rhs = saved["n48_uniform_curl_rhs"]
    baseline_fields = load_npz(
        ROOT / "outputs/research/astra-box-enriched-q18-field-01/fields.npz"
    )
    baseline_result = json.loads(
        (ROOT / "outputs/research/astra-box-enriched-q18-field-01/result.json")
        .read_bytes()
    )
    ...
    for index, frequency in enumerate(frequencies):
        ...
        case = {
            ...
            "xy_vs_old_bilinear_reaction_relative": relative(
                q14["reaction"],
    ...
    output = ROOT / "outputs/research/astra-box-xy-field-review-01"
    ...
    review = {
        "program": PROGRAM,
        "version": VERSION,
        "status": "ACCEPT_SAVED_XY_Q10_Q14_FIELD_ALGEBRA_WITH_SCOPE",
        "input_sha256": PINS,
        ...
        "gates": gates,
        "scope": (
            "No-solve reconstruction of the saved 124-current q10/q14 systems, "
            ...
            "matrices. The old120 comparison is a synthetic incident-field response, "

...
def run(args):
    ...
    try:
        ...
        if getattr(args, "return_assembly", False):
            # Reuse the verified source assembly in a separately owned magnetic solve.
            return {"y": y, "r": r, "b": b, "gauge": old["gauge"],
                    "positive": old["positive"], "negative": old["negative"],
                    "actions": actions, "field_extra": extra, "assembly": contract,
                    "rail_id": l14.RAIL, "frequency_hz": 1e6,
                    "inputs": {key: {"path": str(path), "sha256": digest} for key, (path, digest) in PINS.items()},
                    "source_l14_inputs": old["inputs"], "dc_step_artifacts": step["artifacts"],
                    "gc_receipt": {"path": str(args.gc_receipt), "sha256": args.gc_receipt_sha256},
                    "driver": base._file_receipt(args.output / "driver-at-run.py")}

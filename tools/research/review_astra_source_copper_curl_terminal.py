"""SPD Decap PI Evaluator v0.23.1: saved 52-variable Cu-curl terminal review."""
...
OUT = ROOT/"outputs/research/astra-source-copper-curl-terminal-01"
REVIEW = ROOT/"outputs/research/astra-source-copper-curl-terminal-review-01"
EPS0 = 8.8541878128e-12
MU0 = 4e-7*pi
PINS = {
    "producer": ("tools/research/diagnose_astra_source_copper_curl_terminal.py", "fa3996de5f1b43b78c1ebdb107a3f8f6db2c6a0d13b10cdcea5ef4f5b01c95fd"),
    "driver": ("outputs/research/astra-source-copper-curl-terminal-01/driver-at-run.py", "fa3996de5f1b43b78c1ebdb107a3f8f6db2c6a0d13b10cdcea5ef4f5b01c95fd"),
    "result": ("outputs/research/astra-source-copper-curl-terminal-01/result.json", "0fd925a78ffe4931b5c363431b16bdbcc267bf37b1e306d7b835cbf309040ef6"),
    "fields": ("outputs/research/astra-source-copper-curl-terminal-01/fields.npz", "7680fb2623c7d59bd3d77767d6e966c9bead34f571537f2ab429dd7bd2b0262d"),
    "space": ("outputs/research/astra-source-copper-curl-space-01/space.npz", "e0e6191c54478c51baed4bc03468aab73ce0ccb17e88c0dec70dad75bcc47ede"),
    "old_fields": ("outputs/research/astra-source-potential-terminal-02/fields.npz", "33262a8b2fad33a718af80e7cf341f9611d65d5fa18e28447107528503b1544d"),
    ...
    "green_result": ("outputs/research/astra-source-copper-curl-green-01/result.json", "1eef9f6606b577293ea284a8b24a3bb8eea6274066360716018e0623025db2b2"),
    "precision": ("outputs/research/astra-source-copper-curl-sensitivity-01/result.json", "56927f86d4ee09b7f316e339f9c1d9298665cdad20b3a15c92b3d3185b7c32bc"),
}
...
def run():
    ...
    assert report["status"] == "STOP_SOURCE_COPPER_CURL_TERMINAL_DIAGNOSTIC"
    ...
    precision = json.loads((ROOT/PINS["precision"][0]).read_text(encoding="utf-8"))
    assert precision["status"] == "PASS_STORED52_PRECISION_WITH_MODE_SENSITIVITY"
    ...
    assert saved["mass"].shape == (42, 42) and saved["regional_mass"].shape == (3, 42, 42)
    assert np.array_equal(saved["mass"], space["mass"])
    assert saved["cell_integrated_current_map"].shape == (18, 3, 42)
    assert saved["noncontact_divergence"].shape == (24, 42) and saved["contact_divergence"].shape == (8, 42)
    ...
    for region in range(3):
        ...
        aa = np.einsum("tdi,tdj,t->ij", old_h[choose], old_h[choose], 1/volumes[choose])
        ab = np.einsum("tdi,tdj,t->ij", old_h[choose], new_h[choose], 1/volumes[choose])
        bb = np.zeros((6, 6))
        if region != 1:
            sl = slice(0, 3) if region == 0 else slice(3, 6)
            bb[sl, sl] = space["mass"][36:, 36:][sl, sl]
    ...
    for fi, frequency in enumerate(frequencies):
        ...
        for tag, (kernel_frequencies, face_green) in kernels.items():
            ...
            modal, qc, common = unknown[:42], unknown[42:50], unknown[50:52]
            ...
            maxima["change_report"] = max(maxima["change_report"], float(np.max(abs(current_change-case["current_mass_change_vs36_relative_new"]))),
                float(np.max(abs(complex_change-case["complex_z_change_vs36_relative_new"]))), float(np.max(abs(resistance_change-case["resistance_change_vs36_relative_new"]))))
    ...
    max_mp_current = max(x["current_mass_vs_float64_relative"] for x in precision["precision"])
    max_mp_z = max(max(x["shorted_z_vs_float64_relative"]) for x in precision["precision"])
    gates = {
        ...
        "separate_mp_forward_check_reported": max_mp_current < 3e-14 and max_mp_z < 3e-15,
    }
    ...
    receipt = {
        ...
        "status": "ACCEPT_SAVED_52_ALGEBRA_EXACT_MASS_WORK__CONFIRM_STOP_INTERFACE",
        ...
        "reported_mpmath60_max_current_relative": max_mp_current, "reported_mpmath60_max_z_relative": max_mp_z,
        "scope": ("Read-only saved-array review: 52-equation residual and frozen36 subblock, independent/contact charges, terminal circuit, "
                  "regional exact polynomial mass loss and RMS, interface own traces, saved body far power, and q14/q20 changes. "
                  "No solve or Green integration was repeated. The separate 60-digit receipt is pinned and quoted, not rerun. "
                  "The algebra/work is accepted while the physical field remains STOP above the 1% interface discriminator. "
                  "Mode-removal percentages are sensitivity, not attribution or convergence. No board-Z or PowerSI accuracy approval.")}
    REVIEW.mkdir()

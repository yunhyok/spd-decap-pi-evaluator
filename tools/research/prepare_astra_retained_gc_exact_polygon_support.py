"""SPD Decap PI Evaluator v0.23.1: bounded retained-GC polygon support.
...
def _island_rule(checkpoints: Path, island: str, geometry):
    ...
    import shapely
    progress = checkpoints / "progress.json"; staged = progress.with_suffix(".tmp")
    staged.write_text(json.dumps({"phase": "triangulating", "island_id": island,
                                  "coordinate_count": int(shapely.get_num_coordinates(geometry)),
                                  "completed_island_rules": len(list(checkpoints.glob("*.npz")))}, sort_keys=True), encoding="utf-8")
    staged.replace(progress)
    xy, weights = triangles(geometry)
    temporary = path.with_suffix(".tmp.npz")
    np.savez_compressed(temporary, xy=xy, weights=weights)
    temporary.replace(path)
    staged.write_text(json.dumps({"phase": "triangulated", "completed_island_rules": len(list(checkpoints.glob("*.npz"))), "last_island_id": island}, sort_keys=True), encoding="utf-8")
    staged.replace(progress)
    return xy, weights
...
def run(output: Path) -> None:
    ...
    checkpoints = output.with_name(output.name + "-checkpoints")
    checkpoints.mkdir(parents=True, exist_ok=True)
    frozen_driver = checkpoints / "driver-at-run.py"; source_bytes = Path(__file__).read_bytes()
    if frozen_driver.exists() and frozen_driver.read_bytes() != source_bytes:
        raise ValueError("checkpoint driver differs from current source")
    if not frozen_driver.exists():
        temporary_driver = frozen_driver.with_suffix(".tmp")
        temporary_driver.write_bytes(source_bytes); temporary_driver.replace(frozen_driver)
    ...
    def support(island, face_z):
        ...
        support_rows.append({"island_id": island, "conductor_face_z_um": face_z, "polygon_wkb_hex": polygons[island].wkb_hex,
    ...
    sums = np.bincount(columns, weights=weights, minlength=len(support_rows))
    if len(owner_rows) and not np.allclose(sums, 1., rtol=0, atol=2e-15):
        raise ValueError("normalized spread failed")
    support_weight = np.bincount(columns, weights=weights, minlength=len(support_rows))
    centroid_xy = (np.column_stack((np.bincount(columns, weights=weights * points[:, 0], minlength=len(support_rows)),
                                    np.bincount(columns, weights=weights * points[:, 1], minlength=len(support_rows)))) / support_weight[:, None]
                   if len(support_rows) else np.empty((0, 2)))
    centroid_error = max((float(np.linalg.norm(centroid_xy[i] - np.asarray(polygons[row["island_id"]].centroid.coords[0]))) for i, row in enumerate(support_rows)), default=0.0)
    ...
    gather = np.bincount(columns, weights=(weights*phi).real, minlength=len(support_rows)) + 1j*np.bincount(columns, weights=(weights*phi).imag, minlength=len(support_rows))
    ...
    np.savez_compressed(artifact, owner_partial_ordinal=np.asarray([r["partial_ordinal"] for r in owner_rows], dtype=np.int16), owner_local_row=np.asarray([r["local_row"] for r in owner_rows], dtype=np.int32), owner_local_col=np.asarray([r["local_col"] for r in owner_rows], dtype=np.int32),
                        ...
                        support_island_id=np.asarray([r["island_id"] for r in support_rows]), support_face_z_um=np.asarray([r["conductor_face_z_um"] for r in support_rows]), support_wkb_hex=np.asarray([r["polygon_wkb_hex"] for r in support_rows]),
    ...
    gates = {"selector_has_31_retained_partials": len(retained) == 31, "all_4284_owners_available": complete,
             ...
             "spread_gather_ordinary_transpose": duality_error < 2e-12, "no_board_solve": True}
    ...
    manifest = dict(program=PROGRAM, version=VERSION, status="PASS_RETAINED_GC_EXACT_POLYGON_SUPPORT_NO_SOLVE" if passed else "PARTIAL_RETAINED_GC_POLYGON_LOOKUP", run_released=RUN_RELEASED,
        driver_sha256=sha(Path(__file__)), artifact_sha256=sha(artifact), retained_partial_ordinals=retained.tolist(),
        ...
        metrics={"spread_normalization_max_abs": float(np.max(abs(sums - 1), initial=0.0)), "support_centroid_first_moment_max_error_um": centroid_error, "spread_gather_transpose_relative": duality_error},
        gates=gates,

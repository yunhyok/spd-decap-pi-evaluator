"""SPD Decap PI Evaluator v0.23.1: disabled 10 MHz flexible-GCROT L04 checkpoint recovery."""
...
FUTURE_OUTPUT_NAME = "astra-l04-10mhz-flexible-gcrotmk-checkpoint-recovery-01"
PINS = {
    "solver_result": (R / "astra-l04-10mhz-flexible-joint-l25-gcrotmk-01/result.json", "c81140d9c31ffd0dd137cfbe385613c26fc79f480ca683ed6e8492d6f78ce015"),
    "solver_driver": (R / "astra-l04-10mhz-flexible-joint-l25-gcrotmk-01/driver-at-run.py", "a53845330850500040a19cae813e2ebe5eaa5621995428d7ed17d60b63608f77"),
    "solver_external": (R / "astra-l04-10mhz-flexible-joint-l25-gcrotmk-01/external-budget.json", "4e113329b2a6fde807c968c3c2f0ddfea519cc2033d0e06edf7f53a0af53a577"),
    "solver_field": (R / "astra-l04-10mhz-flexible-joint-l25-gcrotmk-01/unvalidated-flexible-gcrotmk-field.npz", "643767bd5ade5480431519c89e5c28125a471732647b3b1082bae93dc8744552"),
...
def _solver_metadata(result: dict[str, object]) -> tuple[dict[str, object], int, float]:
    ...
    return artifact, info, float(final)
...
def self_check() -> None:
    ...
    _, info, residual = _solver_metadata(accepted)
...
def _verify_solver() -> tuple[dict[str, object], dict[str, object], int, float, dict[str, object]]:
    ...
    selected, info, true_residual = _solver_metadata(result)
    ...
    return result, selected, info, true_residual, {"solver_result": result_receipt, "solver_driver": driver,
...
def worker(output: Path) -> None:
    ...
    try:
        solver_result, selected, info, true_residual, prior = _verify_solver()
        ...
        assert info == 1 and gate is False
        report = {"program": PROGRAM, "version": VERSION,
                  ...
                  "numerical": {"raw_gcrotmk_info": info,
                                "warm_scaled_residual_relative": solver_result["warm_relative"],
...
def main() -> None:
    ...
    if args.self_check:
        self_check(); return
    if args.preflight:
        result, selected, info, residual, receipts = _verify_solver()
        print(json.dumps({"status": "PASS_DISABLED_FLEXIBLE_GCROTMK_RECOVERY_INPUTS", "raw_gcrotmk_info": info,
                          "field_relative": residual, "original_numerical_gate": original_numerical_gate(info, residual),
                          "receipts": receipts}, sort_keys=True)); return
    assert RUN_RELEASED, "RUN_RELEASED remains false pending HQ review"
    assert not pending(), f"unfilled pins: {pending()}"

...
RECOVERY_DIR = R / 'astra-l04-10mhz-flexible-gcrotmk-checkpoint-recovery-01'
FUTURE_OUTPUT_NAME = 'astra-l04-10mhz-flexible-gcrotmk-physical-diagnostic-01'
...
PINS.update({
    ...
    'recovery_result': (RECOVERY_DIR / 'result.json', '622a4ea1a04765e88341541621f3e6b9547b1e3efd2f64b774e57616dbe3a95b'),
    'recovery_driver': (RECOVERY_DIR / 'driver-at-run.py', '0145c581bc7f7129a904d258eb99cd811c25a6671db89897a3d346cfcd8a4433'),
    'recovery_external': (RECOVERY_DIR / 'external-budget.json', '6226e052a6f2542043c28022239a151fd302e77497bfee819da6cbdb5806f31e'),
    'recovery_field': (RECOVERY_DIR / 'recovered-unvalidated-full-contact-l04-10mhz-field.npz', '49485633921a39f9f2d92123ccd802baffaf5ee861cea6aa563bbe908dd6c7ea'),
...
def _require_recovery_contract(result_path, result_sha256, field_path, field_sha256):
    ...
    assert raw['warm_scaled_residual_relative'] == solver['warm_relative']
    assert raw['final_scaled_residual_relative'] == solver['final_relative']
    assert raw['field_scaled_residual_relative'] == solver['final_relative']
    assert solver['artifact']['sha256'] == result['inputs']['solver_field']['sha256']
    numerical_gate = raw['raw_gcrotmk_info'] == 0 and raw['field_scaled_residual_relative'] <= 1e-9
...
def main():
    ...
    if args.preflight:
        ...
        print(json.dumps({'status':'PASS_DISABLED_FLEXIBLE_GCROTMK_PHYSICAL_STATIC_INPUTS',

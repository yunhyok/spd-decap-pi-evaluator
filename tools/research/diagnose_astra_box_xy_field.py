"""SPD Decap PI Evaluator v0.23.1: four axial-current finite-frequency test.

Keep the frozen120-current block and boundary charge; append two axial currents
and their exact xy mirrors. Frozen charge quadrature is not exactly xy symmetric.
"""
...
def run(output,space_file,space_hash,kernels_file,kernels_hash):
    ...
    assert qualification['status']=='PASS_XY_MAGNETIC_ENRICHMENT'
    assert qualification['space_sha256']==space_hash and kernels_hash in [c['kernels_sha256'] for c in qualification['cases']]
    ...
    assert np.array_equal(order,np.r_[np.arange(73),np.arange(120,124),np.arange(73,120)]) and mass.shape==(124,124)
    old_indices=np.argsort(order)[:120];split=77
    ...
    with np.load(ROOT/'outputs/research/astra-box-polynomial-current-space-01/space.npz',allow_pickle=False) as saved:
        old_order=saved['n48_coordinate_order'];old_moment_rhs=saved['n48_uniform_curl_rhs']
    ...
    with np.load(ROOT/'outputs/research/astra-box-enriched-q18-field-01/fields.npz',allow_pickle=False) as saved:
        ...
        for index,frequency in enumerate(frequencies):
            ...
            amplitude=np.concatenate((old_amp,xy.far_added(dimensions,k,directions)),axis=2)[:,:,order]
            ...
            assert np.count_nonzero(xy.far_added(dimensions,k,np.array([[0.,0.,1.]])))==0
            rhs=np.vstack((saved[f'case_{index:02d}_incident_rhs'],np.zeros((4,2))))[order]
            ...
            baseline_modal=np.vstack((saved[f'case_{index:02d}_modal_current'],np.zeros((4,2))))[order]
    ...
    result=dict(program='SPD Decap PI Evaluator',version='0.23.1',status=('COMPLETE_' if all(gates.values()) else 'STOP_')+'BOX_XY_FIELD',
        pins=pins,space_sha256=space_hash,kernels_sha256=kernels_hash,script_sha256=prior.old.source.sha(Path(__file__)),
        symmetry_helper_sha256=prior.old.source.sha(Path(xy.__file__)),frozen_charge_symmetry_qualified=False,fields_sha256=prior.old.source.sha(output/'fields.npz'),
        gates=gates,cases=cases,elapsed_s=monotonic()-start,
        scope='Frozen120-current block plus two axial currents and their exact xy mirrors,124 total with77closed and47unchanged charge ranges. Magnetic columns reuse qualified current-space symmetry; radiation is computed directly from original and mirrored Fourier amplitudes. Same material, static/degree8 real kernels, and unchanged frozen charge matrix. That charge matrix has a2.73e-6 mass-scaled covariance defect and full-operator symmetry is not certified. Current, loss and magnetic dipole changes diagnose selected trial-space enrichment, not port Z, full3D/broadband convergence or board/PowerSI accuracy.')
...
if __name__=='__main__':
    ...
    try:code=run(destination,args.space.resolve(),args.space_sha256,args.kernels.resolve(),args.kernels_sha256)
        ...
        (destination/'failure.json').write_text(json.dumps(dict(status='FAILED_XY_FIELD',error=repr(error)),indent=2),encoding='utf-8')
        raise

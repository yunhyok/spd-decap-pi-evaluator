...
def run(output):
    ...
    for fi,frequency in enumerate(frequencies):
        ...
        magnetic=old.project(h,kv+tv[fi]);p=(ps+pt[fi])/(4*np.pi*old.source.EPS0)
    ...
    result=dict(program='SPD Decap PI Evaluator',version='0.23.1',status=('PASS_' if all(gates.values()) else 'STOP_')+'POTENTIAL_TERMINAL_DIAGNOSTIC',

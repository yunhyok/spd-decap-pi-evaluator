...
def _validate_spooled_rows(connection: sqlite3.Connection, binding: Mapping[str, Any], cancelled: Any) -> None:
    ...
    if connection.execute("SELECT 1 FROM terminal_bindings t LEFT JOIN rail_bindings b ON lower(t.rail_id)=lower(b.rail_id) AND lower(t.role)=lower(b.role) LEFT JOIN _ir_records n ON lower(t.source_node_record_id)=n.record_fold LEFT JOIN _ir_records v ON lower(t.via_record_id)=v.record_fold LEFT JOIN _ir_islands i ON lower(t.island_id)=i.island_fold WHERE t.via_record_required<>1 OR t.status<>'complete' OR b.rail_id IS NULL OR n.record_fold IS NULL OR lower(n.kind)<>'node' OR v.record_fold IS NULL OR lower(v.kind)<>'via' OR i.island_fold IS NULL OR lower(t.island_id)<>lower(b.island_id) OR lower(t.rail_id)<>lower(?) OR t.source_node_record_id='' OR t.via_owner_id='' OR t.finite_edge_id='' OR t.raw_pad_shape_ordinal IS NULL OR t.raw_pad_shape_sha256 IS NULL", (binding["target_rail_id"],)).fetchone() is not None:
    ...
    if connection.execute("SELECT 1 FROM terminal_bindings t LEFT JOIN _ir_owners o ON lower(t.via_owner_id)=o.owner_fold WHERE o.owner_fold IS NULL OR lower(o.owner_kind)<>'via' OR o.edge_id IS NULL OR o.rail_id IS NULL OR o.island_fold IS NULL OR lower(o.edge_id)<>lower(t.finite_edge_id) OR lower(o.rail_id)<>lower(t.rail_id) OR lower(o.island_fold)<>lower(t.island_id)").fetchone() is not None:
        _fail("SOURCE_PLANE_OWNERSHIP_IR_TERMINAL_INCOMPLETE", "terminal via owner relation is invalid")
    terminal_roles = connection.execute("SELECT COUNT(*),SUM(role='power'),SUM(role='ground') FROM terminal_bindings WHERE lower(rail_id)=lower(?)", (binding["target_rail_id"],)).fetchone()
    if connection.execute("SELECT 1 FROM terminal_bindings GROUP BY lower(rail_id),lower(branch_id),lower(pin_id),lower(role) HAVING COUNT(*)<>1").fetchone() is not None or terminal_roles != (2, 1, 1):

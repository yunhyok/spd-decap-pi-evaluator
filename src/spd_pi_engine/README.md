...
- 게이트 G4의 f_res 항(D10, 2026-09-20 정정): `ladder_gates`가 모델 f_res를 **그 케이스의 참조 f_res**(`G4_f_res_rel_err_vs_ref < 0.10`)와 비교한다. 정정 전에는 exp3에서 넘어온 상수 1.585 MHz(260729 Port18의 참조값)와 비교해 다른 레일에서는 G4가 틀린 이유로 실패했다(`docs/engine/W14D_REPORT.md`, W14B §10-6, `DECISIONS.md` D10). **이 커밋 이전에 만든 영수증에는 옛 키 `G4_f_res_rel_err_vs_1.585MHz`만 있다**(새 영수증은 둘 다 기록하되 판정은 새 키로 한다). 수치는 불변(`numerics_id` 27d81996…).

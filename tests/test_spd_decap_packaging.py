...
def test_spd_decap_release_identity_is_explicit_and_versioned() -> None:
    ...
    assert APP_DISPLAY_NAME == "SPD Decap PI Evaluator v0.22.1"
    assert EXECUTABLE_BASENAME == "SPDDecapPIEvaluator"
    assert INSTALLER_BASENAME == "SPDDecapPIEvaluatorSetup-0.22.1"
...
def test_windows_version_resource_matches_release_identity() -> None:
    ...
    assert "filevers=(0, 22, 1, 0)" in version_info
    assert "prodvers=(0, 22, 1, 0)" in version_info
    assert "StringStruct('FileVersion', '0.22.1')" in version_info
    assert "StringStruct('ProductVersion', '0.22.1')" in version_info

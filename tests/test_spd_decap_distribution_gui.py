...
def test_physical_landing_retarget_has_no_obsolete_column_warning() -> None:
    ...
    try:
        ...
        assert "SOURCE-PROVEN VIA TRANSITION REQUIRED" in summary
        assert "missing transition evidence is excluded fail-closed" in summary
        assert (
            "fulfillment > minimum isolation gaps > minimum PWR NET relabels"
            in summary
        )
    finally:
        window._dirty = False
...
def test_full_preview_exports_saves_and_applies_one_atomic_revision(
    ...
    try:
        ...
        try:
            ...
            assert (
                metadata["Eligibility Policy"]
                == "SOURCE_PROVEN_TARGET_LAYER_TRANSITION_V1"
            )
            assert (
                metadata["Planner Priority"]
                == "FULFILLMENT > ISOLATION_GAPS > RELABELS > DISTANCE"
            )
            assert "Planner Diagnostics" in metadata
...
def test_applied_distribution_reloads_clear_into_layerwise_evaluation_workers(
    ...
    for refdes, connection in source.connection_analysis.connections.items():
                ...
                landing.model_copy(
                    update={
                        "path_evidence": (
                            ScenarioViaPathEvidence(
                                target_layer="PWR1",
                                target_node_id=f"{landing.endpoint_node_id}-PWR1",
        ...
        connections[refdes] = connection.model_copy(
            update={"power_vias": tuple(power_vias)}
        )
    connection_analysis = source.connection_analysis.model_copy(
        update={"connections": connections}
    )
    ...
    monkeypatch.setattr(
        "spd_decap_pi.evaluation._layerwise_artwork_bounds_um",
        lambda *args, **kwargs: (0.0, 10_000.0, 0.0, 8_000.0),
    )

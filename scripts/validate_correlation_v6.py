"""Validate a release-gated current PowerSI correlation report.

Usage:
    python scripts/validate_correlation_v6.py <correlation_report.json>
...
accuracy metrics. The copied v5 structure is retained as the trust boundary;
v6 only adds current identity and conditioning evidence.
"""
...
if len(sys.argv) != 2:
    print(
        "usage: python scripts/validate_correlation_v6.py <correlation_report.json>",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
MAX_FACTOR_PIVOT_RATIO = 1.0e13
V6_PASS_MARKER = "PASS: v6 current correlation structure/conditioning validated"
...
split = obj(report.get("score_split"), "$.score_split")
if tuple(split.get("vqps_development", ())) + tuple(split.get("vqps_holdout", ())) + tuple(split.get("loaded_final_holdout", ())) != RAILS:
    fail("$.score_split", "groups do not concatenate the exact ordered 16 rails")

for mode in ("10", "12"):
    mode_rails = obj(obj(obj(report.get("runs"), "$.runs").get("candidate"), "$.runs.candidate").get(mode), f"$.runs.candidate.{mode}").get("rails", {})
    for rail, outcome_value in mode_rails.items():
        outcome = obj(outcome_value, f"$.runs.candidate.{mode}.rails.{rail}")
        terminal = obj(outcome.get("terminal_complete_batch_reuse"), f"$.runs.candidate.{mode}.rails.{rail}.terminal_complete_batch_reuse")
        diagnostics = obj(terminal.get("layer_surface_global_y_diagnostics"), f"$.runs.candidate.{mode}.rails.{rail}.pivot")
        ratio = diagnostics.get("maximum_factor_pivot_ratio")
        if isinstance(ratio, bool) or not isinstance(ratio, (int, float)) or not math.isfinite(float(ratio)) or not 1.0 <= float(ratio) <= MAX_FACTOR_PIVOT_RATIO:
            fail(f"$.runs.candidate.{mode}.rails.{rail}.pivot", "maximum_factor_pivot_ratio outside [1,1e13]")
...
summary: dict[str, Any] = {
    ...
    "validator_sha256": sha256(Path(__file__).read_bytes().replace(b"\r\n", b"\n").replace(b"\r", b"\n")).hexdigest(),
}
for mode in (10, 12):
...
if errors:
    print(f"FAIL: {len(errors)} v6 report validation error(s)", file=sys.stderr)
